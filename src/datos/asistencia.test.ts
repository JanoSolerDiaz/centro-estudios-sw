...
void test('listarRegistrosDeSlotsEnRango: una sola petición con "in" y el rango completo (R-22, cualquier estado)', async () => {
  let peticion: PeticionSimulada | undefined;
  const postgrest = crearCliente((p) => {
    peticion = p;
    return { estado: 200, cuerpo: [FILA, { ...FILA, id: 'as2', slot_id: 'slot2', estado: 'anulada', motivo_anulacion: 'Registrado por error' }] };
  ...
  const filas = await listarRegistrosDeSlotsEnRango(
    postgrest,
    ['slot1', 'slot2'],
  ...
  assert.equal(url.searchParams.get('slot_id'), 'in.(slot1,slot2)');
  assert.equal(url.searchParams.get('estado'), null);
  assert.deepEqual(url.searchParams.getAll('ocurrido_en'), ['gte."2026-09-20T22:00:00.000Z"', 'lte."2026-09-25T21:59:59.999Z"']);
  assert.equal(filas.length, 2);
});

void test('listarRegistrosDeSlotsEnRango: admite otra zona horaria explícita', async () => {
  let peticion: PeticionSimulada | undefined;
  const postgrest = crearCliente((p) => {
    peticion = p;
    return { estado: 200, cuerpo: [] };
  ...
  await listarRegistrosDeSlotsEnRango(postgrest, ['slot1'], new Date('2026-06-10T12:00:00.000Z'), new Date('2026-06-10T12:00:00.000Z'), 'UTC');
...
void test('listarRegistrosDeSlotsEnRango: sin slotIds, ninguna petición y lista vacía', async () => {
  let llamadas = 0;
  const postgrest = crearCliente(() => {
    llamadas += 1;
    return { estado: 200, cuerpo: [] };
  ...
  const filas = await listarRegistrosDeSlotsEnRango(postgrest, [], new Date('2026-09-21T12:00:00.000Z'), new Date('2026-09-25T12:00:00.000Z'));
