...
| #8 | 2026-09-05 | Alcance de datos personales / RGPD art. 9 | alta | ABIERTO | R-02 (`db/011_justificacion_ausencia.sql`, escrita y empujada el 2026-09-04, todavía sin aplicar) añade `asistencia.motivo_justificacion` con un `CHECK` a una lista cerrada que incluye `'enfermedad'` y `'cita_medica'`, más `nota_justificacion` (texto libre, sin ninguna restricción de contenido) — visible para `teacher`/`administrator` en «Registros», en el histórico y en la exportación CSV (requisito 4 de R-02, ya implementado). Tanto `'enfermedad'` como `'cita_medica'` son, por definición (art. 4.15 RGPD: información que revela el estado de salud), datos de la categoría especial del artículo 9, con independencia de que la lista sea corta y sin diagnóstico — y `nota_justificacion`, al ser texto libre sin restricción, permite a un profesor escribir voluntariamente detalle médico todavía más explícito de un menor. `HOJA_DE_RUTA.md` §0.2 (documento inmutable, "Datos personales — alcance y límites") dice literalmente: "Sigue prohibido sin decisión expresa del dueño: ... datos de salud ... y cualquier categoría especial del artículo 9 del RGPD"; la propia tabla de puntos de control de este documento clasifica esa violación como severidad alta. No hay ninguna decisión del dueño que autorice el campo: `DECISIONES_TECNICAS.md` solo registra para R-02 decisiones de mecanismo (dónde vive el `CHECK`, cómo se sustituye el trigger de historial), nunca la pregunta de categoría de dato; `SEGUIMIENTO.md` §6 no tiene ninguna entrada sobre ello; y la propia spec de R-02 (`roadmap/ROADMAP_PRODUCTO.md:190-215`, `Origen: roadmap` — la propuso el ciclo del PM, no el dueño) fija explícitamente "Bloqueo humano: ninguno". Es un contraste llamativo con el propio historial del proyecto: T-09 (bloqueo de cuenta) sí generó pregunta en §6 y fila en §7 antes de ampliar alcance sobre una base mucho menos sensible. Arrastra además dos documentos redactados el mismo día que quedaron desactualizados sin que nadie volviera sobre ellos tras el commit de R-02: `legal/POLITICA_PRIVACIDAD.md:32-34` afirma textualmente "no se guardan ... datos de salud, ... ni ninguna categoría especial del artículo 9 del RGPD", y `roadmap/PRODUCCION_T25.md` (inventario RGPD de T-25, commit `4499eaf`, anterior en la secuencia a R-01/R-02/R-03) afirma "cero dato de salud, cero categoría del artículo 9" verificado solo contra `001_esquema_inicial.sql` — cierto en el instante en que se escribió, falso hoy. La base jurídica que `POLITICA_PRIVACIDAD.md` §3 ofrece para el registro de asistencia ("interés legítimo") no serviría en ningún caso para un dato del artículo 9: ese exige consentimiento explícito u otra excepción del art. 9.2. Todavía no es una fuga en producción — `011` sigue sin aplicar en `dev` (`db/APLICADAS.md`, "Pendiente de aplicar") y no existe proyecto de producción — pero es exactamente la desviación entre lo decidido (§0.2) y lo ya escrito en `develop` que esta auditoría existe para atrapar antes de que llegue a producción, y debe bloquear tanto la aprobación final de los textos legales de T-25 como la aplicación de `011`/`012` en su forma actual hasta que el dueño decida: aceptar el campo como dato de salud (con base jurídica y medidas del artículo 9 explícitas, corrigiendo los cuatro documentos de `legal/` y el inventario de `PRODUCCION_T25.md`), reformularlo para no revelar salud (p. ej. una única categoría "justificada" sin desglose médico), o retirarlo. Verificado de forma independiente por dos vías: lectura directa de este auditor de `HOJA_DE_RUTA.md` §0.2, `db/011_justificacion_ausencia.sql`, `roadmap/ROADMAP_PRODUCTO.md:190-215` y `DECISIONES_TECNICAS.md`, y dos subagentes de investigación despachados por separado (uno centrado en T-25/`legal/`, otro en la coherencia roadmap-vs-código) que llegaron al mismo hallazgo sin verse el uno al otro. **Actualización (2026-09-06):** el duodécimo ciclo del PM (commit `f1215ca`, 2026-09-05) convirtió este hallazgo en la pregunta **#16** de §6 de `SEGUIMIENTO.md`, con las mismas tres opciones planteadas arriba, y condicionó la fila 14 de §3 (no aplicar `011`) y la fila 12 (no aprobar los textos legales de T-25) a que el dueño la responda. Verificado por este auditor leyendo la fila 16 completa: recoge las tres opciones sin recortar ni cambiar su sentido. Es la vía de resolución correcta para un hallazgo de esta clase, pero no una resolución de fondo — no hay todavía respuesta del dueño, `motivo_justificacion` sigue en el esquema tal cual y `011` sigue sin aplicar (`db/APLICADAS.md` sin cambio) —, así que **permanece ABIERTO**. **Confirmado sin cambio (2026-09-10):** la pregunta #16 de §6 de `SEGUIMIENTO.md` sigue con la columna "Respuesta" vacía (verificado por este auditor con lectura directa de la fila), la fila 14 de §3 sigue diciendo "NO aplicar todavía... antes de nada, responder la pregunta #16", y ninguna de las seis sesiones nuevas desde la pasada anterior ha tocado `011_justificacion_ausencia.sql` ni los textos de `legal/`. Sigue **ABIERTO**. **Confirmado sin cambio (2026-09-11):** la pregunta #16 de §6 sigue con la columna "Respuesta" vacía, la fila 14 de §3 mantiene literalmente la misma condición de bloqueo, `db/APLICADAS.md` no registra `011` como aplicada, y ninguno de los seis commits nuevos desde la pasada anterior (`9f0a377`, `29de11d`, `72772aa`, `ac7cafc`, `0120178`, `970381f`) toca `db/011_justificacion_ausencia.sql` ni ningún documento de `legal/`: todos son R-17, R-18, el cierre de los hallazgos #15/#17/#18/#19, dos ciclos programados sin trabajo accionable, y la apertura de R-19 (Oleada v5, sin relación con este hallazgo). Verificado de forma independiente por un subagente de investigación despachado por separado. Sigue **ABIERTO**. **Confirmado sin cambio (2026-09-12):** la fila 16 de §6 de `SEGUIMIENTO.md` sigue terminando en una columna "Respuesta" vacía (lectura directa de la fila completa), la fila 14 de §3 conserva literalmente la misma condición de bloqueo ("Antes de nada, responder la pregunta #16 de §6..."), `db/APLICADAS.md` no registra ningún cambio (`011` sigue "Pendiente de aplicar", `009` sigue siendo la última aplicada), y ninguno de los seis commits nuevos desde la pasada anterior (`970381f`, `0120178`, `c9f40ac`, `acff221`, `d7f4cf8`, `81e6afa`, `ca13186`, `62dd385` — ocho, no seis: R-19 completada, cuatro ciclos sin trabajo accionable y la apertura de R-20/Oleada v6) toca `db/011_justificacion_ausencia.sql` ni ningún documento de `legal/`. Sigue **ABIERTO**, sin ninguna vía para que cambiara sin una decisión del dueño. **Confirmado sin cambio (2026-09-13):** el único commit nuevo desde la pasada anterior (`bdd4508`, decimonoveno ciclo del PM) es bookkeeping de roadmap puro — `git diff e79de3c..HEAD` toca solo `ROADMAP_PRODUCTO.md`, `SEGUIMIENTO.md` y `HISTORIAL_SESIONES.md`, ninguno de los tres en `db/` ni en `legal/`. La fila 16 de §6 de `SEGUIMIENTO.md` sigue terminando en una columna "Respuesta" vacía (lectura directa de la fila completa), la fila 14 de §3 conserva literalmente la misma condición de bloqueo, y `db/APLICADAS.md` no registra ningún cambio (`011` sigue "Pendiente de aplicar", `009` sigue siendo la última aplicada). Sigue **ABIERTO**. **Confirmado sin cambio (2026-09-14):** el único commit nuevo desde la pasada anterior (`bbc329e`, vigésimo ciclo del PM) vuelve a ser bookkeeping de roadmap puro — `git diff 346969f..HEAD` toca solo `roadmap/ROADMAP_PRODUCTO.md`, `roadmap/SEGUIMIENTO.md` y `roadmap/HISTORIAL_SESIONES.md`, ninguno en `db/` ni en `legal/`. La fila 16 de §6 de `SEGUIMIENTO.md` sigue terminando en una columna "Respuesta" vacía, la fila 14 de §3 conserva literalmente la misma condición de bloqueo, y `db/APLICADAS.md` no registra ningún cambio (`011` sigue "Pendiente de aplicar", `009` sigue siendo la última aplicada). Sigue **ABIERTO**, décimo ciclo consecutivo sin novedad de fondo. **Confirmado sin cambio (2026-09-15):** de los tres commits nuevos desde la pasada anterior (`ea8e71f` R-20, `8937c53` P-28, `53a0bbe` apertura de R-21), ninguno toca `db/011_justificacion_ausencia.sql` ni ningún documento de `legal/` — `git diff 7477c26..HEAD --stat` lo confirma: los únicos ficheros de código tocados son de la función de auditoría de cambios (R-20) y de `db/pruebas_rls.sql` (P-28). La pregunta #16 de §6 de `SEGUIMIENTO.md` sigue con la columna "Respuesta" vacía y la fila 14 de §3 mantiene la misma condición de bloqueo (verificado por lectura directa de ambas). R-21 (Oleada v7, abierta el mismo día) es una spec nueva, no una implementación, y su propio texto cita explícitamente este hallazgo como lección aprendida ("motivo... nunca una lista cerrada... aprendido del hallazgo #8/pregunta #16 de §6") — señal de que la lección ha calado en el diseño de tareas nuevas, aunque no resuelve el hallazgo original. Sigue **ABIERTO**, undécimo ciclo consecutivo sin novedad de fondo. **Confirmado sin cambio (2026-09-16):** de los seis commits nuevos desde la pasada anterior (`454da97` R-21 — código y tests completos, migración `017` escrita y empujada, todavía sin aplicar —, cuatro rutinas de programador sin trabajo accionable y `efdbfd4`, apertura de R-22/Oleada v8), ninguno toca `db/011_justificacion_ausencia.sql` ni ningún documento de `legal/` (`git diff 2963bfe..HEAD --stat` lo confirma: los únicos ficheros de código tocados son `db/017_pausa_alumno.sql` y la sección 8n de `db/pruebas_rls.sql`, ambos de R-21). La pregunta #16 de §6 de `SEGUIMIENTO.md` sigue con la columna "Respuesta" vacía y la fila 14 de §3 mantiene la misma condición de bloqueo (verificado por lectura directa de ambas). Dato relevante en sentido contrario, ya señalado la pasada anterior y confirmado ahora en el código real: `pausa_alumno.motivo`/`motivo_anulacion` (la propia R-21, tabla nueva del mismo commit) son texto libre SIN ningún `CHECK` de lista cerrada ni opción de salud — la lección de este hallazgo ya se aplica en la práctica a las tareas nuevas, no solo se cita en su spec. Sigue **ABIERTO**, duodécimo ciclo consecutivo sin novedad de fondo. **Confirmado sin cambio (2026-09-20):** el único commit nuevo desde la pasada anterior (`53c8a60`, vigésimo sexto ciclo del PM) es bookkeeping de roadmap puro — `git diff f998218..HEAD --stat` toca solo `roadmap/HISTORIAL_SESIONES.md`, `roadmap/ROADMAP_PRODUCTO.md` y `roadmap/SEGUIMIENTO.md`, ninguno en `db/` ni en `legal/`. La fila 16 de §6 de `SEGUIMIENTO.md` sigue terminando en una columna "Respuesta" vacía (verificado por lectura directa de la fila completa), la fila 14 de §3 conserva literalmente la misma condición de bloqueo, y `db/APLICADAS.md` no registra ningún cambio (`011` sigue "Pendiente de aplicar", `009` sigue siendo la última aplicada). Sigue **ABIERTO**, decimotercer ciclo consecutivo sin novedad de fondo. **Confirmado sin cambio (2026-09-21):** el único commit nuevo desde la pasada anterior (`c1c1833`, vigésimo séptimo ciclo del PM, "sin R-XX nueva, tercer ciclo consecutivo") es bookkeeping de roadmap puro — `git diff c1c1833^..c1c1833 --stat` toca solo `roadmap/HISTORIAL_SESIONES.md`, `roadmap/ROADMAP_PRODUCTO.md` y `roadmap/SEGUIMIENTO.md`, ninguno en `db/` ni en `legal/`. La fila 16 de §6 de `SEGUIMIENTO.md` sigue terminando en una columna "Respuesta" vacía (verificado por lectura directa de la fila completa), la fila 14 de §3 conserva literalmente la misma condición de bloqueo, y `db/APLICADAS.md` no registra ningún cambio (`011` sigue "Pendiente de aplicar", `009` sigue siendo la última aplicada). Sigue **ABIERTO**, decimocuarto ciclo consecutivo sin novedad de fondo. **Confirmado sin cambio (2026-09-22):** de los seis commits nuevos desde la pasada anterior (`161c4b9` P-30, `0655e02` R-25, tres rutinas de programador sin trabajo accionable y `f58ea9c`, vigésimo octavo ciclo del PM, que abre la Oleada v12 con R-26/R-27), ninguno toca `db/011_justificacion_ausencia.sql` ni ningún documento de `legal/` — `git diff 5a34ae0..HEAD --stat -- db/ legal/` no devuelve nada. La pregunta #16 de §6 de `SEGUIMIENTO.md` sigue terminando en una columna "Respuesta" vacía (verificado por lectura directa de la fila completa), la fila 14 de §3 conserva literalmente la misma condición de bloqueo, y `db/APLICADAS.md` no registra ningún cambio (`011` sigue "Pendiente de aplicar", `008` sigue siendo la última realmente aplicada en `dev`). Sigue **ABIERTO**, decimoquinto ciclo consecutivo sin novedad de fondo. **Confirmado sin cambio (2026-09-23):** de los seis commits nuevos desde la pasada anterior (`543e46c` R-26, `5bca805` R-27, `a8ea748` P-31/P-32 — cierra los hallazgos #23/#24 de este mismo documento, sin tocar ningún fichero de `db/` ni de `legal/` —, dos rutinas de programador sin trabajo accionable y `f604799`, vigésimo noveno ciclo del PM, que abre la Oleada v13 con R-28/R-29), ninguno toca `db/011_justificacion_ausencia.sql` ni ningún documento de `legal/` — `git diff 8b2f7f4..HEAD --stat -- db/ legal/` no devuelve nada. La pregunta #16 de §6 de `SEGUIMIENTO.md` sigue terminando en una columna "Respuesta" vacía (verificado por lectura directa de la fila completa), la fila 14 de §3 conserva literalmente la misma condición de bloqueo, y `db/APLICADAS.md` no registra ningún cambio (`011` sigue "Pendiente de aplicar", `009` es la última realmente aplicada en `dev` — verificado con lectura directa de la tabla de cabecera del documento). Sigue **ABIERTO**, decimosexto ciclo consecutivo sin novedad de fondo. **Confirmado sin cambio (2026-09-24):** de los seis commits nuevos desde la pasada anterior (`df5a751` R-28 — indicador de ausencias repetidas, puramente cliente, sin migración —, `55a9084` R-29 — migración `019_aviso_ausencia_profesor.sql`, escrita y empujada, todavía sin aplicar —, tres rutinas de programador sin trabajo accionable y `b2f9c4b`, trigésimo ciclo del PM, que cierra la Oleada v13 y abre la v14 con R-30/R-31), ninguno toca `db/011_justificacion_ausencia.sql` ni ningún documento de `legal/` — `git diff 5062608..HEAD --stat -- db/ legal/` solo devuelve `019_aviso_ausencia_profesor.sql` (tabla distinta, sin relación) y los ficheros de seguimiento habituales (`MODELO.md`, `APLICADAS.md`, `pruebas_rls.sql`). La pregunta #16 de §6 de `SEGUIMIENTO.md` sigue terminando en una columna "Respuesta" vacía (verificado por lectura directa de la fila completa), la fila 14 de §3 conserva literalmente la misma condición de bloqueo, y `db/APLICADAS.md` no registra ningún cambio (`011` sigue "Pendiente de aplicar", `009` sigue siendo la última realmente aplicada en `dev`). Dato en sentido contrario, ya señalado por la propia cabecera de `019_aviso_ausencia_profesor.sql` (líneas 56-58): `aviso_ausencia_profesor.motivo` es SIEMPRE texto libre opcional, sin ninguna lista cerrada, citando explícitamente "lección del hallazgo #8/pregunta #16 de §6 sobre R-02" — la lección sigue calando en cada tabla nueva, sin que eso resuelva la original. Sigue **ABIERTO**, decimoséptimo ciclo consecutivo sin novedad de fondo. **Confirmado sin cambio (2026-09-25):** de los seis commits nuevos desde la pasada anterior (`636dbf2` R-30 — enlazado puro de cliente, sin migración —, `d3dc2c5` R-31 — importación masiva de personas de referencia, sin migración, `persona_referencia` ya existente desde T-07/T-10 —, tres rutinas de programador sin trabajo accionable y `048d887`, trigésimo primer ciclo del PM, que cierra la Oleada v14 y abre la v15 con R-32), ninguno toca `db/011_justificacion_ausencia.sql` ni ningún documento de `legal/` — `git diff 64b659b..HEAD --stat -- db/ legal/` no devuelve nada. La pregunta #16 de §6 de `SEGUIMIENTO.md` sigue terminando en una columna "Respuesta" vacía (verificado por lectura directa de la fila completa), la fila 14 de §3 conserva literalmente la misma condición de bloqueo, y `db/APLICADAS.md` no registra ningún cambio (`011` sigue "Pendiente de aplicar", `009` sigue siendo la última realmente aplicada en `dev`). Sigue **ABIERTO**, decimoctavo ciclo consecutivo sin novedad de fondo. | `db/011_justificacion_ausencia.sql`; `roadmap/ROADMAP_PRODUCTO.md:190-215`; `legal/POLITICA_PRIVACIDAD.md:32-34`; `legal/CONSENTIMIENTO_TRATAMIENTO.md`; `roadmap/PRODUCCION_T25.md` §3; origen: auditoría #8 (2026-09-05); escalado a pregunta #16 de §6 por el duodécimo ciclo del PM |
...
| #21 | 2026-09-17 | Autorización (RLS) / calidad de la batería de pruebas — privilegios de tabla | alta | RESUELTO (2026-09-18) | `db/018_baja_profesor.sql` (R-22, commit `a222862`) crea la tabla `baja_profesor` — la decimotercera tabla de `public` — pero ninguno de los tres barridos genéricos "todas las tablas" de `db/pruebas_rls.sql` se actualizó para incluirla: ni la sección 6 (barrido obligatorio de `student`, líneas 741-744), ni la sección 8 (`TRUNCATE` por `authenticated`/`administrator`, líneas 1201-1205 — la comprobación más grave del fichero, la que ya falló una vez de verdad con `perfil`), ni el barrido de `anon` (líneas 1856-1859). Verificado por `grep -n "'pausa_alumno'" db/pruebas_rls.sql`: las tres listas terminan en `pausa_alumno` (R-21, migración anterior), ninguna menciona `baja_profesor`. Exactamente el mismo patrón que el hallazgo **#10** (`RESUELTO`, 2026-09-08): una migración nueva añade tabla y la batería de barrido general no se actualiza en el mismo commit — la diferencia es que aquí la propia documentación del proyecto demuestra que la omisión no es un simple despiste de redacción, sino un paso real que no se ejecutó: la fila de `SEGUIMIENTO.md` §3 para la migración anterior (fila 20, `017_pausa_alumno`) afirma explícitamente que esa migración quedó "añadida a los barridos obligatorios de `student` —sección 6—, `TRUNCATE` —sección 5— y `anon` —sección 8f—", mientras que la fila equivalente para esta migración (fila 21, `018_baja_profesor`) y la entrada correspondiente de `db/APLICADAS.md` describen con detalle los 17 casos propios de la sección 8o (rechazo de rol, tipo inválido, exclusión por asistencia/duplicado, etc.) sin afirmar en ningún momento que se tocaran las tres listas genéricas — no se perdió al escribir la nota, no se hizo. Riesgo real hoy: bajo, no nulo — `018_baja_profesor.sql:182-184` sí sigue el patrón correcto (`revoke all ... from anon, authenticated, service_role` seguido solo de `grant select ... to authenticated`), confirmado por lectura directa, así que `student`/`anon` ya están bloqueados contra `baja_profesor` por el `GRANT`/RLS actual. Lo que falta es la red de regresión que se supone debe demostrarlo y que atraparía el descuido inverso en una migración futura: es precisamente la clase de fallo que el punto de control permanente "Privilegios de tabla" advierte que "se reintroduce solo" en cada tabla nueva de Supabase, y `baja_profesor` es hoy la única tabla del esquema sin un caso negativo ejecutable de `student`, de `anon` ni de `TRUNCATE` en su contra. Confirmado también en ejecución por este auditor (no solo por lectura): `npm run typecheck`, `npm run lint` y `npm test` (**1719/1719**, coincide con `SEGUIMIENTO.md`) en verde — la suite no falla por esto porque nunca llega a ejercitar esa celda de la matriz. Arreglo propuesto: añadir `'baja_profesor'` a los tres arrays (líneas 743, 1204 y 1858 de `db/pruebas_rls.sql`), mismo patrón exacto que ya usó la corrección del hallazgo #10. **Resuelto (2026-09-18):** P-29 (commit `65d0a34`, 2026-09-17) añadió `'baja_profesor'` a los tres arrays exactamente como se proponía — verificado por este auditor con `grep -n "baja_profesor" db/pruebas_rls.sql`: aparece ahora en las tres listas genéricas (líneas 743, 1204 y 1858) además de en su propia sección 8o. `npm run typecheck`, `npm run lint` y `npm test` (**1734/1734**) en verde tras el cambio. Cierra sin ninguna acción pendiente. | `db/018_baja_profesor.sql:182-184`; `db/pruebas_rls.sql` líneas 743, 1204 y 1858 (arreglo verificado); `roadmap/SEGUIMIENTO.md` fila 21 de §3 (compárese con la fila 20, que sí declara la actualización de los tres barridos); `db/APLICADAS.md` (entrada de `018_baja_profesor.sql`, mismo silencio); precedente `#10` (`RESUELTO` 2026-09-08, mismo patrón exacto sobre `cierre_centro`/`excepcion_slot`); origen: auditoría de este auditor, 2026-09-17; resuelto por P-29, commit `65d0a34` |
| #22 | 2026-09-19 | Funcionalidad / coherencia código-esquema aplicado — `actualizar_asistencia` | alta | RESUELTO (2026-09-21) | `src/datos/asistencia.ts#actualizarAsistencia` (usada por "Registros", T-21, **`COMPLETADA`** desde 2026-09-02, y ahora también por "Anular" de pasar lista, R-24, commit `004d778`, 2026-09-18) envía en **todas** sus llamadas los 13 parámetros de la firma completa de `actualizar_asistencia` — incluidos `p_justificar`/`p_motivo_justificacion`/`p_nota_justificacion` (añadidos por `db/011_justificacion_ausencia.sql`) y `p_marcar_salida`/`p_ocurrido_en_salida` (añadidos por `db/012_registro_salida.sql`) — sin ninguna rama condicional según qué acción se esté usando. Verificado por lectura directa de `src/datos/asistencia.ts:126-140`. Pero la función realmente desplegada en `dev` sigue siendo la de `db/008_rpc_actualizar_asistencia.sql` (última fila de `db/APLICADAS.md`, 8 parámetros: `p_asistencia_id`, `p_alumno_id`, `p_slot_id`, `p_ocurrido_en`, `p_anular`, `p_motivo_anulacion`, `p_nota`, `p_nota_provista`) — `011` y `012` están escritas y empujadas pero **todavía sin aplicar** (`db/APLICADAS.md`, sección "Pendiente de aplicar"), y `011` en concreto tiene aplicación **expresamente bloqueada** por el hallazgo #8 de esta misma tabla (RGPD artículo 9, `ABIERTO` desde 2026-09-05, sin fecha de resolución a la vista). PostgREST resuelve una llamada RPC por coincidencia exacta de los nombres de parámetro contra la firma de la función en el servidor; un nombre que la función no declara no se ignora, hace fallar la resolución completa de la llamada (error de función no encontrada, HTTP 404/`PGRST202`), incluso si el resto de parámetros son correctos y llevan su valor por defecto. Consecuencia concreta: **hoy, contra la base de datos real de `dev`, ninguna llamada a `actualizar_asistencia` puede tener éxito** — ni "editar"/"cambiar alumno"/"ajustar hora"/"anular" desde "Registros" (T-21, la pantalla de revisión diaria, en producción funcional desde hace más de dos semanas según el roadmap) ni "Anular" desde pasar lista (R-24, recién fusionada). No es el mismo caso que R-02/R-03 (`BLOQUEADA` correctamente, de forma explícita, a la espera de su propia migración: eso es correcto y está bien señalizado) ni el de R-04/R-13 (que declaran por escrito que su informe fallará con error de servidor hasta que `013`/`014` se apliquen, decisión razonada y aceptada en `DECISIONES_TECNICAS.md`): aquí lo que se rompió es una función **compartida** por una tarea ya `COMPLETADA` y verificada en ejecución en su día (T-21, `db/APLICADAS.md` fila 008, "89 comprobaciones, 0 fallidas"), como efecto colateral silencioso de que sesiones posteriores (R-02 el 2026-09-04, R-03 el 2026-09-04) ampliaron esa misma función de cliente para adelantar trabajo contra una firma de servidor que todavía no existe — y ninguna sesión posterior (incluida la de R-24, que añade un tercer punto de llamada a la misma función ya rota) lo ha detectado. La razón de que nadie lo haya visto es estructural, no un despiste puntual: **toda la batería de 1748 tests corre contra un doble de `ClientePostgrest` que nunca valida el nombre de un parámetro contra una firma de PostgreSQL real** — sólo comprueba qué claves construye el cliente, nunca si esas claves existirían en el servidor de verdad —, y nadie ha vuelto a ejecutar `npm run probar-rls` (la única herramienta que golpea la base de datos real) desde antes de que R-02 tocara esta función, porque su ejecución depende del dueño y las migraciones que la motivarían siguen bloqueadas. Coincide exactamente con lo que el auditor debe vigilar en este proyecto: "una suite de tests triviales que pasa siempre es peor que no tener tests, porque da una falsa sensación de red" — aquí la falsa sensación cubre una función ya en uso diario documentado, no solo una nueva sin estrenar. **Mientras el hallazgo #8 (RGPD) no se resuelva, `011` no debe aplicarse — y con ella tampoco `012`, que la da por base (`create function` con la firma completa, no una segunda sobrecarga) —, así que esta rotura de T-21/R-24 seguirá activa indefinidamente si nadie actúa**: las opciones no las puede decidir el auditor (ni el PM en solitario, por tocar una RPC ya `COMPLETADA` con usuarios reales dependiendo de ella): (a) acelerar la respuesta del dueño a la pregunta #16 de §6 de `SEGUIMIENTO.md` para poder aplicar `011`+`012` juntas cuanto antes; (b) una migración correctiva que reintroduzca **solo** la firma de `012` (marcar/ajustar salida) sin los tres parámetros de `011` (justificar), desacoplando R-03 de la RPC bloqueada por RGPD; o (c) revertir temporalmente `actualizarAsistencia`/`anularAsistencia`/`marcarSalidaAsistencia` a la firma de 8 parámetros de `008` (con los campos de R-02/R-03 detrás de una rama condicional hasta que su migración exista), para que T-21 y R-24 vuelvan a funcionar de verdad en `dev` sin esperar a que se resuelva el RGPD de R-02. **Confirmado sin cambio (2026-09-20):** el único commit nuevo desde que se abrió este hallazgo (`53c8a60`, vigésimo sexto ciclo del PM, 2026-09-19) es la propia sesión de PM que lo registró en `SEGUIMIENTO.md` y decidió correctamente no generar ninguna R-XX, dejándolo anotado para el programador (§0.3) — no ha corrido ninguna sesión de programador desde entonces que pudiera tomarlo. Verificado de nuevo por lectura directa: `src/datos/asistencia.ts:126-140` sigue enviando los 13 parámetros sin ninguna rama condicional; `db/APLICADAS.md` sigue sin `011`/`012` aplicadas. Ejecutado de nuevo en limpio (`npm ci && npm run typecheck && npm run lint && npm test`): los tres primeros sin errores y **1748/1748** tests en verde, que sigue sin detectar el problema por la misma razón estructural ya señalada (el doble de `ClientePostgrest` no valida nombres de parámetro contra una firma real). Sigue **ABIERTO**. **Nota de proceso, todavía no un fallo:** esta es la primera pasada desde la apertura del hallazgo en la que ninguna sesión de programador lo ha atendido como P-XX urgente, pese a que §0 de `HOJA_DE_RUTA.md` exige atenderlo "antes de la cola" — pero el único ciclo transcurrido es el propio ciclo de PM que lo registró, así que todavía no hay ninguna sesión de programador que haya incumplido la regla. Si la próxima sesión de programador retoma la cola normal sin pasar antes por este hallazgo, eso sí sería una desviación de proceso a escalar. **Confirmado sin cambio (2026-09-21):** el único commit nuevo desde la pasada anterior (`c1c1833`, vigésimo séptimo ciclo del PM, 2026-09-20) vuelve a ser una sesión de PM, no de programador — `git diff --stat` confirma que toca solo `roadmap/HISTORIAL_SESIONES.md`, `roadmap/ROADMAP_PRODUCTO.md` y `roadmap/SEGUIMIENTO.md`, y su propio texto ("R-25... sigue `PENDIENTE`, sin que ningún programador la haya tomado todavía") registra explícitamente que sigue sin haber ninguna sesión de programador que pudiera haber incumplido la regla de atender este hallazgo "antes de la cola" (§0.3 de `HOJA_DE_RUTA.md`). Verificado de nuevo por lectura directa: `src/datos/asistencia.ts:126-140` sigue enviando los 13 parámetros sin ninguna rama condicional; `db/APLICADAS.md` sigue sin `011`/`012` aplicadas (`008`, 8 parámetros, sigue siendo la firma real en `dev`). Ejecutado de nuevo en limpio (`npm ci && npm run typecheck && npm run lint && npm test`): los tres primeros sin errores y **1748/1748** tests en verde — mismo número exacto que la pasada anterior, coherente con que ningún fichero de `src/` cambió, y que sigue sin detectar el problema por la misma razón estructural (el doble de `ClientePostgrest` no valida nombres de parámetro contra una firma real). Sigue **ABIERTO**. **Nota de proceso, ahora son ya TRES ciclos de PM consecutivos (25º→27º) sin que ninguna sesión de programador haya corrido:** la cola de trabajo (R-25, Oleada v11) lleva PENDIENTE desde el 2026-09-18 sin que nadie la tome, con dos hallazgos `ABIERTO` de severidad alta esperando (este y el #8) — el proyecto no está roto ni desviado (el propio PM sigue señalizando ambos correctamente en cada ciclo, y no está fabricando trabajo para rellenar el vacío), pero conviene que el dueño sepa que la ejecución está de facto parada: nadie puede aplicar `011`/`012` sin la respuesta a la pregunta #16, y nadie ha tomado R-25 ni este hallazgo #22 por otra vía. No se convierte en un hallazgo nuevo de este auditor porque no hay ningún documento ni regla incumplida — es una observación de ritmo, no de coherencia ni de seguridad. **RESUELTO (2026-09-22, verificado por este auditor):** el mismo día en que se escribió esta nota de proceso, la sesión de programador de hoy (`161c4b9`, P-30) tomó el hallazgo antes de la cola normal, exactamente como exigía §0.3 de `HOJA_DE_RUTA.md` — la nota de proceso de la pasada anterior no llegó a convertirse en una desviación real. Aplicó la opción (c) que el propio hallazgo proponía: `actualizarAsistencia` (`src/datos/asistencia.ts:144-166`) vuelve a construir SOLO los 8 parámetros de la firma real de `008_rpc_actualizar_asistencia.sql`; pedir `justificar`/`marcarSalida`/`ocurridoEnSalida` ahora lanza `AccionNoDisponibleTodavia` (clase nueva en `erroresDominio.ts`, con su propio mensaje en `mensajesAbuso.ts`) ANTES de tocar la red o el limitador de tasa de cliente — verificado por lectura directa de las tres piezas y por un test nuevo que fija la lista exacta y ordenada de las ocho claves del cuerpo (`asistencia.test.ts`, "el cuerpo nunca lleva las claves de R-02/R-03"), más un test por cada acción bloqueada que confirma cero llamadas a la red y que `marcarSalidaAsistencia` no consume el limitador. Ejecutado de nuevo en limpio (`npm ci && npm run typecheck && npm run lint && npm test`): typecheck y lint sin errores, **1770/1770** tests en verde. Corrección sólida y bien verificada, con una salvedad real que se abre como hallazgo nuevo **#23** más abajo: el guardián corta la llamada de RED, pero los controles de "Justificar" (`pantallaRegistrosSlot.ts`, Registros) y "Marcar salida" (`pantallaPasarLista.ts`, pasar lista) siguen ofreciéndose en pantalla sin ninguna condición sobre el estado de la migración, así que un profesor o administrator puede rellenarlos y pulsar el botón para recibir, ahora sí, un mensaje limpio de "todavía no disponible" en vez del error de servidor confuso de antes de P-30 — una mejora real, pero no el cierre completo de la experiencia rota mientras `011`/`012` sigan bloqueadas. | `src/datos/asistencia.ts:126-140` (payload de `actualizarAsistencia`, sin condicionales); `db/008_rpc_actualizar_asistencia.sql:80-89` (firma real desplegada, 8 parámetros); `db/011_justificacion_ausencia.sql:149-161` y `db/012_registro_salida.sql:161-175` (firmas ampliadas, ninguna aplicada, confirmado en `db/APLICADAS.md`); hallazgo #8 (bloqueo de `011`); `roadmap/SEGUIMIENTO.md` fila de T-21 ("código y tests COMPLETOS") y fila de R-24 ("`COMPLETADA`, 2026-09-18"); origen: auditoría de este auditor, 2026-09-19, verificado con `npm ci && npm test` (1748/1748 en verde, no detecta el problema) y lectura directa de las tres firmas SQL |
| #23 | 2026-09-22 | Funcionalidad / UX — controles muertos por migración bloqueada | media | RESUELTO (2026-09-23) | P-30 (commit `161c4b9`, hallazgo #22) corrige que `actualizarAsistencia` rompiera **todas** sus acciones, pero deja sin tocar los controles de pantalla que solo tienen sentido cuando `011`/`012` estén aplicadas: el bloque "Justificar" de `pantallaRegistrosSlot.ts` (líneas ~775-823, condicionado únicamente a `puedeJustificarAusencia(registro)` — el registro está `ausente`, sin comprobar en ningún momento si la migración que lo sostiene existe) y el botón "Marcar salida" de `pantallaPasarLista.ts` (`manejarSalida`, condicionado solo a `puedeMarcarSalida(tarjeta.asistencia)` — la tarjeta está `registrado`). Ninguno de los dos comprueba `accionPendienteDeMigracion` ni ninguna señal equivalente antes de ofrecerse: ambos se pintan siempre que el estado del registro lo permite, un profesor o administrator puede rellenar el formulario y pulsar el botón, y la única consecuencia de P-30 es que el error que reciben pasó de ser confuso (antes de hoy) a ser el mensaje limpio de `AccionNoDisponibleTodavia` ("Esta función todavía no está disponible en este centro..."). El botón de pasar lista es especialmente visible: `puedeMarcarSalida` se cumple sobre cualquier tarjeta ya registrada como presente, así que aparece en el flujo diario principal de CADA profesor, no en una pantalla secundaria de revisión. `011` está bloqueada por el hallazgo #8 (RGPD artículo 9), abierto desde 2026-09-05 y sin fecha de resolución a la vista — con el dueño sin responder la pregunta #16 de §6 durante 15 ciclos consecutivos del PM, estos dos controles pueden seguir ofreciéndose y fallando así indefinidamente, no unos pocos días. Verificado que no hay ningún test que cubra este camino: `grep -n "AccionNoDisponibleTodavia\|no está disponible" src/ui/pantallaRegistrosSlot.test.ts src/ui/pantallaPasarLista.test.ts` no devuelve nada — nadie ha escrito ni comprobado qué ve el usuario al pulsar estos dos botones hoy. No es un hallazgo de seguridad (`ejecutar()`/`manejarSalida()` manejan el error con corrección: `guardando` se libera, no se pierde la fila, no hay estado colgado) ni de datos (nada se escribe a medias), y P-30 en sí es una corrección sólida de lo que arregló — es que dejó una segunda mitad sin recorrer. Corrección mínima razonable: ocultar los dos controles (mismo criterio que ya usa el propio `accionPendienteDeMigracion` para decidir "disponible o no", expuesto como una función que la pantalla también pueda consultar antes de pintar el bloque), no solo interceptar el clic. **Resuelto:** **P-31** (commit `a8ea748`, 2026-09-22) hizo exactamente esa corrección mínima: `datos/asistencia.ts` gana `justificarAusenciaDisponible()`/`marcarSalidaDisponible()` sobre el mismo `accionPendienteDeMigracion` (tipo de entrada estrechado a solo los campos relevantes); `pantallaRegistrosSlot.ts`/`pantallaPasarLista.ts` las reciben como dependencia OPCIONAL (`deps.justificarAusenciaDisponible?()`/`deps.marcarSalidaDisponible?()`, `?? true` si se omiten) y ocultan el bloque completo mientras devuelvan `false`, en vez de importar las funciones directamente — así los ~15 tests existentes del flujo completo (doble toque, reconciliación de conflicto) siguen en verde contra los dobles, sin fingir la migración aplicada. Wireadas en `aplicacion.ts` a las funciones reales en los tres puntos de montaje (`registros` de `administrator`/`teacher` línea 391-392 y 769-770, `pasar-lista` línea 868). Verificado por este auditor con lectura directa de los tres puntos de montaje y de `pantallaRegistrosSlot.ts:644-654,785-788` / `pantallaPasarLista.ts:557-566` (el control ahora comprueba `salidaDisponible`/`deps.justificarAusenciaDisponible?.()` ANTES de decidir qué pintar, no solo antes de enviar), y de los 5 tests nuevos que fijan el caso oculto con `() => false` (`asistencia.test.ts:551-557`, `pantallaPasarLista.test.ts:1235-...`, `pantallaRegistrosSlot.test.ts:632-...,887-...`). Suite completa en verde: 1816/1816 (`npm test`), typecheck y lint limpios. | `src/ui/pantallaRegistrosSlot.ts:644-654,785-788`; `src/ui/pantallaPasarLista.ts:557-566` (`manejarSalida`, control "Marcar salida"); `src/datos/asistencia.ts:126-150` (`accionPendienteDeMigracion`, `justificarAusenciaDisponible`, `marcarSalidaDisponible`); `src/ui/aplicacion.ts:391-392,769-770,868`; hallazgo #22 (P-30, commit `161c4b9`); origen: auditoría de este auditor, 2026-09-22; resuelto por P-31, commit `a8ea748` |
| #24 | 2026-09-22 | Gobernanza documental | baja | RESUELTO (2026-09-23) | La "Matriz rol × tabla × operación" de `roadmap/DECISIONES_TECNICAS.md` (§ posterior al registro append-only, "referencia de consulta" que este propio documento de auditoría usa como punto de contraste para RLS) no tiene ninguna fila para `pausa_alumno` (R-21, `db/017_pausa_alumno.sql`, completada el 2026-09-15/16), aunque sí incluye una fila para `baja_profesor` (R-22, `018_baja_profesor.sql`), la tarea inmediatamente siguiente. Verificado por lectura directa de la sección completa (`sed -n '352,375p' roadmap/DECISIONES_TECNICAS.md`): docena de filas — `centro_estudios`, `alumno` (identificación y contacto, dos filas), `persona_referencia`, `slot_horario`, `asistencia`, `asistencia_historial`, `evento_error`, `limite_tasa`, bucket `avatares`, `cierre_centro`, `excepcion_slot`, `baja_profesor` — ninguna de `pausa_alumno`. No es un fallo de seguridad real: las políticas de `pausa_alumno` sí existen y son correctas (`pausa_alumno_admin_leer_todas`, `pausa_alumno_teacher_leer_de_sus_alumnos` en `db/017_pausa_alumno.sql:153-160`, sin ninguna política para `student`, confirmado en el barrido de `student` y en el de `TRUNCATE` de `db/pruebas_rls.sql`, ambos con las 13 tablas de `public` incluida `pausa_alumno`) — es la tabla de referencia la que quedó incompleta, exactamente el mismo patrón de desfase documental que ya causó los hallazgos #4/#5/#6/#9/#11 (todos `RESUELTO`). Al atenderlo: añadir la fila de `pausa_alumno` a la matriz, mismo formato que las demás. **Resuelto:** **P-32** (commit `a8ea748`, 2026-09-22, mismo commit que P-31) añadió la fila que faltaba. Verificado por este auditor con lectura directa de `roadmap/DECISIONES_TECNICAS.md:383` (la fila de `pausa_alumno`, mismo formato que las demás — lee vía RPC `declarar_pausa_alumno()`/`cancelar_pausa_alumno()`/`acortar_pausa_alumno()`, sin GRANT directo, y describe correctamente el alcance de lectura del `teacher` acotado a sus propios alumnos). | `roadmap/DECISIONES_TECNICAS.md:352-383` (matriz, fila añadida); `db/017_pausa_alumno.sql:153-160` (políticas reales, correctas); `db/pruebas_rls.sql` (barridos de `student`/`TRUNCATE`, ambos completos); origen: auditoría de este auditor, 2026-09-22; resuelto por P-32, commit `a8ea748` |
...
`636dbf2` (R-30, de aviso a excepción en un enlace), `d3dc2c5` (R-31, importación masiva de
personas de referencia), tres rutinas programadas de programador sin trabajo accionable (`c0825d1`,
`d1f78d4`, `eeaa41d`) y `048d887` (trigésimo primer ciclo del PM, cierra la Oleada v14 y abre la v15
con R-32, sin código). `git checkout develop && git pull origin develop`: fast-forward limpio hasta
`048d887`. **Ningún cambio bajo `db/` ni bajo `legal/`** en todo el rango (`git diff 64b659b..HEAD
--stat -- db/ legal/` no devuelve nada): las dos tareas de este lote son, respectivamente, cliente
puro y una tabla que ya tenía RLS desde T-07/T-10. Por eso esta pasada no repite la revisión línea a
línea de ningún fichero SQL nuevo — no lo hay — y se concentra en el código de R-30/R-31 y en la
coherencia documental del cierre de oleada.

**R-30 (de aviso a excepción, en un enlace) revisada contra alcance y superficie de acceso:**
`pantallaPanelCentro.ts` añade un botón «Declarar sustitución o cancelación» en el bloque de avisos
de ausencia (R-29), detrás de una dependencia opcional nueva `deps.irARegistro` que reutiliza tal
cual el mismo patrón ya usado por R-20 en `pantallaRegistroAuditoria.ts` — sin ruta, RPC, columna ni
tabla nueva; navega con los propios `profesor_id`/`slot_id`/`fecha_sesion` del aviso, ya visibles
para `administrator` en ese mismo bloque. Requisito 4 (nunca ofrecer el enlace en un aviso ya
atendido) se cumple por construcción, no por lógica añadida: `listarAvisosAusenciaPendientes` ya
filtra por `estado = 'pendiente'`, así que un aviso atendido nunca llega a esa lista — verificado
leyendo la función, no solo el mensaje de commit. Sin efecto sobre ningún punto de control
permanente: no toca `asistencia`, RLS, ni ningún dato de menor.

**R-31 (importación masiva de personas de referencia) revisada contra el punto de control "datos de
personas de referencia" y contra alcance de dato:** el nuevo bloque en `pantallaImportacionMasiva.ts`
solo se cablea dentro de `mostrarAppAdministrador` (`src/ui/aplicacion.ts`, verificado por lectura
directa del diff) — ninguna ruta de `teacher` importa `listarPersonasReferenciaExistentes` ni
`importarPersonasReferencia`, coherente con el requisito 6 de su propia spec ("reservado a
`administrator`, igual que R-08 y T-13") y con la matriz de `DECISIONES_TECNICAS.md`, que ya reserva
`persona_referencia` en exclusiva a ese rol. El alta escribe con un `INSERT` directo por PostgREST
contra `persona_referencia` (`datos/importacionMasiva.ts#importarPersonasReferenciaValidados`), sin
RPC — correcto aquí porque la política `persona_referencia_admin_todo` (`FOR ALL`, T-10) ya reduce a
cero filas esa misma operación para cualquier rol que no sea `administrator`; no es el mismo caso que
`asistencia`, donde la RPC existe para fijar columnas que el cliente no debe elegir (autor, hora), no
solo para restringir el rol. Columnas del CSV verificadas una a una contra la cabecera obligatoria
(`CABECERA_PERSONAS_REFERENCIA_CSV`, `dominio/importacionPersonasReferencia.ts:31-40`): nombre,
apellidos, teléfono, email — exactamente las de T-13, **sin ningún campo `relacion`** (la spec cita
explícitamente que esa columna sigue pendiente de la pregunta #9 de §6, todavía sin decisión del
dueño, y el código no la incluye). Emparejamiento de alumno por nombre y apellidos EXACTOS (no
acento-insensible, a diferencia del CSV de alumnos de R-08) y deduplicación de personas de referencia
acotada POR ALUMNO (`Map` por `alumno_id`) para que dos padres homónimos de dos hermanos no se pisen
entre sí — las dos decisiones están registradas en `DECISIONES_TECNICAS.md` con su motivo, coherentes
con los requisitos 1 y 3 de la spec, literal por literal. 27 tests nuevos
(`importacionPersonasReferencia.test.ts`, 16 casos) cubren precisamente los bordes que importan para
un dato de contacto de terceros: coincidencia exacta que una variante de tildes no debe superar,
alcance del duplicado limitado al mismo alumno, dos filas del mismo fichero para el mismo alumno, un
alumno con varias personas de referencia distintas — no es una batería trivial que solo ejercita el
camino feliz.

**Coherencia roadmap ↔ código:** `ROADMAP_PRODUCTO.md` (specs de R-30/R-31/R-32) y
`DECISIONES_TECNICAS.md` (cinco decisiones nuevas de mecanismo, ninguna de alcance de dato) coinciden
con lo implementado; §1 de `SEGUIMIENTO.md` refleja R-30/R-31 como `COMPLETADA` el mismo día que su
commit, y §7 (desviaciones) no gana ninguna fila nueva porque no hay ningún requisito incumplido ni
alcance ampliado sin decisión que registrar — confirmado con `git diff 64b659b..HEAD -- roadmap/
SEGUIMIENTO.md`, que solo toca la cabecera/bitácora y la fila de §1, no §7. `HOJA_DE_RUTA.md`
permanece intacto (0 líneas de diff), igual que el runner de migraciones y sus guardas
(`herramientas/migraciones`).

**Puntos de control permanentes:** sin cambio bajo `db/` en todo el rango, así que el estado
verificado en la pasada anterior (2026-09-24) sobre `019_aviso_ausencia_profesor.sql` y sobre las
migraciones ya aplicadas (`001`-`009`) sigue vigente sin necesidad de repetir esa lectura línea a
línea. Verificado de forma independiente lo que sí puede cambiar sin tocar `db/`: `package.json`
sigue con `dependencies` vacío y las mismas `devDependencies` (`npm ci` reinstala 130 paquetes, 0
vulnerabilidades); `git ls-files | grep -i '\.env'` solo devuelve `.env.ejemplo`, y `.gitignore` sigue
excluyendo `.env`/`.env.local`; ningún `service_role`, contraseña ni access token real en el diff ni
en el repositorio (solo referencias a nombres de variable de entorno, en `DEVELOPERS.md` y
`roadmap/prompts-agentes.md`, como ya verificaban pasadas anteriores).
...
— **1894 pruebas, 0 fallidas** (subida desde las 1867 de la pasada anterior, coherente con los 27
tests nuevos de R-31) + `npm run build` limpio. Los cuatro comandos ejecutados directamente por este
...
personales de R-02/artículo 9) sigue exactamente igual — decimoctavo ciclo consecutivo sin movimiento
de fondo, todavía a la espera de que el dueño responda la pregunta #16 de §6 — y el trabajo nuevo de
esta pasada (R-30, R-31) no introduce ningún dato personal nuevo, respeta la reserva de personas de
referencia a `administrator` y mantiene la cobertura de pruebas sobre lógica de negocio real, no
superficial.

### Auditoría 2026-09-24

**Alcance real de esta pasada — seis commits desde la anterior (`5062608`, 2026-09-23):**
`df5a751` (R-28, indicador de ausencias repetidas en pasar lista y Mi horario), `55a9084` (R-29,
aviso de ausencia del profesor, sin salir de la aplicación), tres rutinas programadas de
programador sin trabajo accionable (`de26e66`, `08509e4`, `2f26f9e`) y `b2f9c4b` (trigésimo ciclo
del PM, cierra la Oleada v13 y abre la v14 con R-30/R-31, sin código). `git checkout develop &&
git pull origin develop`: fast-forward limpio hasta `b2f9c4b`. Único cambio bajo `db/`: la
migración nueva `019_aviso_ausencia_profesor.sql` (R-29) — escrita y empujada, **todavía sin
aplicar** en `dev` (`db/APLICADAS.md` sigue con `009` como última aplicada; fila 22 de §3 de
`SEGUIMIENTO.md`, `PENDIENTE`).

**`019_aviso_ausencia_profesor.sql` revisada línea a línea contra los doce puntos de control
permanentes:** tabla nueva con RLS habilitada desde el propio fichero (nunca "próximo lote de
políticas"); `revoke all` a `anon`/`authenticated`/`service_role` seguido de `grant select` (solo
lectura) a `authenticated` y del `grant` completo a `service_role` — **sin ningún `GRANT` de
`INSERT`/`UPDATE`/`DELETE`/`TRUNCATE` a `authenticated`**, la clase de fallo que ya se coló una vez
en `perfil` (000b) y otra en `pausa_alumno`/`baja_profesor` (hallazgo #21) por los privilegios por
defecto de Supabase; aquí queda evitado desde el primer commit de la tabla, no corregido después.
Toda escritura pasa por dos RPC `SECURITY DEFINER` (`avisar_ausencia_profesor`,
`marcar_aviso_ausencia_atendido`) que fijan `profesor_id`/`registrado_en`/`atendido_por`/
`atendido_en` en el servidor y comprueban el rol (`teacher`/`administrator` respectivamente) con
`raise exception ... using errcode = '42501'`, nunca en el cliente; `avisar_ausencia_profesor`
además comprueba la pertenencia del slot (`v_slot.profesor_id <> auth.uid()`) y la ventana
temporal (no pasado, no una sesión de hoy ya en curso) contra `now()` del servidor. Sin política
para `student` en ninguna parte de la tabla — con RLS habilitada y sin `GRANT`, el rol queda fuera
por defecto, no por una regla explícita que pudiera olvidarse. `motivo` es texto libre opcional sin
ningún `CHECK` de lista cerrada, y su propio comentario de columna cita explícitamente la lección
del hallazgo #8 (ver el registro de hallazgos, abajo). `db/pruebas_rls.sql` incorpora la tabla
nueva a los dos barridos genéricos que el hallazgo #21 dejó como lección permanente — el barrido
`student` (línea 744) y el de `TRUNCATE`/`anon` (línea 1205) — además de su propia sección 8p (18
comprobaciones: alta propia, slot ajeno rechazado, fecha fuera del día de la semana rechazada,
motivo solo espacios rechazado, sesión pasada y de hoy ya en curso rechazadas, segundo aviso de la
MISMA sesión desde OTRO alumno del mismo slot rechazado mientras el primero sigue pendiente y
permitido de nuevo tras quedar atendido, `administrator`/`student` rechazados en
`avisar_ausencia_profesor`, `teacher`/`student` rechazados en `marcar_aviso_ausencia_atendido`,
lectura aislada entre profesores y `administrator` viendo todos). `DECISIONES_TECNICAS.md` gana su
fila de la matriz rol × tabla × operación para la tabla nueva, coherente con el SQL real.

**R-28 revisado contra minimización de datos y el principio de "una consulta de más, nunca una por
elemento" (§0.2):** `dominio/avisoAusenciasRepetidas.ts` reutiliza
`rankingAusenciasSinJustificarPanelCentro` (R-11) tal cual, sin ninguna función nueva de conteo,
tal como exige el requisito 3 de su propia spec — verificado leyendo la función, no solo el
mensaje de commit. Sin migración, sin RPC nueva, sin ninguna columna ni dato personal añadido: el
indicador es un recuento de ausencias ya visibles para ese `teacher` sobre sus propios alumnos.
`pantallaPasarLista.ts`/`pantallaMiHorario.ts` piden `listarAusenciasRecientes` UNA vez por
`cargar()` dentro del mismo `Promise.all` que ya agrupaba el resto de datos de la pantalla, no una
petición por card. Wireado en `aplicacion.ts` solo dentro del router de `teacher`, acotado por
`profesorId: perfil.id` — ningún profesor puede ver el ranking de otro ni el del centro completo
(ese sigue siendo exclusivo del Panel de centro, `administrator`, sin cambios). R-28 abrió la
pregunta #18 de §6 (umbral/ventana configurables), sin bloquear nada mientras no haya respuesta —
patrón ya asentado por el proyecto para decisiones de producto no críticas.

**Coherencia roadmap ↔ código:** `ROADMAP_PRODUCTO.md` (specs de R-28/R-29) y
`DECISIONES_TECNICAS.md` (seis decisiones de mecanismo, ninguna de alcance de dato) coinciden con
lo implementado; ninguna de las dos tareas necesitó fila nueva en §7 de `SEGUIMIENTO.md` — no hay
ningún requisito incumplido ni ningún alcance ampliado sin decisión, solo decisiones de mecanismo
ya documentadas en su sitio. `HOJA_DE_RUTA.md` permanece intacto (0 líneas de diff), igual que el
runner de migraciones y sus guardas (`herramientas/migraciones`).

**Calidad de la batería de pruebas y del código, verificada en ejecución, no solo leída:**
`npm run typecheck` limpio, `npm run lint` limpio, `npm test` — **1863 pruebas, 0 fallidas**
(subida desde las ~1780 de la pasada anterior, coherente con las pruebas nuevas de R-28/R-29). Los
tests nuevos de dominio (`avisoAusenciaProfesor.test.ts`, `avisoAusenciasRepetidas.test.ts`) cubren
casos límite reales (una sesión que empieza exactamente ahora, envoltura de semana, domingo como
día 7), no solo el camino feliz; los de `datos/avisosAusenciaProfesor.test.ts` verifican que los
errores del servidor (`SinPermiso`, `Conflicto`) se propagan sin perderse. Ningún secreto
(`service_role`, contraseña, access token) en el diff; `package.json` sigue con `dependencies`
inexistente, sin ningún framework ni SDK de Supabase nuevo.

**Conclusión:** pasada limpia, sin hallazgo nuevo. El único hallazgo `ABIERTO` (#8, alcance de
datos personales de R-02/artículo 9) sigue exactamente igual — decimoséptimo ciclo consecutivo sin
movimiento de fondo, todavía a la espera de que el dueño responda la pregunta #16 de §6 — y el
trabajo nuevo de esta pasada (R-28, R-29) refuerza la lección que ese hallazgo dejó en vez de
repetir el problema.

### Auditoría 2026-09-23

**Alcance real de esta pasada — seis commits desde la anterior (`8b2f7f4`, 2026-09-22):**
`543e46c` (R-26, recordatorio local antes de una sesión), `5bca805` (R-27, alta de una sesión de
grupo completa), `a8ea748` (P-31/P-32, cierra los hallazgos #23 y #24 de este mismo documento),
dos rutinas programadas de programador sin trabajo accionable (`2e61129`, `19e893b`) y `f604799`
(vigésimo noveno ciclo del PM, cierra la Oleada v12 y abre la v13 con R-28/R-29, sin código). `git
checkout develop && git pull origin develop`: fast-forward limpio hasta `f604799`. `git diff
8b2f7f4..HEAD --stat` confirma que **no hay ningún cambio bajo `db/`**: ninguna migración nueva,
ninguna tabla nueva, ninguna política tocada — R-26 y R-27 son ambas "sin migración ni RPC nueva"
por su propio mensaje de commit, confirmado con el diff real, no solo leído en el texto.

**Cierre de los hallazgos #23 y #24 (ambos `ABIERTO` desde la pasada anterior):** verificados
directamente contra el código, no solo contra el mensaje de commit. **#23** (controles "Justificar"/
"Marcar salida" pintados siempre, aunque `011`/`012` sigan sin aplicar): P-31 añade
`justificarAusenciaDisponible()`/`marcarSalidaDisponible()` (`src/datos/asistencia.ts:126-150`) y
las inyecta como dependencia OPCIONAL en `pantallaRegistrosSlot.ts:644-654,785-788` y
`pantallaPasarLista.ts:557-566` — el control ahora se OCULTA mientras la señal sea `false`, no solo
falla al pulsarlo — wireadas a las funciones reales en los tres puntos de montaje de
`aplicacion.ts` (líneas 391-392, 769-770, 868). Cinco tests nuevos fijan el caso oculto con
`() => false`. **RESUELTO**, con evidencia añadida en el registro de hallazgos. **#24** (matriz
rol × tabla × operación de `DECISIONES_TECNICAS.md` sin fila para `pausa_alumno`): P-32, mismo
commit, añade la fila que faltaba (`DECISIONES_TECNICAS.md:383`), con el mismo formato que las
demás y sin ningún error de contenido (RPC correctas, alcance del `teacher` bien descrito).
**RESUELTO**. Ambos cierres verificados por lectura directa del código y de la documentación, no
por el mensaje de commit — ver el registro de hallazgos para el detalle línea a línea.

**Puntos de control permanentes — sin ningún cambio, releídos contra el SQL en disco (un
subagente de investigación despachado por separado llegó a la misma conclusión, punto por punto,
sin ver este trabajo):** con `db/` intacto desde la pasada anterior, los doce puntos más sensibles
siguen exactamente igual — escritura de `asistencia` solo por `registrar_asistencia`/
`actualizar_asistencia` (sin GRANT de escritura directa para ningún rol); trigger `BEFORE UPDATE`
(`001_esquema_inicial.sql:289-311`) abortando cambios a `registrado_en`/`profesor_id`/`peticion_id`;
trigger `AFTER UPDATE` copiando a `asistencia_historial`, sin GRANT de escritura para nadie ni
siquiera `service_role`; pertenencia comprobada en servidor dentro de `actualizar_asistencia`
(`008_rpc_actualizar_asistencia.sql:116-130`); ninguna política para `student` fuera de
`perfil_leer_propio`; ningún `TRUNCATE`/`REFERENCES`/`TRIGGER` concedido a `anon`/`authenticated`
en ninguna de las 19 migraciones (000-018), con las guardas activas de
`herramientas/migraciones/guardas.ts` y `verificarPrivilegios.ts` intactas; bucket `avatares`
privado, sin acceso de `anon`/`student`, escritura solo `administrator`, lectura de `teacher`
acotada a `activo`; `persona_referencia` solo `administrator`; el `GRANT` de columna de `alumno` a
`authenticated` sigue sin `email_alumno`/`telefono_alumno` (`003_politicas_rls.sql:107-109`), esas
dos solo via `alumno_ficha`, filtrada `where es_administrator()` en la propia vista; RLS habilitada
con políticas explícitas en las 13 tablas de `public`, incluidas `pausa_alumno` (017) y
`baja_profesor` (018); sin ningún secreto real en el repo (`.env.ejemplo` con claves vacías,
`herramientas/verificarFugaSecretos.test.ts` solo referencia nombres/patrones, nunca valores) y el
runner sigue exigiendo `--entorno=prod` **y** `PERMITIR_PROD=1` para poder apuntar a producción.
`package.json` sigue con `dependencies` vacío (todo en `devDependencies`: `jsdom`, `typescript`,
`eslint`) y sin ningún `fetch` de aplicación fuera de `src/datos/`.

**R-26 (recordatorio local) y R-27 (alta de sesión de grupo), leídas contra su propia spec y
contra §0.2:** ninguna añade dato personal nuevo del alumno ni amplía el alcance de `student`. R-26
guarda el interruptor en `localStorage` (ajuste de dispositivo, no dato de sesión) y su propio
código documenta y cumple la restricción de no mostrar el nombre del alumno en la notificación del
sistema operativo (`dominio/recordatorioSesion.ts:59-61`, requisito 7 de su spec) — verificado
también en el punto de disparo real (`pantallaMiHorario.ts:232-239`): el cuerpo de la notificación
lleva solo día/hora/asignatura. El nuevo `notificationclick` de `sw.js` navega a una de dos rutas
fijas (`#/pasar-lista` o `#/horario`) decidido por comparación de épocas, sin ninguna entrada
externa que pudiera inyectar una URL distinta. R-27 reutiliza el buscador ya auditado de T-20 y el
mismo patrón de alta de T-15/R-25 (una llamada a `crearSlot` por alumno, con el mismo tratamiento
de solape parcial); no toca ninguna tabla ni columna nueva. Ambas quedan `COMPLETADA` en
`SEGUIMIENTO.md` §1, consistente con la Oleada v12 cerrada.

**Coherencia documental — sin desviación nueva sin registrar:** el vigésimo noveno ciclo del PM
abrió la pregunta #18 de §6 (umbral/ventana de R-28) al mismo tiempo que la propia spec de R-28,
mismo patrón ya establecido (T-11/#7, T-12/#8, T-13/#9-10); §7 no necesita ninguna fila nueva —
ni R-26 ni R-27 se desviaron de su criterio de aceptación, y P-31/P-32 son correcciones de bugs ya
registrados como hallazgos de este documento, no desviaciones de la hoja de ruta. Suite completa
verde: `npm test` 1816/1816, `npm run typecheck` y `npm run lint` sin errores (dependencias
instaladas con `npm install` sobre un árbol de trabajo limpio, sin tocar ningún fichero de
código).

**Conclusión:** dos hallazgos cerrados en el registro (#23, #24), ninguno nuevo. El hallazgo #8
(RGPD artículo 9, severidad alta) sigue `ABIERTO` por decimosexto ciclo consecutivo, exactamente en
el mismo estado que la pasada anterior: sigue sin haber ninguna vía para que cambie sin que el
dueño responda la pregunta #16 de §6. El proyecto mantiene su disciplina de cerrar cada hallazgo del
auditor en el ciclo de programador inmediatamente siguiente a su apertura, sin excepción hasta la
fecha.

### Auditoría 2026-09-22

**Alcance real de esta pasada — seis commits desde la anterior (`5a34ae0`, 2026-09-21):**
`161c4b9` (P-30, corrige el hallazgo #22), `0655e02` (R-25, vista de horario del centro), tres
rutinas programadas de programador sin trabajo accionable (`9026f69`, `2af293c`, `6e50fca`) y
`f58ea9c` (vigésimo octavo ciclo del PM, abre la Oleada v12 con R-26/R-27, sin código). `git
checkout develop && git pull origin develop`: fast-forward limpio hasta `f58ea9c`. `git diff
5a34ae0..HEAD --stat` confirma que **no hay ningún cambio bajo `db/`**: ninguna migración nueva,
ninguna tabla nueva, ninguna política tocada — así que los puntos de control de seguridad más
sensibles (RLS, privilegios de tabla, triggers de `asistencia`, bucket de avatares) no tenían nada
nuevo que pudiera haberlos roto desde la pasada anterior. Aun así se releyeron directamente contra
el SQL en disco, no solo por herencia de la pasada anterior: el trigger `BEFORE UPDATE`
(`asistencia_proteger_inmutables`, `db/001_esquema_inicial.sql:289-311`) sigue abortando cambios en
`registrado_en`/`profesor_id`/`peticion_id` y fija `actualizado_en`/`actualizado_por` él mismo; el
trigger `AFTER UPDATE` (`asistencia_copiar_a_historial:372-397`) sigue copiando a
`asistencia_historial`, tabla que sigue sin ningún `GRANT` de escritura para nadie, ni siquiera
`service_role` (`revoke all ... from anon, authenticated, service_role`, línea 366); ninguna
política nueva ni antigua menciona a `student` fuera de `perfil_leer_propio`; el barrido de
`TRUNCATE` de `db/pruebas_rls.sql` (sección 8, líneas 1201-1205) sigue cubriendo las 13 tablas
reales de `public` (incluida `baja_profesor`, que cerró el hallazgo #21); las cuatro políticas de
`storage.objects` sobre el bucket `avatares` (`db/003_politicas_rls.sql:247-278`) siguen siendo
`administrator` CRUD + `teacher` SELECT acotado a `a.activo`, sin ninguna para `anon` ni `student`.
`package.json` sigue sin clave `dependencies` (solo `devDependencies` de herramientas de build/lint)
y `grep -rln "fetch(" src/` fuera de `src/datos/` no devuelve nada. Ejecutado en limpio: `npm ci`,
`npm run typecheck` y `npm run lint` sin errores, `npm test` — **1770/1770** tests en verde.

**Hallazgo #22 (severidad alta, `actualizar_asistencia` rota contra la firma real de `dev`) →
RESUELTO, verificado en profundidad.** P-30 (`161c4b9`) hizo exactamente lo que el propio hallazgo
proponía como opción (c): `actualizarAsistencia` (`src/datos/asistencia.ts:144-166`) vuelve a
construir solo los 8 parámetros de `008_rpc_actualizar_asistencia.sql`, y pedir
`justificar`/`marcarSalida`/`ocurridoEnSalida` (R-02/R-03, todavía sin aplicar) lanza
`AccionNoDisponibleTodavia` (clase nueva, fuera de la taxonomía cerrada de `ErrorDeDominioSupabase`
por diseño — no es una traducción de respuesta HTTP, se lanza en cliente antes de la red) con
mensaje propio en `mensajesAbuso.ts`. Verificación propia, no solo lectura del código: el test
nuevo `actualizarAsistencia: el cuerpo nunca lleva las claves de R-02/R-03...` fija la lista exacta
y ordenada de las ocho claves del cuerpo con `assert.deepEqual` sobre las claves ordenadas — la
forma correcta de proteger justo el tipo de fallo que causó el hallazgo (un nombre de parámetro de
más rompe la resolución completa de la llamada en PostgREST); tres tests más confirman que
`justificar`/`marcarSalida`/`ocurridoEnSalida` lanzan sin tocar la red (`llamadas === 0`) y que
`marcarSalidaAsistencia` bloqueado no consume el limitador de tasa de cliente. T-21 ("Registros") y
R-24 ("Anular" en pasar lista) vuelven a funcionar de verdad en `dev`. Cierro el hallazgo.

**Pero la corrección deja una segunda mitad sin recorrer — hallazgo nuevo #23 (media):** el
guardián de P-30 corta la llamada de red, no la propia oferta en pantalla. `pantallaRegistrosSlot.ts`
sigue pintando el bloque "Justificar" siempre que el registro esté `ausente`
(`puedeJustificarAusencia`, sin comprobar si `011` está aplicada) y `pantallaPasarLista.ts` sigue
ofreciendo "Marcar salida" siempre que la tarjeta esté `registrado` (`puedeMarcarSalida`, mismo
patrón) — ninguna de las dos pantallas consulta `accionPendienteDeMigracion` ni nada equivalente
antes de pintar el control. Antes de hoy, pulsar cualquiera de los dos rompía la llamada entera
(hallazgo #22); desde hoy, el error es limpio ("todavía no disponible"), una mejora real, pero
sigue siendo un control que un profesor o administrator puede rellenar y enviar sabiendo que
fallará siempre — y el de pasar lista es visible en el flujo diario de cualquier profesor con un
alumno ya marcado presente, no en una pantalla secundaria. Sin ningún test que cubra qué ve el
usuario al pulsar estos dos botones hoy (verificado con `grep`). Con el hallazgo #8 sin resolver
desde hace 15 ciclos, este estado puede persistir indefinidamente si nadie oculta los dos
controles. No es un hallazgo de seguridad ni de integridad de datos — el manejo de error de ambas
pantallas es correcto, sin estado colgado —, así que severidad media, no alta.

**Hallazgo #8 (severidad alta, dato de salud del artículo 9 del RGPD en R-02) — confirmado sin
cambio, decimoquinto ciclo consecutivo.** `git diff 5a34ae0..HEAD --stat -- db/ legal/` no devuelve
nada: ninguno de los seis commits de esta ventana toca `db/011_justificacion_ausencia.sql` ni
ningún documento de `legal/`. La pregunta #16 de §6 de `SEGUIMIENTO.md` sigue con la columna
"Respuesta" vacía (lectura directa de la fila completa), la fila 14 de §3 conserva la misma
condición de bloqueo, y `db/APLICADAS.md` sigue sin `011`/`012` (`008` sigue siendo la última
aplicada de verdad en `dev`). Sigue **ABIERTO**, bloqueado exclusivamente por una decisión
pendiente del dueño.

**R-25 (vista de horario del centro, commit `0655e02`) revisada contra los puntos de control —
sin hallazgos.** Pantalla exclusiva de `administrator` (`puedeGestionarHorarios`, verificado en
`pantallaHorarioCentro.ts:107`), sin RPC ni migración nueva (reutiliza `modificarSlot`/`cesarSlot`
de T-15 alumno a alumno), sin ningún dato personal nuevo, y "solo nombres, nunca fotografía"
confirmado por `grep -n "avatar" src/ui/pantallaHorarioCentro.ts` — únicas coincidencias son el
comentario que cita la regla, no ningún uso real. La agrupación por sesión reutiliza
`slotsDeLaMismaSesion` en vez de reimplementar el criterio (una sola fuente de verdad, ver la fila
de `DECISIONES_TECNICAS.md` del 2026-09-21). R-26/R-27 (Oleada v12, spec únicamente, sin código
todavía) tampoco añaden dato personal, RPC ni migración según su propio texto — nada que auditar
todavía en código.

**Hallazgo nuevo #24 (gobernanza documental, baja):** la "Matriz rol × tabla × operación" de
`DECISIONES_TECNICAS.md` —la propia referencia que este documento usa para contrastar RLS— no tiene
fila para `pausa_alumno` (R-21, completada el 2026-09-15/16), aunque sí la tiene `baja_profesor`
(R-22, la tarea siguiente). Las políticas reales de `pausa_alumno` son correctas y están probadas
(`db/017_pausa_alumno.sql:153-160`, sin política para `student`, cubierta en los barridos de
`student`/`TRUNCATE` de `pruebas_rls.sql`): es solo la tabla de referencia la que quedó
incompleta, mismo patrón que los hallazgos #4/#5/#6/#9/#11, todos ya `RESUELTO`.

**Conclusión de esta pasada:** el proyecto sigue sin ninguna violación de los puntos de control de
seguridad permanentes — RLS completa, aislamiento de `student`, privilegios de tabla, inmutabilidad
y rastro de `asistencia`, bucket de avatares, superficie de columnas, alcance de datos personales —,
y el único hallazgo de seguridad/RGPD pendiente (#8) sigue correctamente bloqueado a nivel de
código y de documento en espera de una decisión que solo el dueño puede tomar. La sesión de
programador de hoy cerró con solidez un hallazgo de severidad alta (#22) que llevaba tres pasadas
abierto, tomándolo antes de la cola normal como exige el protocolo, con tests que prueban de verdad
el defecto que lo causó — exactamente el estándar que este documento pide. Los dos hallazgos nuevos
(#23 media, #24 baja) son de UX y de higiene documental, no de seguridad ni de integridad de datos.

---

### Auditoría 2026-09-21

**Alcance real de esta pasada — un único commit desde la anterior (`e57e4af`, 2026-09-20):**
`c1c1833` (vigésimo séptimo ciclo del PM, "sin R-XX nueva, tercer ciclo consecutivo" — solo
`roadmap/HISTORIAL_SESIONES.md`, `roadmap/ROADMAP_PRODUCTO.md` y `roadmap/SEGUIMIENTO.md`;
confirmado con `git diff e57e4af..HEAD --stat`, cero entradas bajo `db/`, `src/`, `legal/` ni
`roadmap/HOJA_DE_RUTA.md`/`roadmap/DECISIONES_TECNICAS.md`). `git checkout develop && git pull
origin develop`: fast-forward limpio hasta `c1c1833`. Con cero cambios de código y cero cambios de
SQL desde la pasada anterior, este auditor no se limitó a heredar la conclusión: releyó de forma
independiente, con `grep`/lectura directa (no de memoria de pasadas previas), los puntos más
críticos del propio SQL aplicado — `003_politicas_rls.sql`/`001_esquema_inicial.sql` para las
políticas de `asistencia` (solo `SELECT`, `revoke all` previo, sin `INSERT`/`UPDATE`/`DELETE`), los
dos triggers de `asistencia` (`asistencia_before_update` sigue abortando cambios en
`registrado_en`/`profesor_id`/`peticion_id` y fijando `actualizado_en`/`actualizado_por` él mismo;
`asistencia_after_update` sigue copiando en `asistencia_historial`, con esa tabla en `revoke all` a
los tres roles y una única política de lectura para `administrator`), la comprobación de
pertenencia/ventana de 7 días dentro de `actualizar_asistencia` (`008_rpc_actualizar_asistencia.sql:113-128`,
en el servidor, no en el cliente), las políticas del bucket `avatares` (lectura de `teacher`
todavía acotada a `alumno.activo` vía `storage.foldername(name)`, sin ninguna política de
`student` ni de `anon`), el `grant select` de columna de `alumno` (limitado a
`id, nombre, primer_apellido, segundo_apellido, avatar_ruta, activo`; `email_alumno`/
`telefono_alumno` solo alcanzables por la vista `alumno_ficha`, exclusiva de `administrator`), que
la corrección del hallazgo #18 sigue en pie (`contarAlumnosActivosDeCentro` sigue consultando
`alumno_ficha`, nunca la tabla base, para `centro_referencia_id`), que la de #21 también
(`baja_profesor` sigue presente en el barrido de `student`, sección 6, y en el de `TRUNCATE`,
sección 8, de `db/pruebas_rls.sql`), que las catorce tablas de `public` (contadas de nuevo:
`esquema_migracion`, `perfil`, `centro_estudios`, `alumno`, `persona_referencia`, `slot_horario`,
`asistencia`, `asistencia_historial`, `evento_error`, `limite_tasa`, `excepcion_slot`,
`cierre_centro`, `pausa_alumno`, `baja_profesor`) tienen todas `enable row level security`, que
ningún `grant` de todo `db/` concede `TRUNCATE` a `anon` ni a `authenticated`, y que `package.json`
sigue sin clave `dependencies` (siete `devDependencies`, ningún SDK de Supabase ni framework). Los
catorce puntos de control permanentes de la tabla de este documento siguen intactos porque nadie ha
tocado el SQL ni el cliente que los sostiene. Se instalaron dependencias en limpio (`npm ci`, sin
avisos) y se ejecutaron de verdad `npm run typecheck`, `npm run lint` y `npm test` — no solo por
lectura del commit anterior —: los tres en verde, **1748/1748** tests, el mismo número exacto que
la pasada del 2026-09-20 (coherente con que ningún fichero de `src/` cambió entre medias).

**Hallazgos `ABIERTO` reevaluados, ninguno con novedad de fondo:** **#8** (RGPD/dato de salud en
R-02, alta) sigue esperando la respuesta del dueño a la pregunta #16 de §6 de `SEGUIMIENTO.md`
(columna "Respuesta" todavía vacía, verificado por lectura directa), decimocuarto ciclo consecutivo
sin novedad; `011_justificacion_ausencia.sql` sigue sin aplicar. **#22** (rotura de
`actualizar_asistencia` contra la firma real de 8 parámetros de `dev`, mientras el cliente envía los
13 de la firma ampliada por `011`/`012`, todavía sin aplicar) sigue exactamente igual:
`src/datos/asistencia.ts:126-140` sin ninguna rama condicional, y sigue sin correr ninguna sesión de
programador que pudiera haberlo tomado como `P-XX` urgente — el único commit del período es, de
nuevo, del PM. Ninguno de los dos hallazgos cambia de estado en esta pasada; ninguno nuevo se abre
(no hay superficie nueva de código, esquema ni documento que auditar).

**Observación de proceso, no un hallazgo:** este es ya el tercer ciclo de PM consecutivo (25º, 26º
y 27º) sin que ninguna sesión de programador haya corrido — `git log 184ad55..c1c1833` lo confirma:
los únicos commits del período son dos ciclos de PM y dos pasadas de este auditor, ninguno de
programador. La cola (R-25, Oleada v11) lleva `PENDIENTE` desde el 2026-09-18, y los dos hallazgos
`ABIERTO` de severidad alta (#8, bloqueado en el dueño; #22, a la espera de una sesión de
programador) siguen sin moverse por la misma razón: no ha habido ejecución, no que la ejecución
haya ido mal. El propio PM sigue señalizando ambos hallazgos con precisión en cada ciclo y no
fabrica trabajo para rellenar el vacío, que es el comportamiento correcto — pero la ejecución real
del proyecto está de facto parada en dos frentes independientes (una decisión del dueño; una
sesión de programador que aún no ha llegado), y conviene que quede dicho aquí con esa claridad para
quien lea este documento sin haber seguido las últimas pasadas.

**Conclusión de esta pasada:** sin hallazgos nuevos. Tabla de hallazgos sin cambio de recuento
(22 hallazgos, 20 `RESUELTO`, 2 `ABIERTO`: #8 y #22, ambos reevaluados y confirmados sin cambio de
fondo arriba). Ningún punto de control permanente en riesgo. `git checkout develop`/`pull` limpios
en todo momento; ningún cambio tocó `master`. Mergeado a `develop` al cierre de esta pasada.

---

### Auditoría 2026-09-20

**Alcance real de esta pasada — un único commit desde la anterior (`f998218`, 2026-09-19):**
`53c8a60` (vigésimo sexto ciclo del PM, "sin R-XX nueva" — solo `roadmap/HISTORIAL_SESIONES.md`,
`roadmap/ROADMAP_PRODUCTO.md` y `roadmap/SEGUIMIENTO.md`; confirmado con `git diff --stat
f998218..HEAD`, cero entradas bajo `db/`, `src/`, `legal/` ni `roadmap/HOJA_DE_RUTA.md`). `git
checkout develop && git pull origin develop`: fast-forward limpio hasta `53c8a60`. Con cero
cambios de código y cero cambios de SQL desde la pasada anterior —que ya hizo la verificación
completa de infraestructura (RLS, triggers de inmutabilidad y de rastro, privilegios de tabla,
bucket de avatares, superficie de columnas, rol `student` cerrado) contrastando directamente el
SQL aplicado—, ningún punto de control permanente que dependa de `db/` tiene nada nuevo que
reevaluar: siguen intactos porque nadie los ha tocado. Se instalaron dependencias en limpio
(`npm ci`, 130 paquetes, 0 vulnerabilidades) y se ejecutaron de verdad `npm run typecheck`,
`npm run lint` y `npm test` — no solo por lectura del commit anterior —: los tres en verde,
**1748/1748** tests, idéntico al recuento de la pasada anterior (coherente con que no cambió
ningún fichero de test). Barrido de secretos repetido sobre el repositorio completo (`service_role`,
`SUPABASE_ACCESS_TOKEN`, `sk-`, `eyJhbGci`, contraseñas en claro): cero coincidencias reales, solo
los placeholders esperados de `.env.ejemplo`/`DEVELOPERS.md` y el propio test de redacción de
`src/nucleo/registro.test.ts`. `dependencies` de `package.json` sigue vacío, sin SDK de Supabase
ni framework nuevo.

**Registro de hallazgos:** los dos únicos `ABIERTO` de la tabla —**#8** (RGPD artículo 9, `011`
bloqueada, esperando la pregunta #16 de §6 de `SEGUIMIENTO.md`) y **#22** (rotura de
`actualizar_asistencia` contra la firma real de `dev`, abierto ayer)— siguen exactamente igual,
verificado con lectura directa de las filas 16 de §6, 14 de §3, `db/APLICADAS.md` y
`src/datos/asistencia.ts:126-140`; detalle completo de la reevaluación en sus propias filas de
arriba. Sobre **#22** en concreto: el único commit transcurrido es la propia sesión de PM que lo
registró y correctamente decidió no generar ninguna R-XX, dejándolo anotado para que una sesión de
programador lo atienda como P-XX urgente (§0 de `HOJA_DE_RUTA.md`, "antes de la cola") — todavía no
ha corrido ninguna sesión de programador desde entonces, así que no hay, de momento, ninguna
desviación de proceso que señalar; sí la habría si la siguiente sesión de programador retomara la
cola normal sin pasar antes por este hallazgo. Ninguna fila nueva en §7 de `SEGUIMIENTO.md`
(revisado, sin cambio desde la pasada anterior).

**Conclusión de esta pasada:** pasada de confirmación, no de descubrimiento — el único movimiento
del proyecto en 24 horas fue un ciclo de PM que no tocó código ni SQL y que gestionó correctamente
el hallazgo #22 heredado de ayer (lo escaló al programador en vez de intentar resolverlo o
enmascararlo con una R-XX). La infraestructura de datos y los puntos de control permanentes siguen
verificados porque nadie tocó `db/`. El único riesgo activo del proyecto sigue siendo el mismo de
ayer: **"Registros" (T-21) y "Anular" en pasar lista (R-24) siguen sin funcionar de verdad contra
la base de datos real de `dev`**, y seguirá así hasta que una sesión de programador aplique una de
las tres opciones ya planteadas en la fila #22, o hasta que el dueño responda la pregunta #16 y se
apliquen `011`/`012`. Recomendación sin cambio respecto a ayer: tratar #22 con la urgencia de una
función ya en producción rota en la próxima sesión de programador que arranque.

### Auditoría 2026-09-19

**Alcance real de esta pasada — seis commits desde la anterior (`55a5e8c`, 2026-09-18):**
`004d778` (R-24, "anular un toque equivocado sin salir de pasar lista" — código y tests
completos, sin migración ni RPC nueva), cuatro rutinas de programador (`01c3d44`/`96a9a3c`/
`da33224`/`bce161d`) sin trabajo accionable tras R-24, y `184ad55` (vigésimo quinto ciclo del
PM, abre R-25/Oleada v11 — solo roadmap, cero código). `git checkout develop && git pull
origin develop`: fast-forward limpio hasta `184ad55`. `git diff --stat d61898a..184ad55 --
':(exclude)auditoriacontinua.md'` acota el cambio de código a `src/datos/asistencia.ts`/
`.test.ts`, `src/ui/aplicacion.ts` y `src/ui/pantallaPasarLista.ts`/`.test.ts`; **ningún fichero
de `db/` cambió** desde la pasada anterior (confirmado por el mismo `diff --stat`, sin ninguna
entrada bajo `db/`), así que los puntos de control permanentes que dependen de SQL (escritura
solo por RPC, inmutabilidad de `registrado_en`, rastro de cambios, rol `student` cerrado,
privilegios de tabla, bucket de avatares, superficie de columnas, RLS completa) no tienen nada
nuevo que reevaluar respecto a la pasada del 2026-09-18, que ya los confirmó uno a uno; `roadmap/
HOJA_DE_RUTA.md` y todo `legal/` siguen sin tocar, confirmado por `git diff` vacío. Se instalaron
dependencias (`npm ci`, entorno limpio) y se ejecutaron `npm run typecheck`, `npm run lint` y
`npm test` de verdad — no solo por lectura —: los tres en verde, **1748/1748** tests, coincide
exactamente con lo declarado en el commit de R-24.

**Registro de hallazgos:** ninguno de la tabla estaba `ABIERTO` salvo el **#8** (RGPD, `011`
bloqueada), que sigue exactamente igual — `db/APLICADAS.md` confirma que `011_justificacion_
ausencia.sql` sigue en la sección "Pendiente de aplicar" y que su nota de cabecera sigue
condicionando su aplicación a resolver la pregunta #16 de §6 de `SEGUIMIENTO.md`; sin novedad del
dueño al respecto. Se abre **#22** (alta, `ABIERTO`): revisando qué toca exactamente el commit de
R-24 (`src/datos/asistencia.ts#anularAsistencia`, atajo sobre `actualizarAsistencia`), se leyó el
payload completo que `actualizarAsistencia` envía a la RPC — no solo los campos que R-24 necesita
— y resultó incluir, sin ninguna condición, los cinco parámetros que `011`/`012` añadirán a
`actualizar_asistencia` (`p_justificar`, `p_motivo_justificacion`, `p_nota_justificacion`,
`p_marcar_salida`, `p_ocurrido_en_salida`). Comparado ese payload contra la firma realmente
desplegada en `dev` (`db/008_rpc_actualizar_asistencia.sql`, última fila de `db/APLICADAS.md`,
8 parámetros, sin ninguno de esos cinco), la conclusión es que **ninguna llamada actual a
`actualizar_asistencia` puede tener éxito contra la base de datos real** — PostgREST no ignora un
nombre de parámetro que la función no declara, hace fallar la resolución de la llamada entera.
Esto no afecta solo a R-02/R-03/R-24 (ya `BLOQUEADA`/dependientes de la migración, de forma
correcta y señalizada), sino a **T-21** ("Registros"), tarea `COMPLETADA` desde 2026-09-02 y
verificada en su día en ejecución real (`db/APLICADAS.md`, fila 008: "89 comprobaciones, 0
fallidas") — hoy rota en `dev` como efecto colateral silencioso de que R-02/R-03 ampliaron la
MISMA función de cliente compartida por delante de su propia migración. Detalle completo,
impacto y tres opciones de arreglo (ninguna de las tres decidible por el PM en solitario, por
tratarse de una RPC ya `COMPLETADA` con uso diario documentado) en la fila #22 de arriba. Es la
clase de hallazgo que ninguna sesión de programador iba a encontrar sola: la suite entera corre
contra un doble de `ClientePostgrest` que nunca valida un nombre de parámetro contra una firma de
PostgreSQL real, y nadie ha vuelto a ejecutar `npm run probar-rls` (la única herramienta que sí
golpea la base real) desde antes de que R-02 tocara esta función — su ejecución depende del
dueño, y las migraciones que la motivarían siguen bloqueadas. `npm test` en verde (1748/1748) no
lo detecta ni lo va a detectar mientras la batería siga contra dobles: es exactamente el patrón
que este auditor tiene instrucción de vigilar de cerca en la calidad real de los tests de este
proyecto.

**Revisión de la parte no-SQL del commit de R-24** (independiente del hallazgo #22, sobre el
propio diseño de la funcionalidad nueva): el control "Anular" se oculta por completo fuera de la
ventana de edición (`puedeEditarAsistencia`, primer consumidor real desde T-03) y el fallo de red
se trata sin cola offline propia, con razonamiento explícito y correcto en `DECISIONES_TECNICAS.md`
(la fila que se anula ya existe en el servidor; un fallo de red dice "no sé si se aplicó", nunca
"se perdió el dato", a diferencia de un alta nueva). La propia entrada de `DECISIONES_TECNICAS.md`
del 2026-09-18 reconoce sin rodeos una divergencia real y deliberada: "Registros" (T-21) NUNCA
oculta preventivamente su propio botón de anular fuera de ventana, y R-24 no lo corrige por estar
fuera de su alcance — queda anotada para una sesión futura de unificación, no es un hallazgo nuevo
del auditor porque ya está detectada, documentada y decidida por quien tenía que decidirla.

**Conclusión de esta pasada:** la infraestructura de datos (RLS, triggers de inmutabilidad y de
rastro, privilegios, bucket de avatares) sigue intacta porque nadie tocó `db/` desde la última
revisión completa. El hallazgo real de esta pasada no está en el código nuevo de R-24 en sí —bien
diseñado, bien documentado, con 14 tests que sí prueban lo que dicen probar— sino en una
consecuencia no evaluada de sesiones anteriores (R-02/R-03) que R-24 solo destapa por añadir un
tercer punto de llamada a una función ya rota: **una tarea `COMPLETADA` y en uso (T-21) ha dejado
de funcionar de verdad en `dev`**, sin que ninguna auditoría previa lo detectara porque el cambio
que lo rompió no tocó ningún fichero de `db/` ni cambió el comportamiento visible en los dobles de
prueba. Recomendación al PM/dueño: tratar el hallazgo #22 con la urgencia de una función ya en
producción rota, no con el ritmo de una migración pendiente más.

### Auditoría 2026-09-18

**Alcance real de esta pasada — seis commits desde la anterior (`229f98a`, 2026-09-17):**
`65d0a34` (P-29 urgente, cierra el hallazgo #21, + R-23 "marcar el resto como presente" —
código y tests completos, sin migración ni RPC nueva), cuatro rutinas de programador
(`930553a`/`a9f503d`/`1617a2b`/`325c98c`) sin trabajo accionable tras R-23, y `d61898a`
(vigésimo cuarto ciclo del PM, abre R-24/Oleada v10 — solo roadmap, cero código). `git
checkout develop && git pull origin develop`: fast-forward limpio hasta `d61898a`. `git diff
--stat 229f98a..HEAD` acota el cambio de código a `db/pruebas_rls.sql` (los tres arrays de
barrido), `src/ui/pantallaPasarLista.ts`/`.test.ts` y `src/ui/pantallaRegistrosSlot.ts`/
`.test.ts`; `roadmap/HOJA_DE_RUTA.md` y todo `legal/` sin tocar, confirmado por `git diff`
vacío.

**Registro de hallazgos:** se cierra **#21** (`RESUELTO`, alta): P-29 añadió `'baja_profesor'`
a los tres arrays genéricos de `db/pruebas_rls.sql` exactamente como proponía la pasada
anterior — verificado con `grep -n "baja_profesor" db/pruebas_rls.sql`, presente ahora en las
tres listas (líneas 743, 1204, 1858) además de su propia sección 8o. **#8** sigue **ABIERTO**
(dato de salud del artículo 9 del RGPD en R-02), decimocuarto ciclo consecutivo sin novedad de
fondo: ninguno de los seis commits nuevos toca `db/011_justificacion_ausencia.sql` ni ningún
documento de `legal/`, y la pregunta #16 de §6 de `SEGUIMIENTO.md` sigue con la columna
"Respuesta" vacía (lectura directa de la fila completa, línea 3062). Ningún hallazgo nuevo.

**Auditoría de R-23 ("marcar el resto como presente") contra los puntos de control
permanentes,** leído el diff completo de `65d0a34` sobre `pantallaPasarLista.ts` y
`pantallaRegistrosSlot.ts`:
- **Sin RPC ni migración nueva (tal como declara el commit):** confirmado leyendo el diff
  completo — en `pantallaPasarLista.ts` el bloque reutiliza `manejarToque` tal cual, una vez
  por alumno pendiente (mismo camino que un toque individual, incluida la cola offline de R-07
  y el límite de tasa de T-06 si aplican); en `pantallaRegistrosSlot.ts` reutiliza
  `deps.registrarOlvidado` (misma RPC que ya usa el registro individual "olvidado") sin pasar
  `ocurridoEn`, así que cada llamada queda con la hora real del servidor en el instante de su
  propia ejecución — nunca una hora de bloque simulada ni fijada por el cliente. Coherente con
  el punto de control "hora del servidor y retroactivos": ningún camino nuevo permite al
  cliente fijar `registrado_en`/`ocurrido_en`.
- **Simetría real con R-17, no solo declarada:** `abrirMarcarPresenteEnBloque` congela la lista
  de pendientes al abrir la confirmación exactamente igual que `abrirCierreEnBloque` (R-17), y
  un fallo de un alumno no detiene ni deshace los demás en ninguna de las dos pantallas
  (`manejarToque` ya deja su propia card en `'error'`/`'pendiente_offline'` sin lanzar; en
  `pantallaRegistrosSlot.ts` los fallidos quedan en `cierreCandidatos` para reintentar sueltos).
- **Convivencia sin interferir (requisito 7 de la propia spec):** cada control vive en su
  propio bloque marcado con `data-bloque="ausente"`/`data-bloque="presente"`, y los tests
  nuevos (`nombresEnConfirmacionCierre`/`nombresEnConfirmacionPresente`) acotan la lectura a su
  propio bloque para no confundir uno con otro — verificado leyendo el test, no solo el
  comentario.
- **Ningún dato personal nuevo, ninguna tabla nueva, ninguna columna nueva:** el cambio es
  puramente de interfaz sobre datos y RPC ya auditados.

**Calidad real de los tests (T-03):** los 15 tests nuevos de `65d0a34` (leídos íntegros en
`pantallaPasarLista.test.ts` y `pantallaRegistrosSlot.test.ts`) cubren casos reales con
contraejemplo — el control no se ofrece sin pendientes ni fuera de la clase en curso, la
confirmación lista nominalmente solo a los pendientes vigentes en el instante de abrir (un
alumno que ya tiene entrada queda excluido desde el principio), confirmar llama una vez por
pendiente y dos casos de fallo parcial (uno de dos falla, el otro completa; y el reintento
sobre los restantes) — no son aserciones triviales de "no lanza". Confirmado en ejecución por
este auditor, no solo por el commit: `npm ci` (limpio), `npm run typecheck` (limpio), `npm run
lint` (limpio) y `npm test` (**1734/1734**, coincide exactamente con lo que registra
`SEGUIMIENTO.md`).

**Otros puntos de control verificados sin novedad:** `package.json` sigue con `dependencies`
vacío (7 `devDependencies`, sin framework ni SDK de Supabase); `grep -rniE` de
`service_role`/`SUPABASE_ACCESS_TOKEN`/JWT/contraseñas en claro sobre el repositorio completo:
cero coincidencias reales, solo el nombre del rol en comentarios/documentación legítimos;
`grep -rniE "grant.*truncate|truncate.*to (anon|authenticated)"` sobre todo `db/*.sql`: cero
coincidencias, ningún `TRUNCATE` concedido a `anon`/`authenticated` en ninguna migración,
incluida `018_baja_profesor.sql`; sin ninguna política nueva para un rol `student` en ningún
fichero de `db/`.

**Coherencia documental:** `roadmap/SEGUIMIENTO.md` §7 no gana ninguna fila nueva — no se ha
encontrado ninguna desviación de R-23 respecto a su spec (reutiliza los mismos mecanismos de
R-17 sin ampliar alcance ni tocar ningún dato sensible); ausencia correcta, no un hueco. R-24
(Oleada v10, abierta por el vigésimo cuarto ciclo del PM) es solo una spec nueva sin
implementación todavía — nada que auditar en código hasta que se escriba.

**Conclusión:** proyecto sano. Se cierra el único hallazgo activo aparte de #8 (#21, verificado
resuelto por P-29), no se abre ningún hallazgo nuevo, y el hallazgo #8 permanece exactamente
donde estaba, esperando al dueño en la pregunta #16 de §6 — ninguna vía para que cambie sin esa
decisión.
...
**Alcance real de esta pasada — seis commits desde la anterior (`2963bfe`, 2026-09-15):** `454da97`
(R-21, pausa programada de un alumno — código y tests completos, migración `017` escrita y
empujada, todavía sin aplicar), `23854dc`/`6778da3`/`fc9cf4c`/`607c4ad` (cuatro rutinas de
programador sin trabajo accionable tras R-21) y `efdbfd4` (vigésimo segundo ciclo del PM, abre la
Oleada v8 con R-22 — solo roadmap, cero código). `git checkout develop && git pull origin develop`:
fast-forward limpio hasta `efdbfd4`. `git diff --stat 2963bfe..HEAD -- db/ legal/ roadmap/` acota el
cambio real a `db/017_pausa_alumno.sql` (nueva), `db/pruebas_rls.sql` (sección 8n), `db/APLICADAS.md`,
`db/MODELO.md`, `roadmap/DECISIONES_TECNICAS.md` y `roadmap/SEGUIMIENTO.md`/`ROADMAP_PRODUCTO.md`
(bookkeeping); `roadmap/HOJA_DE_RUTA.md` y todo `legal/` sin tocar, confirmado por `git diff` vacío.

**Registro de hallazgos:** solo queda **#8** `ABIERTO` (dato de salud del artículo 9 del RGPD en R-02,
pregunta #16 de §6). Ninguna acción posible desde el código desde hace 12 pasadas consecutivas:
`db/011_justificacion_ausencia.sql` sigue sin aplicar, la pregunta #16 sigue con la columna
«Respuesta» vacía (lectura directa de la fila), y ninguno de los seis commits nuevos toca
`db/011_justificacion_ausencia.sql` ni `legal/`. Sigue **ABIERTO**, duodécimo ciclo consecutivo sin
novedad de fondo — actualizado su texto en el registro de arriba. Dato positivo: la propia spec de
R-21 (mismo commit `454da97`) cita explícitamente este hallazgo como lección aprendida y su migración
`017` lo demuestra en la práctica — `motivo`/`motivo_anulacion` de `pausa_alumno` son SIEMPRE texto
libre opcional, sin ningún `CHECK` de lista cerrada ni opción que categorice salud (verificado
leyendo `db/017_pausa_alumno.sql` línea a línea, no solo su comentario de cabecera).

**Auditoría de la migración `017_pausa_alumno.sql` (R-21) contra los puntos de control permanentes,**
leída completa junto con su sección de pruebas y su justificación en `DECISIONES_TECNICAS.md`:
- **RLS completa:** `alter table ... enable row level security` presente; dos políticas `select`
  (`administrator` lee todas, `teacher` solo las `activa` de sus propios alumnos vía `exists` contra
  `slot_horario.profesor_id = auth.uid()`); ninguna política para `student` — coherente con el
  checkpoint "rol `student` cerrado".
- **Escritura solo por RPC:** `revoke all ... from anon, authenticated, service_role` seguido de
  `grant select ... to authenticated` únicamente — sin `insert`/`update`/`delete` a `authenticated`.
  Las tres únicas vías de escritura (`declarar_pausa_alumno`/`cancelar_pausa_alumno`/
  `acortar_pausa_alumno`) son `SECURITY DEFINER`, comprueban `rol_actual() = 'administrator'` como
  primera línea y se conceden a `authenticated` en general (mismo patrón razonado que
  `excepcion_slot`: es el `if` interno quien rechaza a `teacher`/`student`, no la ausencia de
  `GRANT`) — verificado también por los tres casos negativos de la sección 8n de `pruebas_rls.sql`.
- **Privilegios de tabla:** ningún `TRUNCATE` ni `GRANT` de más — barrido de `grant .* to
  (anon|authenticated)` sobre los 18 ficheros de `db/*.sql` no muestra ninguna concesión fuera de lo
  ya auditado en pasadas anteriores; el único `grant` nuevo es el `select` de `pausa_alumno` citado
  arriba.
- **No-retroactividad:** `declarar_pausa_alumno` rechaza cualquier rango que se solape con un
  registro de asistencia ya existente del alumno (requisito 4) con una comprobación atómica en el
  servidor, no en el cliente; `cancelar_pausa_alumno` exige que la pausa no haya empezado;
  `acortar_pausa_alumno` exige que esté en curso hoy y solo permite mover `fecha_fin` hacia una fecha
  futura, nunca hacia el pasado ni más allá de la fecha actual.
- **Alcance de datos personales:** ninguna columna nueva fuera de la lista cerrada de §0.2 — `motivo`
  es libre pero sin ninguna categoría cerrada de salud, y la tabla no añade ningún dato de contacto
  ni de persona de referencia.
- **Decisión razonada, no omisión:** `DECISIONES_TECNICAS.md` documenta por qué `registrar_asistencia`/
  `registrar_ausencia` NO se tocan (la pausa es un filtro de propuesta en cliente, no un bloqueo de
  servidor sobre un alta manual) — coherente con el requisito 2 de R-21, que habla literalmente de
  "no se ofrece como pendiente", nunca de rechazar un alta explícita fuera de ese flujo.

**Calidad real de los tests (T-03):** `db/pruebas_rls.sql` sección 8n añade 17 comprobaciones propias
(no genéricas): rechazo de `teacher`/`student` en las tres RPC, rango invertido, solape con un
registro de asistencia real (creado retroactivo a propósito para no chocar con la pausa "en curso"
de la misma sección — razonamiento explícito y correcto en `DECISIONES_TECNICAS.md`), solape entre
dos pausas activas, cancelar una ya empezada, cancelar sin motivo, acortar a una fecha no anterior a
la actual, más el barrido de lectura cruzada `teacher`/`teacher2`/`administrator`. Usa su propio
alumno de prueba en vez de `alumno_prueba` (evita falsos rechazos por el historial acumulado de
secciones previas) — mismo nivel de cuidado que las secciones 8g-8k ya auditadas. En el lado de
dominio, `src/dominio/pausaAlumno.ts` (leído completo) y su test (`pausaAlumno.test.ts`, 30 tests,
fixtures realistas con casos límite de solape/fecha) verifican comportamiento real, no tautologías.
Confirmado en ejecución por este auditor, no solo por el commit: `npm run typecheck` (limpio),
`npm run lint` (limpio) y `npm test` (**1678/1678**, coincide exactamente con lo que registra
`SEGUIMIENTO.md`).

**Coherencia roadmap-vs-código:** `SEGUIMIENTO.md` §1 y §3 reflejan R-21 como `BLOQUEADA — pendiente
aplicar migración 017` (fila 20 de §3) y R-22 como `PENDIENTE`, coincide con el estado real de
`db/APLICADAS.md` (`017` sin aplicar). §7 (desviaciones) sin fila nueva — correcto, R-21 no se
desvió de su propia spec en ningún punto que mereciera registrarse. `ROADMAP_PRODUCTO.md` (R-22,
Oleada v8) especifica una cuarta combinación de la matriz de excepciones (profesor, varios días) que
reutiliza la RPC ya existente de R-06 sin RPC nueva de escritura, sin dato de alumno y exclusiva de
`administrator` — sin riesgo de alcance visible en la especificación.

**Barrido de secretos:** `grep -rniE` de `service_role`/`SUPABASE_ACCESS_TOKEN`/JWT en base64 sobre
el repositorio completo: ninguna coincidencia real fuera de fixtures de test ya auditadas
(`registro.test.ts`, `informadorErrores.test.ts`) y menciones documentales/de nombre de rol.
`package.json` sigue con `dependencies` vacío (checkpoint "stack" intacto).

**Conclusión:** ningún hallazgo nuevo. `#8` sigue siendo el único `ABIERTO`, sin ninguna vía para
que cambie sin una decisión del dueño. R-21 es código de calidad — coherente con sus propias
decisiones documentadas, con la lección del hallazgo #8 ya incorporada al diseño, y con cobertura de
pruebas real, no decorativa — bloqueada únicamente por la aplicación pendiente de su migración, lo
cual es una acción del dueño fuera del alcance de esta auditoría.

---

### Auditoría 2026-09-15

**Alcance real de esta pasada — tres commits desde la anterior (`7477c26`, 2026-09-14):** `ea8e71f`
(R-20, registro de auditoría de cambios), `8937c53` (P-28, cierra el hallazgo #20) y `53a0bbe`
(vigésimo primer ciclo del PM, abre la Oleada v7 con R-21 — solo roadmap, cero código). `git
checkout develop && git pull origin develop`: fast-forward limpio hasta `53a0bbe`. `git diff
--stat 346969f..HEAD` (346969f es la base de la pasada anterior): 19 ficheros, +1863/-48, ninguna
migración nueva en `db/` (solo `db/pruebas_rls.sql`, fichero de pruebas).

**R-20 — registro de auditoría de cambios (`#/auditoria`), verificado exclusivo de
`administrator`.** Nueva pantalla que lista, de todo el centro y en orden cronológico inverso,
cada modificación o anulación de `asistencia_historial`. Verificado en dos capas, no solo una:
(a) `dominio/permisosUi.ts#puedeVerRegistroAuditoria` devuelve `true` únicamente para
`administrator`, con test que cubre los tres roles (`permisosUi.test.ts`); (b) el montaje real en
`ui/aplicacion.ts` cuelga del mismo `if (perfil.rol === 'administrator' && ...)` que ya gatea toda
la aplicación de `administrator` — un `teacher`/`student` no llega ni a construir el router que
conoce la ruta `auditoria`. Por debajo de las dos, la barrera que de verdad importa sigue intacta:
`datos/asistencia.ts#listarHistorialDeCentro` consulta `asistencia_historial` sin repetir ninguna
comprobación de rol, apoyándose en `asistencia_historial_admin_leer` (`003_politicas_rls.sql:218-220`,
sin cambio en esta pasada) — confirmado con lectura directa de la política: sigue siendo la única
vía de lectura y sigue exigiendo `es_administrator()`. Sin migración (T-07/T-10 ya dejaban la tabla
y su política escritas); ningún `GRANT` nuevo. El filtro de "autor del cambio" usa `listarUsuarios`
sin restringir por rol (a diferencia del filtro de "profesor" de T-23) — decisión razonada y
correcta en `DECISIONES_TECNICAS.md`: aquí se filtra por quién *hizo* el cambio, que puede ser
tanto un `teacher` como un `administrator`, no por a quién pertenece el registro. `SEGUIMIENTO.md`
§7 ganó su fila para la única desviación real (el enlace "Ver registro completo" puede apuntar al
slot/día de ANTES del cambio auditado en el caso raro de que el cambio fuera precisamente mover el
slot o el día) en el mismo commit — sin que hiciera falta que este auditor lo señalara, la lección
de los hallazgos #9/#11/#19 parece asentada ya como hábito del equipo.

**P-28 cierra el hallazgo #20 (severidad media, ABIERTO desde 2026-09-14).** Verificado por lectura
directa de `db/pruebas_rls.sql:873-967` (sección 7c, nueva): reproduce el patrón exacto de P-06
para `anon` — `student` no lee el avatar del alumno activo, no lee el del inactivo, no puede
escribir en el bucket, los tres con comprobación previa (sin RLS) de que la fila de fixture existe
antes de exigir que `student` no la vea. Cerrado en la tabla de arriba.

**Verificación en ejecución real de esta pasada (no solo lectura de SQL/TypeScript):** `npm ci`
(130 paquetes, 0 vulnerabilidades) → `npm run typecheck` (limpio) → `npm run lint` (limpio) →
`npm test` → **1617/1617, 0 fallos**, coincide exactamente con la cifra que declara el commit de
R-20 → `npm run build` (limpio). Barrido de secretos (`service_role`, `SUPABASE_ACCESS_TOKEN`,
`sk-`, `eyJhbGci`, `-----BEGIN`) sobre `dist/` recién construido: cero coincidencias reales.
`package.json` sigue sin ninguna clave `dependencies` (solo `devDependencies` de herramientas de
desarrollo): el stack invariante se mantiene. `herramientas/migrar.ts` y `roadmap/HOJA_DE_RUTA.md`
sin cambio desde 2026-09-05 (confirmado con `git log -1`), así que sus guardas y su inmutabilidad
no requieren nueva verificación de fondo esta pasada.

**Hallazgo #8 (RGPD art. 9, alta, ABIERTO desde 2026-09-05) confirmado sin cambio, undécimo ciclo
consecutivo:** ninguno de los tres commits nuevos toca `db/011_justificacion_ausencia.sql` ni
`legal/`; la pregunta #16 de §6 sigue sin respuesta del dueño. Nota positiva sin afectar al cierre:
la spec nueva de R-21 (pausa programada de un alumno) cita explícitamente este hallazgo como lección
aprendida y diseña su propio campo de motivo como texto libre opcional, nunca una lista cerrada —
señal de que la restricción ha calado en el diseño de tareas nuevas, no solo en la corrección de la
que la originó.

**Coherencia roadmap/código:** `DECISIONES_TECNICAS.md` registra las cinco decisiones de R-20 y las
tres de P-28 con razonamiento explícito, ninguna contradice §0.2 de `HOJA_DE_RUTA.md`. R-21 (Oleada
v7) es solo spec, sin código todavía — su propio texto declara "sin dato personal nuevo ni cambio
de alcance para el rol `student`", consistente con lo que pide su requisito 1 (motivo libre, nunca
categoría cerrada). Sin hallazgos nuevos en esta pasada.

### Auditoría 2026-09-14

**Alcance real de esta pasada — un único commit desde la anterior (`346969f`, 2026-09-13): bookkeeping de roadmap puro, cero código, cero SQL.** `git checkout develop && git pull origin develop`: fast-forward limpio hasta `bbc329e` ("roadmap: vigésimo ciclo del PM — sin R-XX nueva, R-20 sigue pendiente"). `git diff --stat 346969f..HEAD`: 3 ficheros, +68/-18, los tres en `roadmap/` (`HISTORIAL_SESIONES.md`, `ROADMAP_PRODUCTO.md`, `SEGUIMIENTO.md`); ninguno en `src/`, `db/` ni `legal/`. R-20 (registro de auditoría de cambios) sigue `PENDIENTE`, sin que ningún programador la haya tomado.

**Verificación en ejecución, no solo lectura.** `npm ci` (130 paquetes) y, con dependencias instaladas: `npm run typecheck` limpio, `npm run lint` limpio, `npm test` → **1586/1586, 0 fallos**, `npm run build` limpio — misma cifra exacta que viene repitiéndose desde `acff221` (R-19), confirmada ejecutando la suite de verdad.

**El hallazgo #8 (dato de salud del artículo 9 en `motivo_justificacion`, R-02) sigue `ABIERTO`, sin cambio de fondo — décimo ciclo consecutivo sin novedad.** La pregunta #16 de §6 de `SEGUIMIENTO.md` sigue con la columna "Respuesta" vacía, la fila 14 de §3 mantiene literalmente la misma condición de bloqueo, y `db/APLICADAS.md` no registra `011` como aplicada (`009` sigue siendo la última). El único commit de este lote es del PM, no del dueño, así que no había ninguna vía para que cambiara. Se actualiza su fila con esta confirmación, sin abrir una entrada nueva.

**Repaso completo de los dieciséis puntos de control permanentes, esta vez con dos subagentes de investigación despachados en paralelo (uno centrado en el `db/` completo: RLS, privilegios, triggers, bucket, superficie de columnas; otro en conformidad de stack y calidad real de los tests) más lectura directa de este auditor sobre los ficheros más sensibles, sin aceptar la palabra de la pasada anterior.** Verificado con cita de fichero y línea, sin ninguna excepción:

- `asistencia`: `revoke all` a `anon`/`authenticated`/`service_role` y `grant select` solo a `service_role` (`001_esquema_inicial.sql:322-323`); ninguna política de `INSERT`/`UPDATE` directo, ninguna de `DELETE`, en ningún fichero de `db/`. Escritura solo por `registrar_asistencia`/`actualizar_asistencia` (`SECURITY DEFINER`), confirmado también por `pruebas_rls.sql:705-715` (ni siquiera `administrator` puede `UPDATE`/`DELETE` directo).
- Trigger `asistencia_before_update` (`001_esquema_inicial.sql:289-311`) sigue abortando cambios en `registrado_en`/`profesor_id`/`peticion_id` y fija él mismo `actualizado_en`/`actualizado_por`. Trigger `asistencia_after_update` sigue copiando la fila anterior en `asistencia_historial`, tabla con `revoke all` a los tres roles (ni siquiera `service_role` tiene `UPDATE`/`DELETE`) y lectura solo para `administrator`.
- `actualizar_asistencia` sigue comprobando pertenencia (`profesor_id = auth.uid()`) y la ventana de 7 días dentro de la propia RPC — sin política `UPDATE` paralela en `asistencia` porque el `GRANT` de `UPDATE` a `authenticated` está revocado por completo: el cierre es aún más estricto que "RPC + política", es "RPC como única puerta posible".
- Ninguna sentencia `create policy` de todo `db/001`-`016` menciona a `student`, salvo `perfil_leer_propio` del bootstrap (`000_bootstrap_perfil.sql`). Confirmado también contra el bucket: las cuatro políticas de `storage.objects` (`003_politicas_rls.sql:247-278`) cubren solo `administrator` (lectura/escritura/actualización/borrado) y `teacher` (lectura acotada a `alumno.activo` vía `storage.foldername(name)`); ninguna para `anon` ni `student`.
- Ningún `grant` de todo el directorio concede `TRUNCATE`/`REFERENCES`/`TRIGGER` a `anon` ni a `authenticated` — el incidente de `perfil` sigue corregido desde `000b_arreglo_permisos.sql` y sigue sin repetirse en ninguna tabla nueva (`limite_tasa`, `excepcion_slot`, `cierre_centro`).
- Bucket `avatares` sigue privado (`public = false`, `004_bucket_avatares.sql:56-57`), ruta en base de datos (nunca URL), URL firmada de vida corta en el cliente.
- Superficie de columnas del `teacher`: el `grant select` de columna sobre `alumno` (`003_politicas_rls.sql:107-109`) sigue limitado a `id, nombre, primer_apellido, segundo_apellido, avatar_ruta, activo` — `email_alumno`/`telefono_alumno` fuera del `GRANT`, así que una consulta directa por esas columnas falla con un error real de Postgres para cualquiera. La vista `alumno_ficha` (sin `security_invoker`, filtro `where es_administrator()` escrito a mano) sigue siendo el único camino de lectura completa.
- `persona_referencia` sigue con política `for all` exclusiva de `administrator` (`003_politicas_rls.sql:159-162`), sin ningún `GRANT` ni política que alcance a `teacher`.
- Todas las tablas de `public` (`000`-`016`) tienen `enable row level security` inmediatamente tras su creación; las dos únicas sin políticas explícitas (`esquema_migracion`, `limite_tasa`) no tienen ningún `GRANT` a rol de API, por diseño, no por descuido.
- `es_retroactivo` sigue forzado por `CHECK` (`asistencia_retroactivo_coherente`, `001_esquema_inicial.sql:265-267`), no por confianza en el cliente; el snapshot del slot en `asistencia` (`slot_dia_semana`/`slot_hora_inicio`/`slot_hora_fin`/`slot_asignatura_o_grupo`) se congela al escribir y no se recalcula nunca del horario vigente.
- `herramientas/migraciones/guardas.ts` conserva los siete patrones prohibidos (`DROP TABLE`, `DROP SCHEMA`, `TRUNCATE`, `DISABLE ROW LEVEL SECURITY`, `DROP POLICY` sin `CREATE POLICY`, `DELETE` sobre `asistencia`, `UPDATE`/`DELETE` sobre `asistencia_historial`), con comentarios SQL despojados antes de escanear para que la propia prosa no dispare un falso positivo; `entorno.ts` sigue exigiendo `--entorno=prod` **y** `PERMITIR_PROD=1` a la vez.
- `package.json` sigue sin clave `dependencies` (siete `devDependencies` de herramienta); cero imports de `@supabase/supabase-js`, cero framework de UI, cero `innerHTML` real en todo `src/` (las únicas apariciones son comentarios que documentan la prohibición); `fetch` aparece solo dentro de `src/datos/**`; `console.*` aparece solo dentro de `src/nucleo/registro.ts`. Las cuatro guardas viven en `eslint.config.js` por herramienta, no solo por documento.
- Barrido de secretos sobre el repositorio completo: sin ninguna coincidencia real de `service_role`, contraseña de base de datos, token de la Management API con forma de JWT, ni en el código ni en la documentación; `.env.ejemplo` solo declara nombres de variable, sin valores; `herramientas/verificarFugaSecretos.test.ts` construye de verdad `dist/` con `tsc -b` y escanea los artefactos generados, no es un test de fachada.
- Calidad real de la suite (T-03): inspección directa de `src/dominio/asistencia.test.ts`, `historicoAsistencia.test.ts`, `slotHorario.test.ts`, `expedienteAlumno.test.ts` y `src/nucleo/limitadorTasa.test.ts` confirma reloj inyectado en los cinco (nunca hora del sistema), aserciones de valor exacto (no solo `truthy`), casos de borde de ventana/vigencia probados en ambos sentidos, y un test dedicado de no-retroactividad que edita el horario después del hecho y comprueba que el CSV exportado no cambia porque lee el snapshot, nunca el horario vigente. No es una suite decorativa.

**Hallazgo nuevo — #20, severidad media: el barrido obligatorio de `student` de `db/pruebas_rls.sql` (sección 6) nunca alcanza el bucket de avatares.** T-10 (requisito 5 de su spec) exige explícitamente que ese barrido falle "en todas las tablas **y en el bucket**", y la propia tabla de puntos de control de este documento lo repite ("incluido el barrido del student"). La sección 6 recorre diez tablas de `public` pero no incluye `storage.objects`; la sección 7 (avatares) solo suplanta a `teacher` y `administrator`; la sección 8f cubre `anon` contra el mismo bucket, ampliación que **P-06** añadió expresamente el 2026-09-02 citando el mismo patrón de bucle de la sección 6 — pero esa ampliación nunca se replicó para `student`. Verificado con `grep` sobre el fichero completo: cero coincidencias de `student` combinado con `avatar`/`storage`/`objects`. La política SQL en sí es correcta (`003_politicas_rls.sql:247-278` no menciona a `student` en ninguna de las cuatro políticas del bucket), así que no hay fuga activa hoy: lo que falta es la prueba en ejecución que lo demuestre y que actúe de guarda de regresión si algún día una migración futura ampliara por error una política del bucket. Es la única combinación rol×recurso de todo el proyecto donde el rol más restringido no tiene un caso negativo ejecutable contra el recurso más sensible del sistema (fotos de menores). Mismo patrón que el hallazgo **#2** (batería que no ejercita un caso obligatorio de su propia spec), pero de alcance mucho más acotado — una sola celda de la matriz, no varias operaciones sobre cuatro tablas —, de ahí severidad media y no alta. Fila añadida al registro de hallazgos con el arreglo propuesto (replicar en la sección 6 el mismo patrón que P-06 ya usó para `anon`).

**Conclusión de esta pasada: un hallazgo nuevo de severidad media (#20, hueco de cobertura de tests, no defecto de seguridad activo), sin ningún hallazgo `RESUELTO` que reabrir.** El registro queda con dos `ABIERTO` (#8, decisión de negocio pendiente del dueño; #20, hueco de cobertura de la batería de RLS) y dieciocho `RESUELTO`. Suite de tests, tipos, lint y build limpios en ejecución real (1586/1586); sin secretos en el repositorio; los dieciséis puntos de control permanentes, correctos sin excepción, verificados de forma independiente por dos subagentes de investigación que no se vieron entre sí y llegaron a la misma conclusión, más lectura directa de este auditor sobre las piezas más sensibles (triggers de `asistencia`, políticas del bucket, vista `alumno_ficha`, guardas del runner, configuración de ESLint).
