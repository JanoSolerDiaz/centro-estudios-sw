...
> **Escrita y pendiente de aplicar:** `db/006_arreglo_limite_tasa_ambiguo.sql` (hash
> `6de505c4b933`) — arregla el bug de la `005` cualificando las cuatro lecturas de la fila previa
> con `limite_tasa.` dentro del `ON CONFLICT ... DO UPDATE` de `aplicar_limite_tasa()`. Misma firma,
> mismo `SECURITY DEFINER`, mismo `errcode = 'PT429'`; no toca nada más. Fila 8 de §3 de
> `SEGUIMIENTO.md`. **No tiene fila en la tabla de arriba a propósito:** esa tabla es solo de
> migraciones verificadas en ejecución, y ésta se anotará cuando `esquema_version()` devuelva `6`.
