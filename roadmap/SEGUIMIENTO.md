...
**Última actualización:** 2026-09-02 (sesión de backlog P-XX) — T-24 sigue **BLOQUEADA por la
migración `009`** (sin cambio: el dueño todavía no la ha aplicado, fila 11 de §3), T-25 depende de
que T-24 se complete de verdad y las R-XX esperan a que el MVP completo esté en producción (§1 de
`ROADMAP_PRODUCTO.md`), así que no había ninguna tarea de la columna vertebral desbloqueada para
esta sesión. Ningún hallazgo `ABIERTO` de severidad alta en `auditoriacontinua.md` (los tres abiertos
siguen siendo de severidad baja, higiene documental). Con la cola principal bloqueada, esta sesión
atendió el backlog `P-XX` pendiente sin urgencia (§0.3: "las P-XX se ejecutan cuando la tarea en
curso está terminada o bloqueada", "no acumular más de 3 P-XX entre dos tareas consecutivas"): las
cuatro que quedaban `PENDIENTE` en §5 se atendieron —una de más sobre la guía de "máximo 3",
justificado porque dos de las cuatro (`P-13`/`P-14`) son correcciones de una sola frase, sin código,
de coste marginal—. **P-05** (los CLI de migraciones ya imprimen el cuerpo real del error de la
Management API, no solo su plantilla genérica, con test), **P-13** (frase residual de
`db/MODELO.md` sobre el montaje del avatar, corregida), **P-14** (numeración cruzada de las
preguntas #12/#13 corregida en este documento y en `DECISIONES_TECNICAS.md`) y **P-15**
(`columnasVisiblesFichaAlumno`, código muerto, eliminada — no conectada, porque el escenario que
preveía, un `teacher` con una ficha de alumno reducida, está prohibido por regla permanente de §0.2,
no solo sin programar). Verificación pre-push completa en verde (`typecheck`, `lint`, 939 tests,
`build`). Detalle de las cuatro en §5 y en `DECISIONES_TECNICAS.md`.

**Sesión anterior (2026-09-02, "T-23 COMPLETADA"):** desbloqueó T-20 y T-21 en este §1 y anotó
