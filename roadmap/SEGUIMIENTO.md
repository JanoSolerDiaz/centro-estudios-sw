...
| T-18 | Alta de asistencia (RPC `registrar_asistencia`) | BLOQUEADA — pendiente aplicar migración `006` | 2026-09-01 | `005_rpc_registrar_asistencia.sql` (renumerada, ver §7) **aplicada en `dev` el 2026-09-01** y verificada (`esquema_version()` = `5`), pero el `npm run probar-rls` de esa misma aplicación destapó un bug dentro de ella: `aplicar_limite_tasa()` lee la fila previa **sin cualificar** dentro de su `ON CONFLICT ... DO UPDATE`, y PostgreSQL la rechaza (`column reference "ventana_inicio" is ambiguous`) porque `limite_tasa` y `excluded` están las dos en ámbito. Tumba las trece comprobaciones de la sección 7b. Arreglo en `006_arreglo_limite_tasa_ambiguo.sql` (fila 8 de §3): una migración aplicada es inmutable, así que `005` no se toca. El resto del alcance de la tarea sigue completo (dominio, datos, tests estáticos y `db/pruebas_rls.sql`). Límite de 60 operaciones por profesor y minuto conectado por primera vez (`limite_tasa`/`aplicar_limite_tasa`) |
...
| 8 | Aplicar la migración `006_arreglo_limite_tasa_ambiguo` en `dev`, **después** de la fila 7 | T-18 | `git pull` y `npm run migrate` en local (aplica en orden numérico). Al terminar, comprobar que `esquema_version()` devuelve `6` y ejecutar **`npm run probar-rls`**: las 67 comprobaciones deben salir **0 fallidas y 0 omitidas**. Mira además el detalle de las líneas `registrar_asistencia / ... (debe fallar)`: ninguna debe seguir diciendo `ventana_inicio is ambiguous`, cada una debe traer su motivo propio (ventana retroactiva excedida, `ocurrido_en` en el futuro, origen `slot` sin `slot_id`, alumno de baja, los dos duplicados, slot de otro profesor, `student` sin acceso). Si algo sale en rojo, pega la salida tal cual | PENDIENTE |
...
| P-10 | **Backlog técnico (fiabilidad del veredicto, hermano de P-07): las comprobaciones "debe fallar" de la batería aprueban con CUALQUIER error, no con el suyo.** Cada bloque `prohibido` de `db/pruebas_rls.sql` registra `ok = true` desde su `exception when others`, sin mirar `sqlerrm`. El 2026-09-01 eso salió a la luz de la peor forma posible: el bug de `aplicar_limite_tasa` (arreglado en `006_arreglo_limite_tasa_ambiguo.sql`) tumbaba las **trece** comprobaciones de la sección 7b, y solo cuatro salieron en rojo —las de acceso *permitido*—; las otras nueve cantaron **`[OK]` por el motivo equivocado**, porque un `column reference is ambiguous` también es un error. **Arreglo propuesto:** que `pg_temp.registrar` acepte un patrón de motivo esperado y que cada caso `prohibido` lo declare (`%row-level security%`, `%permission denied%`, `%ventana permitida%`…); un error que no encaje pasa a `FALLO` con el `sqlerrm` real en `detalle` | El valor de una batería de seguridad está en que un verde signifique lo que dice. Hoy un fallo de **implementación** de una RPC se disfraza de control de acceso funcionando: justo el escenario que esta batería existe para descartar. Origen: hallazgo propio del 2026-09-01 al aplicar `005` | **PROPUESTA 2026-09-01, pendiente de implementar.** Deliberadamente NO se implementa en el mismo empujón que `006`: el agente no puede ejecutar `db/pruebas_rls.sql` (§0.1, no tiene el token de la Management API), y meter cambios no verificados en el mismo fichero que el dueño necesita para validar el arreglo arriesga romperle la validación. Se implementa en la sesión siguiente, una vez `006` esté confirmada | — |
