...
| T-18 | Alta de asistencia (RPC `registrar_asistencia`) | BLOQUEADA — pendiente verificar con `npm run probar-rls` | 2026-09-01 | Las dos migraciones **aplicadas en `dev` el 2026-09-01** y confirmadas en el ledger con `npm run migrate -- --estado`: `005_rpc_registrar_asistencia.sql` (hash `ad3b648eeeb0`) y `006_arreglo_limite_tasa_ambiguo.sql` (hash `6de505c4b933`), que arregla el `column reference "ventana_inicio" is ambiguous` de `aplicar_limite_tasa()`. Código y tests completos (dominio, datos, estáticos y `db/pruebas_rls.sql`). **Lo único que falta para cerrarla es la ejecución de `npm run probar-rls`** que confirme la sección 7b en verde — el agente no puede ejecutarla (§0.1). Fila 8 de §3 |
...
| 8 | Ejecutar `npm run probar-rls` tras la migración `006_arreglo_limite_tasa_ambiguo` | T-18 | ~~`git pull` y `npm run migrate` en local~~ **hecho el 2026-09-01**: `npm run migrate -- --estado` lista la `006` con hash `6de505c4b933`, y las seis migraciones del runner cuadran con sus ficheros. **Falta la mitad de verificación:** ejecutar `npm run probar-rls` y comprobar 67 comprobaciones, **0 fallidas y 0 omitidas**. Mira además el detalle de las líneas `registrar_asistencia / ... (debe fallar)`: ninguna debe seguir diciendo `ventana_inicio is ambiguous`, cada una debe traer su motivo propio (ventana retroactiva excedida, `ocurrido_en` en el futuro, origen `slot` sin `slot_id`, alumno de baja, los dos duplicados, slot de otro profesor, `student` sin acceso). Si algo sale en rojo, pega la salida tal cual | PENDIENTE — solo la verificación |
