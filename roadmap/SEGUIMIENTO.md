...
**Última actualización:** 2026-09-22 (rutina programada de programador — **P-31/P-32, backlog del
...
sin ninguna vía de esta sesión, y es el único de severidad alta — no hay ninguna urgencia que tratar
...
nueva es trabajo del PM, no de esta sesión, así que, con la columna vertebral terminada, esta sesión
toma backlog de auditoría (§0.3: "las P-XX se ejecutan cuando la tarea en curso está terminada o
bloqueada"). **P-31** atiende el hallazgo **#23** (media, "controles muertos por migración
bloqueada"): P-30 (sesión anterior) ya cortaba la llamada de red de "Justificar"/"Marcar salida"
mientras `011`/`012` sigan sin aplicar, pero ninguna pantalla consultaba esa misma señal ANTES de
pintar el control — un profesor o `administrator` podía rellenar el formulario entero para recibir,
solo al pulsar, el mensaje de "todavía no disponible". `datos/asistencia.ts` gana
...
verde antes del push. Mergeado a `develop` al cierre de esta sesión, sin tocar `master` en ningún
momento. Con la Oleada v12 ya cerrada y sin ninguna R-XX nueva abierta, la siguiente sesión de
programador sigue sin cola normal propia hasta que el PM abra la próxima oleada.
