...
| 5 | Aplicar la migración `003_politicas_rls` en `dev`, **después** de la fila 4 | T-10 | ~~`git pull` y `npm run migrate` en local (aplica en orden numérico: no hace nada si `002` sigue pendiente). Al terminar, comprobar que `esquema_version()` devuelve `3`. Opcional pero recomendado: ejecutar también `npm run probar-rls` y revisar que no haya ninguna fila `FALLO`~~ | **RESUELTA 2026-08-31 — migración aplicada Y políticas verificadas en ejecución.** `esquema_version()` = `3`. `npm run probar-rls` contra `dev`: **51 comprobaciones, 4 omitidas, 0 fallidas**. Quedó probado contra la base real lo que hasta hoy solo estaba en SQL estático: que un `teacher` lee las columnas de identificación de un alumno pero **no** `email_alumno` (requisito 4 de T-12/T-13, punto de control permanente del auditor); que la política `for all` de `persona_referencia` bloquea al profesor en SELECT, INSERT, UPDATE y DELETE y permite las cuatro al administrador; que el `student` no lee ninguna de las siete tablas; que `TRUNCATE` está denegado en las ocho para ambos roles; y que `asistencia` rechaza el INSERT directo incluso al administrador. **Salvedad, y no es menor: el aislamiento entre dos profesores sigue sin probarse** — `slot_horario / teacher2 no lee el ajeno` se omite porque `dev` solo tenía un profesor. La semilla ya trae el segundo (P-07 punto b, implementado el 2026-08-31): queda que el dueño ejecute `npm run seed` y relance `probar-rls`, y entonces las omisiones deben bajar de 4 a 3. Llegar aquí exigió arreglar la batería dos veces el mismo día: **P-08** (una regresión que desactivaba diez comprobaciones en silencio) y el `grant` de `_resultados_prueba_rls` |
