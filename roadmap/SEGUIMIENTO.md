...
| T-18 | Alta de asistencia (RPC `registrar_asistencia`) | BLOQUEADA — pendiente verificar con `npm run probar-rls` | 2026-09-01 | Migraciones `005` y `006` **aplicadas y verificadas en el ledger** de `dev`. La RPC **funciona**: la ejecución del 2026-09-01 confirma que el `column reference "ventana_inicio" is ambiguous` desapareció y que las siete validaciones de dominio rechazan por su motivo real (ventana retroactiva, futuro, `slot` sin id, alumno de baja, en nombre de otro, slot ajeno, `student`). Los 6 fallos que quedaron eran **de la batería, no del esquema**: `select f(...) into v_fila` mete la fila compuesta entera en el primer campo, y el `exception` que lo capturaba deshacía el alta que la RPC sí había hecho, dejando sin nada con lo que chocar a las dos comprobaciones de duplicado. Corregido (P-12) junto con P-10; falta una ejecución más que lo confirme (fila 8 de §3) |
...
| 8 | Volver a ejecutar `npm run probar-rls` tras corregir la batería | T-18 | ~~`git pull` y `npm run migrate`~~ **hecho el 2026-09-01** (`005` y `006` en el ledger). ~~Primera ejecución de `probar-rls`~~ **hecha**: confirmó que el ambiguo desapareció y destapó 6 fallos propios de la batería, ya corregidos. **Falta:** `git pull` y `npm run probar-rls` otra vez, esperando **67 comprobaciones, 0 fallidas y 0 omitidas**. Ahora cada línea "debe fallar" exige su motivo: si alguna sale roja con `ERROR INESPERADO — se esperaba … y llego: …`, pega esa línea tal cual, porque significa que el rechazo se produjo por una razón distinta de la esperada | PENDIENTE — solo la verificación |
