...
exclusivamente en el dueño, nada que tratar como P-XX antes de la cola. Revisada §1 completa hasta
R-32 inclusive: **ninguna fila queda `PENDIENTE`**, todo `COMPLETADA` o `BLOQUEADA` por
migración/T-25 (§3, filas 12-22). Revisado `ROADMAP_PRODUCTO.md`: ninguna R-XX por delante de R-32.
Revisado `roadmap/FEEDBACK.md`: sigue con su única fila plantilla vacía, nada que convertir. Sin
ninguna R-XX nueva del PM, ningún hallazgo urgente y ningún backlog propio, esta sesión no tiene
ningún trabajo de código que hacer. Verificación pre-push completa igualmente ejecutada para
confirmar que el estado sigue verde: contenedor sin `node_modules`, `npm ci` recuperó los 130
paquetes declarados (0 vulnerabilidades) + `npm run typecheck` + `npm run lint` + `npm test`
(**1901/1901**, mismo número que la pasada anterior — ningún fichero de `src/` cambió) + `npm run
build`, los cuatro en verde. Commit solo de esta entrada de bitácora y esta cabecera, empujado a
`develop`, sin tocar `master` en ningún momento. La siguiente sesión seguirá sin cola normal propia
hasta que el dueño aplique alguna de las migraciones pendientes, responda la pregunta #16, o el PM
abra la siguiente oleada — esto no es un fallo de proceso, es el estado correcto de una cola
agotada. — Programador, 2026-09-25

**Sesión anterior (2026-09-25, rutina programada de programador, tercera pasada del día —
cola vacía, nada que hacer): protocolo primero — `git checkout develop && git pull origin develop`
fast-forward limpio desde la pasada anterior de esta misma rutina (`fe23ee5`, sin commits entre
medias — ni del dueño ni del PM ni del auditor). Revisado `auditoriacontinua.md`: el único hallazgo
`ABIERTO` sigue siendo **#8** (alta, RGPD artículo 9 en R-02, `motivo_justificacion`), sin novedad de
fondo en la pregunta #16 de §6 (columna «Respuesta» sigue vacía) — bloqueado exclusivamente en el
dueño, nada que tratar como P-XX antes de la cola. Revisada §1 completa hasta R-32 inclusive:
**ninguna fila queda `PENDIENTE`**, todo `COMPLETADA` o `BLOQUEADA` por migración/T-25 (§3, filas
12-22). Revisado `ROADMAP_PRODUCTO.md`: ninguna R-XX por delante de R-32. Revisado
`roadmap/FEEDBACK.md`: sigue con su única fila plantilla vacía, nada que convertir. Sin ninguna R-XX
...
vulnerabilidades) + `npm run typecheck` + `npm run lint` + `npm test` (**1901/1901**, mismo número
...
es un fallo de proceso, es el estado correcto de una cola agotada. — Programador, 2026-09-25

**Sesión anterior (2026-09-25, rutina programada de programador, segunda pasada del día — cola
vacía, nada que hacer): protocolo primero — `git checkout develop && git pull origin develop`
fast-forward limpio desde la pasada anterior de esta misma rutina (`714570b`, sin commits entre
medias — ni del dueño ni del PM ni del auditor). Revisado `auditoriacontinua.md`: el único hallazgo
`ABIERTO` sigue siendo **#8** (alta, RGPD artículo 9 en R-02, `motivo_justificacion`), sin novedad de
fondo en la pregunta #16 de §6 (columna «Respuesta» sigue vacía) — bloqueado exclusivamente en el
dueño, nada que tratar como P-XX antes de la cola. Revisada §1 completa hasta R-32 inclusive:
...
**Sesión anterior (2026-09-25, rutina programada de programador — cola vacía, nada que
...
momento. La siguiente sesión seguirá sin cola normal propia hasta que el dueño aplique alguna de las
...
en R-02), decimoséptimo ciclo consecutivo sin novedad de fondo en la pregunta #16 de §6 — bloqueado
exclusivamente en el dueño, nada que tratar como P-XX antes de la cola. Revisada §1 completa (lectura
directa de la tabla, no solo la bitácora): **ninguna fila queda `PENDIENTE`** salvo la propia fila 12
de §3 (T-25, paso a producción, bloqueada en el dueño) — todo lo demás `COMPLETADA` o `BLOQUEADA` por
migración (§3, filas 13-22). Revisado `FEEDBACK.md`: sigue con su única fila plantilla vacía, nada que
convertir. Sin ninguna R-XX nueva del PM, ningún hallazgo urgente y ningún backlog propio, esta sesión
no tiene ningún trabajo de código que hacer. Verificación pre-push completa igualmente ejecutada para
confirmar que el estado sigue verde: contenedor sin `node_modules`, `npm ci` recuperó los 130 paquetes
declarados (0 vulnerabilidades) + `npm run typecheck` + `npm run lint` + `npm test` (**1894/1894**,
mismo número que la pasada anterior de R-31 — ningún fichero de `src/` cambió) + `npm run build`, los
cuatro en verde. Commit solo de esta entrada de bitácora y esta cabecera, empujado a `develop`, sin
tocar `master` en ningún momento. La siguiente sesión seguirá sin cola normal propia hasta que el
dueño aplique alguna de las migraciones pendientes, responda la pregunta #16, o el PM abra la
siguiente oleada — esto no es un fallo de proceso, es el estado correcto de una cola agotada. —
...
`BLOQUEADA` sin aplicar su migración; nada que tratar como P-XX antes de la cola. Revisada §1: la
siguiente tarea pendiente era **R-30** (De aviso a excepción, en un enlace). Implementada tal cual su
spec: `pantallaPanelCentro.ts` (bloque 5, avisos de ausencia de R-29) gana un botón «Declarar
sustitución o cancelación» junto a «Marcar atendido» en cada fila, detrás de una nueva dependencia
opcional `deps.irARegistro` (mismo patrón que el par `listarAvisosAusenciaPendientes`/
`marcarAvisoAusenciaAtendido` de R-29 y que `deps.irARegistro` ya usado por R-20 en
`pantallaRegistroAuditoria.ts`) — sin ruta, RPC ni columna nueva. El enlace navega a
`#/registros/<profesorId>/<slotId>/<fecha>` con los propios `profesor_id`/`slot_id`/`fecha_sesion` del
aviso; como `listarAvisosAusenciaPendientes` ya filtra por `estado = 'pendiente'`, ningún aviso
`atendido` llega nunca a esta lista, así que el requisito 4 (ocultar el enlace en un aviso ya atendido)
queda satisfecho por construcción, sin lógica adicional. `aplicacion.ts` conecta la dependencia
reutilizando `router.navegar({ nombre: 'registros', profesorId, slotId, fecha })`, idéntico al cableado
de R-20. Sin migración (requisito de la propia spec). 4 tests nuevos (3 en
`pantallaPanelCentro.test.ts`, 1 de cableado extremo a extremo en `aplicacion.test.ts` — 1867 en total,
antes 1863). Verificación pre-push completa en verde (`npm ci` fue necesario primero: `node_modules`
no existía en el contenedor de esta sesión) — `typecheck`, `lint`, 1867 tests, `build`, todos limpios.
R-30 pasa a `COMPLETADA` en §1. R-31 queda como siguiente tarea de la cola, sin empezar. — Programador,
2026-09-24

**Sesión anterior (2026-09-23, trigésimo ciclo del Product Manager — **cierra la Oleada v13
...
pregunta #16 de §6 sin ninguna vía de esta sesión — sigue siendo el único de severidad alta, no hay
urgencia que tratar antes de la cola. Revisada §1 completa (lectura directa de la tabla, no solo la
bitácora): **ninguna fila queda `PENDIENTE`**, sin cambio respecto a la pasada anterior — todo
`COMPLETADA` o `BLOQUEADA` por migración (§3, filas 13-22) o por el paso a producción de T-25 (fila
12). Revisada también §5 completa (los 32 P-XX): ninguno queda sin `RESUELTA`/`IMPLEMENTADA`.
Revisado `FEEDBACK.md`: sigue con su única fila plantilla vacía, nada que convertir. Sin ninguna
R-XX nueva del PM y sin hallazgo urgente ni backlog propio, esta sesión tampoco tiene ningún trabajo
de código que hacer. Verificación pre-push completa igualmente ejecutada para confirmar que el
estado sigue verde: contenedor sin `node_modules`, `npm ci` recuperó los 130 paquetes declarados (0
vulnerabilidades) + `npm run typecheck` + `npm run lint` + `npm test` (**1863/1863**, mismo número
que la pasada anterior — ningún fichero de `src/` cambió) + `npm run build`, los cuatro en verde.
Commit solo de esta entrada de bitácora y esta cabecera, empujado a `develop`, sin tocar `master` en
ningún momento. La siguiente sesión de programador seguirá sin cola normal propia hasta que el
dueño aplique alguna de las migraciones pendientes (`010`-`019`), responda la pregunta #16, o el PM
abra la siguiente oleada — esto no es un fallo de proceso, es el estado correcto de una cola
agotada. — Programador, 2026-09-23

**Sesión anterior (2026-09-23, rutina programada de programador, cuarta pasada del día — cola
vacía):** protocolo primero — `git checkout develop && git pull origin
develop` fast-forward limpio desde la pasada anterior de esta misma rutina (`de26e66`, tercera
pasada del día, sin commits entre medias). Revisado `auditoriacontinua.md` con lectura directa de
la columna `Estado`: solo **#8** (alta, RGPD artículo 9 en R-02) sigue `ABIERTO`, bloqueado en la
pregunta #16 de §6 sin ninguna vía de esa sesión — sigue siendo el único de severidad alta, no hay
urgencia que tratar antes de la cola. Revisada §1 completa (lectura directa de la tabla, no solo la
bitácora): **ninguna fila queda `PENDIENTE`**, sin cambio respecto a la pasada anterior — todo
`COMPLETADA` o `BLOQUEADA` por migración (§3, filas 13-22) o por el paso a producción de T-25 (fila
12). Revisada también §5 completa (los 32 P-XX): ninguno queda sin `RESUELTA`/`IMPLEMENTADA`.
Revisado `FEEDBACK.md`: sigue con su única fila plantilla vacía, nada que convertir. Sin ninguna
R-XX nueva del PM y sin hallazgo urgente ni backlog propio, esa sesión tampoco tuvo ningún trabajo
de código que hacer. Verificación pre-push completa igualmente ejecutada para confirmar que el
estado seguía verde: contenedor sin `node_modules`, `npm ci` recuperó los 130 paquetes declarados (0
vulnerabilidades) + `npm run typecheck` + `npm run lint` + `npm test` (**1863/1863**, mismo número
que la pasada anterior — ningún fichero de `src/` cambió) + `npm run build`, los cuatro en verde.

**Sesión anterior (2026-09-23, rutina programada de programador, tercera pasada del día — cola
vacía):** protocolo primero — `git checkout develop && git pull origin
...
protocolo primero — `git checkout develop && git pull origin develop` fast-forward limpio desde la
última rutina de programador (`19e893b`, quinta pasada del día, cola vacía). Revisado el registro de
hallazgos de `auditoriacontinua.md` (24 filas, lectura directa de la columna `Estado`): solo **#8**
(alta, RGPD artículo 9 en R-02) sigue `ABIERTO` — es una decisión reservada al dueño (pregunta #16 de
§6, decimoquinto ciclo consecutivo sin novedad) y no genera ninguna R-XX nueva. **#23** y **#24**
(media/baja) son un bug de interfaz y deuda documental, ya corregidos en código por P-31/P-32 del
programador, pendientes solo de que el auditor los confirme y cierre en su próxima pasada: no son
hallazgos de producto ni de arquitectura, así que tampoco generan ninguna R-XX — le corresponden al
programador (§0.3), y ya están resueltos. Revisado `FEEDBACK.md`: sigue con su única fila plantilla
vacía, nada que convertir al roadmap. Revisado el roadmap completo (`ROADMAP_PRODUCTO.md` y este §1)
contra la visión de producto y el ICP: la Oleada v12 (R-26, R-27) quedó `COMPLETADA` en código hoy
mismo (sesión de programador anterior a esta), así que se abre la **Oleada v13** con **R-28** (aviso
de ausencias repetidas donde el profesor ya mira — pasar lista y Mi horario —, F-19, reutiliza sin
duplicar `rankingAusenciasSinJustificarPanelCentro` de R-11) y **R-29** (el profesor avisa de que
falta un día sin salir de la aplicación, F-20, migración `019_aviso_ausencia_profesor.sql`); spec
completa en `ROADMAP_PRODUCTO.md`, dos filas `PENDIENTE` nuevas en §1 de este documento, y la pregunta
**#18** nueva en §6 (umbral/ventana de R-28, valor conservador ya en la spec, sin bloquear). Ninguna
oleada ha llegado todavía a `DESPLEGADA EN PRODUCCIÓN` (T-25 sigue `BLOQUEADA`, fila 12 de §3), así que
nada se mueve a `ROADMAP_HISTORICO.md` esta vez — mismo criterio que los veintiocho ciclos anteriores.
Sin ningún commit de código: sesión de producto, no de programador. — PM, 2026-09-22

**Sesión anterior (2026-09-22, rutina programada de programador, quinta pasada del día — cola
...
programador editar `auditoriacontinua.md`. Revisada §1 completa: **ninguna fila `PENDIENTE`** —
...
RGPD artículo 9 en R-02) sigue `ABIERTO`, bloqueado en una decisión del dueño (pregunta #16 de §6)
sin ninguna vía de esa sesión, y es el único de severidad alta — no hay ninguna urgencia que tratar
antes de la cola. Revisada §1 completa: **ninguna fila `PENDIENTE`** (R-27 cerró la Oleada v12 la
sesión anterior; todo lo demás está `COMPLETADA` o `BLOQUEADA` por migración, §3) — abrir una R-XX
nueva es trabajo del PM, no de esa sesión, así que, con la columna vertebral terminada, esa sesión
tomó backlog de auditoría (§0.3: "las P-XX se ejecutan cuando la tarea en curso está terminada o
bloqueada"). **P-31** atiende el hallazgo **#23** (media, "controles muertos por migración
bloqueada"): P-30 (sesión anterior a esa) ya cortaba la llamada de red de "Justificar"/"Marcar
salida" mientras `011`/`012` sigan sin aplicar, pero ninguna pantalla consultaba esa misma señal
ANTES de pintar el control — un profesor o `administrator` podía rellenar el formulario entero para
recibir, solo al pulsar, el mensaje de "todavía no disponible". `datos/asistencia.ts` gana
`justificarAusenciaDisponible()`/`marcarSalidaDisponible()` (mismo criterio que
`accionPendienteDeMigracion`, ya existente); `pantallaRegistrosSlot.ts`/`pantallaPasarLista.ts` las
reciben como dependencia OPCIONAL (`deps.xDisponible?.() ?? true`) en vez de importarlas
directamente, para no dejar en rojo permanente los ~15 tests existentes del flujo completo de esos
controles (doble toque, reconciliación de conflicto, deshabilitado en curso): esa lógica sigue
siendo válida y necesaria en cuanto `011`/`012` se apliquen, así que se sigue probando contra los
dobles con el valor por defecto (`true`), y 5 tests nuevos fijan el caso "oculto" con un `() =>
false` explícito — razonado en `DECISIONES_TECNICAS.md`. Wireadas en `aplicacion.ts` a las
funciones reales en los tres puntos de montaje. **P-32** atiende el hallazgo **#24** (baja,
gobernanza documental): añadida la fila que faltaba de `pausa_alumno` (R-21) a la matriz rol × tabla
× operación de `DECISIONES_TECNICAS.md` — sus políticas ya eran correctas, solo faltaba la fila de
referencia. Ninguna migración: ambas son cambios de cliente/documentación. 5 tests nuevos (1816 en
total, antes 1811). `npm run typecheck`, `npm run lint`, `npm test`, `npm run build` verificados en
verde antes del push. Mergeado a `develop` al cierre de esa sesión, sin tocar `master` en ningún
momento.

**Sesión anterior (2026-09-22, rutina programada de programador — R-27 completada, Oleada v12
cerrada):** protocolo §0.3 primero — revisado `auditoriacontinua.md`: el hallazgo #8 (alta, RGPD
artículo 9 en R-02) sigue `ABIERTO`, bloqueado en una decisión del dueño (pregunta #16 de §6) sin
ninguna vía de esa sesión; #23 (media) y #24 (baja) tampoco alcanzaban la severidad que exige
tratamiento urgente antes de la cola (§0.3: solo alta) — ninguno de los tres bloqueó esa sesión.
Revisada §1: única fila `PENDIENTE` de la cola normal, **R-27** (Oleada v12/F-18, alta de una sesión
de grupo completa, spec en `ROADMAP_PRODUCTO.md`), que cerró la Oleada v12. Implementada por
completo, sin migración (`Migración: No` en su spec, confirmado: ninguna tabla ni RPC nueva —
reutiliza `crearSlot` de T-15 tal cual). `ui/comboboxAlumnoExtra.ts` (T-20) gana `mostrarNota?:
boolean` (por defecto `true`, sin cambio para su único consumidor previo,
`pantallaPasarLista.ts`): `false` quita el campo "Motivo (opcional)" del DOM sin tocar el resto del
combobox accesible, para un segundo consumidor que no necesita un motivo por selección.
`ui/pantallaHorarioCentro.ts` (R-25) gana la acción de nivel de página "Nueva sesión de grupo"
(visible incluso con el centro vacío, requisito 1): día/hora/profesor/asignatura una sola vez y el
buscador de T-20 en modo de selección múltiple (`mostrarNota: false`) para elegir varios alumnos ya
existentes, cada uno con su "Quitar"; un alumno ya elegido solo avisa, nunca se duplica (requisito
5). Al confirmar, `crearSlot` una vez por alumno — mismo patrón EXACTO de reintento parcial que
"Editar sesión completa"/"Cesar sesión completa" (R-25): un solape del propio alumno rechaza SOLO su
alta y queda listo para reintentar con los mismos datos, sin reabrir el formulario, mientras el resto
del grupo se crea con normalidad (requisito 4); el aviso de solape con OTRO profesor (no bloqueante)
pasa por el mismo `avisoGlobal` que ya usaba R-25. 10 tests nuevos (1811 en total, antes 1801).
`npm run typecheck`, `npm run lint`, `npm test`, `npm run build` verificados en verde antes del push.
Mergeado a `develop` al cierre de esa sesión, sin tocar `master` en ningún momento. Con R-27
completada, la Oleada v12 (R-26/R-27, F-17/F-18) quedó cerrada por completo — ninguna fila
`PENDIENTE` quedó en §1 salvo lo `BLOQUEADA` por migración (§3), así que la siguiente sesión de
programador tomó backlog de auditoría en su lugar (ver arriba).
...
que ningún programador lo tomara) SÍ era de esta sesión, y se atendió como **P-30 urgente, antes de
la cola normal** (§5): `src/datos/asistencia.ts#actualizarAsistencia` enviaba los 13 parámetros de
la firma ampliada de `011`/`012` (R-02/R-03) a una función que en `dev` solo declara los 8 de `008`
— PostgREST exige coincidencia exacta de nombres, así que la llamada fallaba siempre, incluso para
las acciones de T-21/R-24 (ya `COMPLETADA`s) que no tocan ningún campo nuevo. Implementada la opción
(c) que el propio hallazgo proponía: solo se envían los 8 parámetros reales; pedir "justificar" o
"marcar/ajustar salida" (R-02/R-03) lanza ahora `AccionNoDisponibleTodavia` (nueva, cliente, nunca
del servidor) antes de tocar la red, con mensaje propio en español, en vez de un 404 opaco o un
envío silencioso sin efecto. R-02 y R-03 siguen exactamente igual de `BLOQUEADA` en §1 — nada se ha
desbloqueado, ninguna migración se ha tocado —, lo que se corrige es que T-21/R-24 vuelvan a
funcionar de verdad en `dev`. Verificación pre-push completa en verde (`npm run typecheck`,
`npm run lint`, `npm test`: 1746/1746, `npm run build`). Detalle en `DECISIONES_TECNICAS.md` y en la
fila P-30 de §5. Sin ninguna cola normal atendida esta sesión (R-25, Oleada v11, sigue `PENDIENTE`
en §1 sin tomar): el protocolo pide atender primero el hallazgo `ABIERTO` de severidad alta, y con
la sesión ya en verificación completa se prioriza cerrar aquí en vez de encadenar R-25 en el mismo
turno. Mergeado a `develop` al cierre de esta sesión, sin tocar `master` en ningún momento.

**Sesión anterior (2026-09-20, vigésimo séptimo ciclo del PM: sin R-XX nueva, tercer ciclo
...
`develop` en el remoto es exactamente el mismo commit (`da33224`) con el que cerró la pasada
...
**Sesión anterior (2026-09-18, rutina programada de programador, tercera pasada sin trabajo
accionable tras R-24): protocolo §0.3 primero — `auditoriacontinua.md` sin pasada nueva desde
...
sin ninguna acción posible desde el código). Sin ningún P-XX urgente nuevo. Revisada §1 completa:
**cero `PENDIENTE`/`EN CURSO`** — R-24 (Oleada v10/F-15) sigue `COMPLETADA` desde la pasada anterior,
y ninguna R-25/Oleada v11 nueva en `ROADMAP_PRODUCTO.md` (su último commit sigue siendo `d61898a`, el
que abrió R-24; confirmado con `git log` directo sobre el documento). Revisado `db/APLICADAS.md`:
`009` sigue siendo la última migración aplicada en `dev`, sin cambio (las nueve de las filas 13 a 21
de §3, `010` a `018`, siguen `PENDIENTE`, más la fila 12 de T-25). Revisadas las 17 preguntas de §6:
todas con la columna "Respuesta" vacía salvo las once ya respondidas, incluida **#16** (la única que
desbloquearía trabajo real de la cola normal) y **#17**, sin cambio. Revisada además §5 (P-XX):
ninguna fila `PENDIENTE`. Confirmado además que el HEAD de `develop` en el remoto es exactamente el
mismo commit (`01c3d44`) con el que cerró la pasada anterior — cero commits nuevos de ningún tipo
(PM, auditor, dueño o programador) desde entonces. Con los cuatro indicadores que podrían desbloquear
trabajo —pasada del auditor, respuesta del dueño, migración nueva aplicada o ciclo de PM abriendo
tarea nueva— sin cambio desde la pasada anterior, esta sesión no ejecuta ninguna T-XX, R-XX ni
P-XX — **sin ningún commit de código de producto**. El contenedor arrancó sin `node_modules`:
`npm ci` recuperó los 130 paquetes declarados, 0 vulnerabilidades. Verificación pre-push completa en
verde como comprobación de salud: `npm run typecheck`, `npm run lint`, **`npm test` 1748/1748** (sin
cambio desde R-24) y `npm run build`. Abrir una R-25/Oleada v11 nueva es decisión del ciclo de PM
(rutina de producto), no de esta sesión de programador.

**Sesión anterior (2026-09-18, rutina programada de programador, primera pasada sin trabajo
accionable tras R-24):** protocolo §0.3 primero — `auditoriacontinua.md` sin pasada nueva desde
`55a5e8c` (2026-09-18, la misma que ya revisó y consumió la sesión anterior de R-24, cerrando
**#21**): confirmado con `grep -n "ABIERTO"` sobre la tabla de hallazgos que solo queda **#8**
(`ABIERTO`, dato de salud del artículo 9 del RGPD en R-02, pregunta #16 de §6, esperando al dueño —
sin ninguna acción posible desde el código). Sin ningún P-XX urgente nuevo. Revisada §1 completa:
**cero `PENDIENTE`/`EN CURSO`** — R-24 (Oleada v10/F-15) sigue `COMPLETADA` desde la sesión anterior,
y ninguna R-25/Oleada v11 nueva en `ROADMAP_PRODUCTO.md` (su último commit sigue siendo `d61898a`, el
que abrió R-24; confirmado con `git log` directo sobre el documento). Revisado `db/APLICADAS.md`:
`009` sigue siendo la última migración aplicada en `dev`, sin cambio (las nueve de las filas 13 a 21
de §3, `010` a `018`, siguen `PENDIENTE`, más la fila 12 de T-25). Revisadas las 17 preguntas de §6:
todas con la columna "Respuesta" vacía salvo las once ya respondidas, incluida **#16** (la única que
desbloquearía trabajo real de la cola normal) y **#17**, sin cambio. Revisada además §5 (P-XX):
ninguna fila `PENDIENTE`. Con los cuatro indicadores que podrían desbloquear trabajo —pasada del
auditor, respuesta del dueño, migración nueva aplicada o ciclo de PM abriendo tarea nueva— sin
cambio desde la sesión anterior, esta sesión no ejecuta ninguna T-XX, R-XX ni P-XX — **sin ningún
commit de código de producto**. El contenedor arrancó sin `node_modules`: `npm ci` recuperó los 130
paquetes declarados, 0 vulnerabilidades. Verificación pre-push completa en verde como comprobación
de salud: `npm run typecheck`, `npm run lint`, **`npm test` 1748/1748** (sin cambio desde R-24) y
`npm run build`. Abrir una R-25/Oleada v11 nueva es decisión del ciclo de PM (rutina de producto), no
de esta sesión de programador.

**Sesión anterior (2026-09-18, rutina programada de programador):** protocolo §0.3 primero —
...
siendo 2026-09-16, el vigésimo tercer ciclo del PM, el que abrió R-23; confirmado también con `git log`
...
trabajo —pasada del auditor, respuesta del dueño, migración nueva aplicada o ciclo de PM abriendo
tarea nueva— exactamente iguales que en las tres sesiones anteriores, esta sesión no ejecuta ninguna
...
**Sesión anterior (2026-09-17, rutina programada de programador, tercera pasada sin trabajo
accionable tras R-23): revisado primero `auditoriacontinua.md` (protocolo §0.3) — sin pasada nueva
...
**#21** sigue `ABIERTO` en el registro del auditor por diseño (el programador no edita
`auditoriacontinua.md`), pero ya quedó atendido de facto por **P-29** (cobertura añadida a
`db/pruebas_rls.sql`), pendiente solo de que el auditor lo confirme y lo cierre en su próxima pasada.
Revisada §1 completa: **cero `PENDIENTE`/`EN CURSO`** — R-23 (Oleada v9/F-14) sigue `COMPLETADA`, y
ninguna R-24/Oleada v10 nueva en `ROADMAP_PRODUCTO.md` (el ciclo de PM no ha corrido desde el
vigésimo tercer ciclo, el que abrió R-23; confirmado también con `grep` directo sobre el documento).
Revisado `db/APLICADAS.md`: `009` sigue siendo la última migración aplicada en `dev`, sin cambio (las
nueve de las filas 13 a 21 de §3, `010` a `018`, siguen `PENDIENTE`, más la fila 12 de T-25). Revisadas
las 17 preguntas de §6: todas con la columna "Respuesta" vacía salvo las once ya respondidas, incluida
**#16** (la única que desbloquearía trabajo real de la cola normal) y **#17**, sin cambio. Revisada
además §5 (P-XX): las 29 filas siguen `RESUELTA`/`IMPLEMENTADA`, ninguna `PENDIENTE`. Con los cuatro
indicadores que podrían desbloquear trabajo —pasada del auditor, respuesta del dueño, migración nueva
aplicada o ciclo de PM abriendo tarea nueva— exactamente iguales que en la sesión anterior, esta
sesión no ejecuta ninguna T-XX, R-XX ni P-XX — **sin ningún commit de código de producto**. El
contenedor arrancó sin `node_modules`: `npm ci` recuperó los 130 paquetes declarados, 0
vulnerabilidades. Verificación pre-push completa en verde como comprobación de salud: `npm run
typecheck`, `npm run lint`, **`npm test` 1734/1734** (sin cambio desde R-23) y `npm run build`. Abrir
una R-24/Oleada v10 nueva es decisión del ciclo de PM (rutina de producto), no de esta sesión de
programador.

**Sesión anterior (2026-09-17, rutina programada de programador, primera pasada sin trabajo
accionable tras R-23): revisado primero `auditoriacontinua.md` (protocolo §0.3) — sin pasada nueva
...
(R-22) añadió la tabla `baja_profesor` sin actualizar los tres barridos genéricos "todas las tablas"
de `db/pruebas_rls.sql` (`student` sección 6, `TRUNCATE` sección 8, `anon` sección 8f) — sin fuga
activa (el `GRANT`/RLS real ya es correcto), pero sin la red de regresión que lo demuestre. Por ser
severidad alta, atendido como **P-29 urgente (§0.3)** antes de la cola normal: añadida `'baja_profesor'`
a los tres arrays (líneas 743, 1204 y 1858), registrada primero en §5. **#8** sigue `ABIERTO` sin
cambio (esperando al dueño en la pregunta #16 de §6). Con eso resuelto, siguiente tarea `PENDIENTE`
de §1: **R-23** (cierre de slot en un toque: marcar el resto como presente en bloque, Oleada v9/F-14,
simétrica a R-17). **Implementada completa, sin migración:** en `pantallaPasarLista.ts`, bloque nuevo
"Marcar el resto como presente" (`marcarPresenteEnBloque` en el estado, zona propia
`zonaMarcarPresenteEnBloque`, visualmente distinguible del de R-17 — requisito 1), que reutiliza
`manejarToque` TAL CUAL una vez por alumno pendiente (requisito 3: misma idempotencia, reconciliación
de `Conflicto` y cola offline de R-07 que si se tocara una a una), sobre una foto congelada de
`clavesPendientes()` tomada al abrir la confirmación (requisito 2: lista nominalmente, nunca una
cifra) — mismo patrón exacto que `cierreEnBloque` de R-17 con `manejarAusente`. En
`pantallaRegistrosSlot.ts` (requisito 5, "disponible también desde Registros"), bloque simétrico
`presenteConfirmando`/`presenteGuardando`/`presenteError` sobre el MISMO `estado.cierreCandidatos`
que ya usa R-17 (requisito 4: mismo criterio de exclusión sirve para los dos sentidos), llamando a
`deps.registrarOlvidado` **sin `ocurridoEn`** (registro en vivo con la hora real del servidor en el
instante de su propia llamada, nunca la hora manual que pide "Añadir registro olvidado" — requisito
3), misma RPC exacta, ninguna nueva. Los dos controles conviven sin interferir (requisito 7): cada
uno pinta y opera solo sobre su propia zona/estado, verificado con un test dedicado en cada pantalla
(usar "ausente" sobre unos y "presente" sobre el resto sin conflicto, sin duplicar ni sobrescribir
ningún registro). R-23 pasa a `COMPLETADA`. 15 tests nuevos (1734 en total, antes 1719).
`DEVELOPERS.md` actualizado en las dos entradas de `pantallaPasarLista.ts`/`pantallaRegistrosSlot.ts`.
Verificación pre-push completa en verde: `npm run typecheck`, `npm run lint`, `npm test` (**1734/1734**)
y `npm run build`, tras `npm ci` (130 paquetes, 0 vulnerabilidades).

**Sesión anterior (2026-09-16, rutina programada de programador, cuarta pasada sin trabajo
accionable tras R-22): revisado primero `auditoriacontinua.md` (protocolo §0.3) — sin pasada nueva del
auditor desde `c338c04` (2026-09-16, la misma que ya revisaron las tres sesiones anteriores de hoy):
sigue **ABIERTO** solo **#8** (alta), sin ninguna acción posible desde el código, esperando al dueño
en la pregunta #16 de §6 (columna "Respuesta" verificada vacía por lectura directa de la fila).
Revisada §1 completa: **cero `PENDIENTE`/`EN CURSO`** — ninguna tarea de código a la que pasar, y
ningún R-23/Oleada v9 nueva en `ROADMAP_PRODUCTO.md` (el ciclo de PM no ha corrido desde `efdbfd4`).
Revisado `db/APLICADAS.md`: `009` sigue siendo la última migración aplicada en `dev`, sin cambio (las
nueve de las filas 13 a 21 de §3, `010` a `018`, siguen `PENDIENTE`). Revisada además §5 (P-XX):
ninguna fila `PENDIENTE`. Con los tres indicadores que podrían desbloquear trabajo —pasada del
auditor, respuesta del dueño, migración nueva— exactamente iguales que en las tres sesiones
anteriores, esta sesión no ejecuta ninguna T-XX, R-XX ni P-XX — **sin ningún commit de código**. El
contenedor arrancó sin `node_modules`: `npm ci` recuperó los 130 paquetes declarados, 0
vulnerabilidades. Verificación pre-push completa en verde como comprobación de salud: `npm run
...
**Sesión anterior (2026-09-16, rutina programada de programador, tercera pasada sin trabajo
accionable tras R-22): revisado primero `auditoriacontinua.md` (protocolo §0.3) — sin pasada nueva del
auditor desde `c338c04` (2026-09-16, la misma que ya revisaron las dos sesiones anteriores de hoy):
sigue **ABIERTO** solo **#8** (alta), sin ninguna acción posible desde el código, esperando al dueño
en la pregunta #16 de §6 (columna "Respuesta" verificada vacía por lectura directa de la fila).
Revisada §1 completa: **cero `PENDIENTE`/`EN CURSO`** — ninguna tarea de código a la que pasar, y
ningún R-23/Oleada v9 nueva en `ROADMAP_PRODUCTO.md` (el ciclo de PM no ha corrido desde `efdbfd4`).
...
nueve de las filas 13 a 21 de §3, `010` a `018`, siguen `PENDIENTE`). Revisada además §5 (P-XX):
ninguna fila `PENDIENTE`. Con los tres indicadores que podrían desbloquear trabajo —pasada del
auditor, respuesta del dueño, migración nueva— exactamente iguales que en las dos sesiones anteriores,
esta sesión no ejecuta ninguna T-XX, R-XX ni P-XX — **sin ningún commit de código**. El contenedor
arrancó sin `node_modules`: `npm ci` recuperó los 130 paquetes declarados, 0 vulnerabilidades.
Verificación pre-push completa en verde como comprobación de salud: `npm run typecheck`, `npm run
lint`, **`npm test` 1719/1719** (sin cambio) y `npm run build`. Abrir una R-XX nueva (Oleada v9) es
decisión del ciclo de PM (rutina de producto), no de esta sesión de programador.

**Sesión anterior (2026-09-16, rutina programada de programador, segunda pasada sin trabajo
accionable tras R-22): revisado primero `auditoriacontinua.md` (protocolo §0.3) — sin pasada nueva del
...
§6. Revisada §1 completa: con R-22 recién implementada y `BLOQUEADA` solo por migración, **cero
`PENDIENTE`/`EN CURSO`** — ninguna tarea de código a la que pasar. Revisadas las 17 preguntas de §6:
todas con la columna "Respuesta" vacía de `#7` a `#17`, incluida **#16** (la única que desbloquearía
R-02/T-25). Revisado `db/APLICADAS.md`: `009` sigue siendo la última migración aplicada en `dev`, sin
cambio (las nueve de las filas 13 a 21 de §3, `010` a `018`, siguen `PENDIENTE`). Revisada además §5
(P-XX): ninguna fila `PENDIENTE`. Con los tres indicadores que podrían desbloquear trabajo —pasada del
auditor, respuesta del dueño, migración nueva— exactamente iguales que en la sesión anterior (la que
implementó R-22), esta sesión no ejecuta ninguna T-XX, R-XX ni P-XX — **sin ningún commit de código**.
El contenedor arrancó sin `node_modules`: `npm ci` recuperó los 130 paquetes declarados, 0
vulnerabilidades. Verificación pre-push completa en verde como comprobación de salud: `npm run
typecheck`, `npm run lint`, **`npm test` 1719/1719** (sin cambio) y `npm run build`. Abrir una R-XX
nueva (Oleada v9) es decisión del ciclo de PM (rutina de producto), no de esta sesión de programador.

**Sesión anterior (2026-09-16, rutina programada de programador): R-22 implementada** (Oleada v8, cuarta combinación de la
...
(2026-09-14/15) — solo quedan **#8** (`ABIERTO`, alta, esperando al dueño en la pregunta #16 de §6,
sin ninguna acción posible desde el código) y **#20** (`ABIERTO`, media, ya implementada por P-28,
pendiente solo de confirmación del dueño/reevaluación del auditor); ninguno bloquea la columna
vertebral ni exige una P-XX urgente. Con eso, se tomó la siguiente tarea `PENDIENTE` de §1: **R-21**
(spec completa en `ROADMAP_PRODUCTO.md`), abierta el día anterior por el ciclo de PM. Código y tests
completos, contra dobles: migración `017_pausa_alumno.sql` (tabla `pausa_alumno` + RPC
`declarar_pausa_alumno`/`cancelar_pausa_alumno`/`acortar_pausa_alumno`, `SECURITY DEFINER`, mismo
patrón de opacidad que `excepcion_slot` de R-06 — ver `DECISIONES_TECNICAS.md`), `dominio/pausaAlumno.ts`
(nuevo, 30 tests), `datos/pausasAlumno.ts` (nuevo, 10 tests); `dominio/informeMensualAlumno.ts`
(`sesionesEsperadasDelMes` gana `alumnoId`/`pausas`, tercer criterio de exclusión junto a
`esDiaCerrado`/`esDiaCanceladoParaSlot`) y `dominio/panelCentro.ts`
(`rankingAusenciasSinJustificarPanelCentro` gana `pausas`/`zonaHoraria`, opcionales, sin romper
ninguna llamada existente) reutilizan el mismo criterio sin tocar su forma pública más de lo
necesario. UI: `pantallaPasarLista.ts` (T-19, requisito 2: alumno en pausa excluido de pendientes,
listado aparte "En pausa hoy", requisito 6), `pantallaMiHorario.ts` (T-22: relabela la fila de hoy de
un alumno en pausa, mismo criterio que la excepción de slot), `pantallaFichaAlumno.ts` (T-12: bloque
sexto nuevo, "Pausa programada" — listar, declarar, cancelar una futura, acortar una en curso),
`pantallaHistorico.ts`/`pantallaPanelCentro.ts` (pasan `pausas` a R-04/R-11). Nueva
`permisosUi.ts#puedeGestionarPausasAlumno` (exclusiva de `administrator`). 61 tests nuevos (1678 en
total, antes 1617). Migración escrita y empujada, **todavía sin aplicar**: R-21 pasa a `BLOQUEADA —
pendiente aplicar migración 017` en §1 (fila 20 de §3 nueva). Verificación pre-push completa en
verde: `npm run typecheck`, `npm run lint`, `npm test` (1678/1678) y `npm run build`. `db/pruebas_rls.sql`
gana la sección 8n (17 comprobaciones) y `pausa_alumno` se añade a los tres barridos obligatorios
(`student` sección 6, `TRUNCATE` sección 5, `anon` sección 8f) — sin poder ejecutarse en esta sesión
(§0.1: el agente nunca tiene credenciales de `dev`), queda para la primera pasada de
`npm run probar-rls` del dueño tras aplicar la migración.

**Sesión anterior (2026-09-14, rutina programada de producto, vigésimo primer ciclo del PM:
...
pregunta #16 de §6); **#20** sigue `ABIERTO` de severidad media, pero ya **implementada** por P-28 en
la sesión anterior — pendiente solo de que el dueño la confirme con su próximo `npm run probar-rls` y
de que el auditor la reevalúe en su próxima pasada, nada nuevo que hacer sobre ella desde el código.
Revisada §1 completa: **cero `PENDIENTE`/`EN CURSO`**, ninguna tarea de código a la que pasar.
Revisadas las 17 preguntas de §6: todas con la columna "Respuesta" vacía, incluida **#16** (la única
que desbloquearía R-02/T-25). Revisado `db/APLICADAS.md`: `009` sigue siendo la última migración
aplicada en `dev`, sin cambio. Revisada además §5 (P-XX): ninguna fila `PENDIENTE`. Con los tres
...
**Sesión anterior (2026-09-14, rutina programada de programador, P-28 implementada): revisado
primero `auditoriacontinua.md` (protocolo §0.3) — sin pasada nueva del auditor desde `7477c26`
(2026-09-14, la misma que ya revisaron las dos sesiones anteriores de hoy): **#8** sigue `ABIERTO` de
severidad alta, sin ninguna acción posible (esperando al dueño en la pregunta #16 de §6, fuera del
alcance de esta sesión); **#20** sigue `ABIERTO` de severidad media, sin activar el régimen de
urgencia de §0.3, pero es un hueco de cobertura concreto, pequeño y sin dependencia de migración ni
de decisión del dueño — exactamente el perfil de una P-XX de backlog (§0.3: "por defecto, las P-XX se
ejecutan cuando la tarea en curso está terminada o bloqueada", y §1 sigue con cero `PENDIENTE`/`EN
CURSO`). Revisadas también las 17 preguntas de §6 (todas con "Respuesta" vacía, incluida **#16**) y
`db/APLICADAS.md` (`009` sigue siendo la última migración aplicada en `dev`): sin cambio en ninguno de
los dos, igual que las dos sesiones anteriores de hoy. **Registrada e implementada P-28** (§5): nueva
sección **7c** en `db/pruebas_rls.sql`, que añade al barrido obligatorio de `student` (T-10 requisito
5) los tres casos que le faltaban contra el bucket `avatares`/`storage.objects` — no lee el avatar del
alumno activo, no lee el del alumno inactivo (reutilizando los fixtures de la sección 7, con
comprobación previa de que la fila sigue existiendo, para que "0 filas" no sea un falso positivo por
falta de dato) y no puede escribir en el bucket — mismo patrón que P-06 ya usó para `anon`. Sin
migración: solo añade comprobaciones a un script de prueba que el agente no ejecuta contra ninguna
base real (§0.1); pendiente de que el dueño lo confirme con su próximo `npm run probar-rls` y de que
el auditor reevalúe el hallazgo #20 en su próxima pasada. El contenedor arrancó sin `node_modules`:
`npm ci` recuperó los 130 paquetes, 0 vulnerabilidades. Verificación pre-push completa en verde:
`npm run typecheck`, `npm run lint`, **`npm test` 1617/1617** (sin cambio: el fichero tocado es SQL,
fuera de la suite de `node:test`) y `npm run build`.

**Sesión anterior (2026-09-14 08:13, rutina programada de programador, sin trabajo accionable tras
R-20): revisado primero `auditoriacontinua.md` (protocolo §0.3) — sin pasada nueva del auditor desde
`7477c26` (2026-09-14), la misma que ya revisó y agotó la sesión anterior (la que completó R-20):
**#8** sigue `ABIERTO` de severidad alta, sin ninguna acción posible (esperando al dueño en la
...
producto). `auditoriacontinua.md` trajo una pasada nueva desde el ciclo anterior (`e79de3c`,
2026-09-12) que confirma sin cambio el único hallazgo que sigue `ABIERTO` (**#8**, dato de salud del
artículo 9 en `motivo_justificacion` de R-02, esperando al dueño en la pregunta #16 de §6) y no abre
ninguno nuevo. `FEEDBACK.md` sigue con su única fila plantilla vacía, nada que convertir. **R-20**
(Oleada v6/F-11, abierta el ciclo anterior) seguía `PENDIENTE` en §1: ninguna sesión de programador
la había tomado todavía, así que la cola de trabajo no estaba vacía y no había base para abrir una
Oleada v7 ese ciclo. Sin cambios de estado de T-XX/R-XX en §1; ninguna R-XX nueva ese ciclo.
...
nuevo — **sin hallazgos nuevos esta pasada**. Solo **#8** sigue `ABIERTO` de verdad, esperando al
dueño en la pregunta #16 de §6, sin novedad. Sin ningún hallazgo `ABIERTO` de severidad alta nuevo que
atender como P-XX urgente, se revisó §1: con **R-18** ya `COMPLETADA` la siguiente tarea `PENDIENTE`
que no depende de nada sin terminar era **R-19** ("Informe de horas propias para el profesor",
Oleada v5/F-10, spec en `ROADMAP_PRODUCTO.md`, `Migración: No`, depende de R-15, `COMPLETADA`).

**R-19 completada.** La spec (requisito 5) pedía reutilizar `dominio/informeHorasProfesor.ts` (R-15)
tal cual, sin duplicar ningún cálculo — así se hizo: `ui/pantallaMisHorasProfesor.ts` (nuevo) llama a
`informeHorasProfesor` con un array de un ÚNICO profesor (`deps.profesorId`/`deps.profesorNombre`,
el propio, nunca resuelto contra un listado del centro), así que no hay ningún camino de código por
el que el profesor pueda ver la cifra de otro (requisito 2: "sin selector de otro profesor, sin
ranking"). Misma fila SIEMPRE presente (en ceros si no hay ninguna sesión, mismo criterio de R-15),
sin el mensaje de "ningún profesor en este rango" de la pantalla de `administrator` — aquí la fila
propia nunca falta. Mismos botones "Descargar CSV"/"Imprimir / PDF" sobre la misma fuente de filas
que la tabla (criterio de aceptación: "el CSV exportado coincide con lo mostrado en pantalla"). Nueva
`dominio/permisosUi.ts#puedeVerInformeHorasPropio` (exclusiva de `teacher`); mismo criterio de
inaccesibilidad estructural ya aceptado para R-15/R-10 (`DECISIONES_TECNICAS.md`): la pantalla solo
se monta dentro del router de `teacher` (`crearRouterProfesor`, nueva ruta `#/mis-horas` en
`nucleo/router.ts`), así que un `administrator` no llega a ella por ningún camino — `administrator`
sigue con el informe completo de R-15 sin ningún cambio (requisito 6). Botón "Mis horas" nuevo en la
barra de navegación de `teacher` (`ui/aplicacion.ts`). **14 tests nuevos en total** (1586 en total,
antes 1572): 11 de `pantallaMisHorasProfesor.ts` (acceso denegado a `administrator`/`student`,
"Cargando…", fila propia en ceros sin ninguna sesión, sesiones propias y horas teóricas, una
sustitución contada aparte de lo propio, sin ningún `<select>` de profesor, error amigable, CSV,
impresión, cambio de rango), 1 de `puedeVerInformeHorasPropio`, 1 de `analizarRutaProfesor`/
`hashDeRutaProfesor` (`#/mis-horas`) y 1 de `aplicacion.test.ts` (botón "Mis horas" navega y no
pinta ningún selector). Sin migración: `db/APLICADAS.md` sin cambio. Sin ningún dato ni tabla nuevos
(requisito 6). `FEEDBACK.md` sigue con su única fila plantilla vacía: nada que convertir.

Verificación pre-push completa en verde (tras `npm install`, contenedor sin `node_modules`
preinstalados — 130 dependencias, cero vulnerabilidades): `npm run typecheck`, `npm run lint`, `npm
test` **1586/1586**, `npm run build`.

**Sesión anterior (2026-09-10, rutina programada de producto, "decimoséptimo ciclo del PM —
...
(**#7 a #17**) siguen con la columna "Respuesta" vacía, incluida **#16**; **(c)** `db/APLICADAS.md`
sigue con `009` como última migración aplicada en `dev`, `010` a `016` sin cambio. Los tres
indicadores idénticos a la sesión anterior confirman que el estado no se ha movido: sin pasada nueva
del auditor no hay hallazgo `ABIERTO` nuevo que atender como P-XX urgente (§0.3), sin respuesta nueva
del dueño no se desbloquea ninguna T-XX/R-XX `BLOQUEADA`, y sin migración nueva aplicada tampoco. Esta
sesión no ejecutó ninguna T-XX, R-XX ni P-XX — **no hay ningún commit de código**. Se instaló
`node_modules` (el contenedor de esta sesión arrancó sin dependencias instaladas, `npm install`
recuperó las 130 declaradas en `package.json`, cero vulnerabilidades) y se ejecutó igualmente la
verificación pre-push completa como comprobación de salud: `npm run typecheck`, `npm run lint`,
**`npm test` 1572/1572** (sin cambio), `npm run build`, los cuatro en verde — sin ninguna regresión
silenciosa. Próxima sesión: repetir estos mismos tres chequeos antes de volver a recorrer §1/§5
enteros; si los tres siguen iguales, no hace falta repetir esta nota con el mismo detalle.

**Sesión anterior (2026-09-10, rutina programada de programador, "nada accionable: los tres frentes
— auditor, backlog P-XX y cola T-XX/R-XX — están agotados a la vez por primera vez"):** revisado
primero el registro de hallazgos de `auditoriacontinua.md` (protocolo §0.3): sin pasada nueva del
auditor desde la de esta misma tarde (commit `8f775de`, 2026-09-10, ya vista por las dos sesiones
anteriores), confirmado con `git log -- auditoriacontinua.md`. De los cuatro hallazgos `ABIERTO` en
el registro, los tres que tenían dirección de arreglo backlog (**#15**/P-25, **#17**/P-26,
**#19**/hueco de §7) ya quedaron implementados por la sesión inmediatamente anterior ("P-25/P-26...
columna vertebral agotada") y el de severidad alta ya corregido por P-27 en la sesión previa a esa
(**#18**) — los cuatro pendientes solo de que el auditor los reevalúe en su próxima pasada, que esa
sesión no pudo forzar. Solo quedaba **#8** `ABIERTO` de verdad, esperando al dueño en la pregunta #16
de §6, sin novedad. Revisado §1 completo (T-00 a R-18, 44 filas): **36 `COMPLETADA`, 8 `BLOQUEADA`,
cero `PENDIENTE`/`EN CURSO`** — confirmado con una pasada íntegra de la tabla. Revisada además §5
(registro de P-XX) de punta a punta: **las 27 filas `RESUELTA`/`IMPLEMENTADA`, ninguna `PENDIENTE`**
— el backlog del auditor que sostuvo las dos sesiones anteriores también se agotó. Revisadas las
respuestas del dueño en §6: sin cambio en ninguna de las once preguntas sin respuesta (**#7 a #17**,
incluida **#16**, la única que desbloquearía trabajo nuevo real). Revisado `db/APLICADAS.md`: `009`
seguía siendo la última migración aplicada en `dev`, `010` a `016` en "Pendiente de aplicar". Con los
tres frentes agotados a la vez, y sin ninguna vía autónoma nueva que abrir sin inventar trabajo
(§0.2), esa sesión no ejecutó ninguna T-XX, R-XX ni P-XX — sin ningún commit de código. Verificación
pre-push completa en verde como comprobación de salud: `npm run typecheck`, `npm run lint`, `npm
test` 1572/1572 (sin cambio), `npm run build`.
...
en la pregunta #16 de §6, sin novedad; **#19** es de gobernanza documental, baja. Sin ningún hallazgo
`ABIERTO` de severidad alta nuevo que atender como P-XX urgente, se revisó §1: con **R-18** ya
`COMPLETADA` (sesión anterior) no queda ninguna T-XX/R-XX `PENDIENTE`/`EN CURSO` sin depender de una
migración sin aplicar o de una decisión del dueño — el resto de `BLOQUEADA` lo está solo por una fila
de §3 o por la pregunta #16 de §6 (T-25, R-01, R-02, R-03, R-06, R-08, R-12, R-14). Con la columna
vertebral agotada (mismo precedente que la sesión de "P-23/P-24 urgentes"), esta sesión aplicó §0.3 y
atendió backlog no urgente del propio auditor: los dos hallazgos `ABIERTO` que ya traían dirección de
arreglo señalada por él mismo como "evidente y de bajo riesgo" (**P-25**, severidad media, hallazgo
#15; **P-26**, severidad baja, hallazgo #17), más el hueco de gobernanza documental del hallazgo #19
(fila de §7 que le faltaba a R-16, ver más abajo).

- **P-25** (hallazgo #15 de auditoría, severidad media): la importación masiva de alumnos (R-08) no
  era idempotente ante un reintento — `importarAlumnosValidados` generaba un `id` nuevo con
  `crypto.randomUUID()` en CADA invocación, así que un corte de red justo después de que el `INSERT`
  en lote llegara al servidor pero antes de que la respuesta llegara al navegador dejaba la pantalla
  lista para reenviar el mismo lote con 48 `id` nuevos, duplicando el alta entera. **Arreglado**
  moviendo la generación del `id` al punto donde el lote se analiza (`ui/pantallaImportacionMasiva.ts`,
  nuevo `deps.generarId` inyectable, wireado a `crypto.randomUUID()` solo en `aplicacion.ts` — mismo
  criterio que `generarPeticionId` de T-18/R-07), guardado en el estado del bloque (`idsPorFila`) y
  reutilizado tal cual en cualquier reintento; `datos/importacionMasiva.ts#importarAlumnosValidados` ya
  no genera ningún `id`, los recibe ya puestos (`FilaAlumnoParaConfirmar`). Un reintento tras un
  alta ya escrita ahora choca con la clave primaria de `alumno` (sin ninguna migración nueva: ya era
  única) y el servidor responde `409`/`Conflicto` en vez de crear una fila duplicada. 3 tests nuevos.
- **P-26** (hallazgo #17 de auditoría, severidad baja): dos defectos menores de `sw.js`. **Arreglados**
  los dos exactamente como el propio hallazgo señalaba: `RESPUESTA_SIN_RED_NI_CACHE` pasa de constante
  de módulo a función `respuestaSinRedNiCache()` (una `Response` es de un solo uso; una constante
  compartida fallaría si el `fallback` hiciera falta una segunda vez en la vida del worker), y la
  comprobación de mismo origen del manejador de `fetch` pasa de comparar por prefijo de cadena
  (`peticion.url.startsWith(...)`) a comparar el `origin` real (`new URL(peticion.url).origin === ...`).
  `URL` añadido a los globales de `sw.js` en `eslint.config.js`. Sin test propio, mismo motivo que el
  resto de `sw.js` (JavaScript plano, sin `tsc`).
- Hueco de §7 (hallazgo #19, severidad baja, gobernanza documental): R-16 resuelve su criterio de
  aceptación ("un `teacher` recibe `SinPermiso` al intentarlo") por el mismo mecanismo de
  inaccesibilidad estructural que R-15, sin haber ganado la fila de §7 que ese mismo patrón motivó
  añadir para R-15/R-10. Añadida ahora la fila que falta.

Verificación pre-push completa en verde: `npm run typecheck`, `npm run lint`, `npm test`
**1572/1572** (antes 1570, +2 de P-25; P-26 no añade tests, `sw.js` no pasa por `tsc`/tests), `npm run
build`. Ninguna de las dos toca el esquema: `db/APLICADAS.md` sin cambio. Ambas registradas en §5
(`P-25`, `P-26`), pendientes de que el auditor reevalúe y cierre los hallazgos #15, #17 y #19 en su
próxima pasada — el programador no edita `auditoriacontinua.md`.

**Sesión anterior (2026-09-10, rutina programada de programador, "R-18 completada"):** revisado
primero el registro de hallazgos de `auditoriacontinua.md` (protocolo §0.3): sin pasada nueva del
...
R-04/R-11/R-13/R-15—, T-19 y T-21, las dos últimas `COMPLETADA`).

**R-17 completada.** En este modelo de datos `slot_horario` es siempre POR ALUMNO: "un slot con ocho
alumnos" de la spec son en realidad ocho filas que comparten profesor, día de la semana, horario y
`asignatura_o_grupo` — nunca una sola fila con una lista de alumnos. Nueva función pura
`dominio/asistencia.ts#slotsDeLaMismaSesion(referencia, slots, fecha)` (9 tests) resuelve ese grupo
para T-21 (Registros, que elige un slot cualquiera de cualquier fecha); en pasar lista (T-19) no hizo
falta ninguna función nueva, porque `alumnosPropuestos` (T-17) ya agrupa por instante exacto, un
criterio más estricto que ya usa toda la pantalla. Nueva `datos/asistencia.ts#listarRegistrosDeSlotsYFecha`
(3 tests): registros de VARIOS slots en una sola petición `in.(...)`, nunca una por alumno.

- **`pantallaPasarLista.ts` (T-19):** botón "Marcar el resto como ausente", ofrecido solo con el slot
  EN CURSO y al menos una card `'pendiente'` — nunca activo por defecto ni disparado por el tick del
  programador. La confirmación congela la lista de pendientes al abrirse y los lista NOMINALMENTE
  (requisito 2); al confirmar, reutiliza `manejarAusente` TAL CUAL una vez por alumno (nunca una
  función de orquestación paralela): cada card acaba en el mismo estado que si el profesor la hubiera
  tocado una a una, con la misma reconciliación de `Conflicto`, la misma cola offline de R-07 si está
  inyectada, y el mismo `'error'` reintentable card a card si falla — sin ningún resumen aparte,
  porque la propia rejilla ya dice "cuáles no se pudieron marcar" (requisito 3). Una card `'error'`
  (un intento de PRESENCIA fallido) NUNCA entra en el cierre en bloque, para no convertir en silencio
  un "vino" en un "faltó" sin que el profesor lo decida para ese alumno en concreto (ver
  `DECISIONES_TECNICAS.md`). 8 tests nuevos.
- **`pantallaRegistrosSlot.ts` (T-21):** mismo control sobre la sesión del slot elegido y la fecha
  elegida (requisito 5, "el mismo criterio que R-01, que ya se ofrece en los dos sitios") —
  `cierreCandidatos` se recalcula en cada `cargarRegistros()` contra la verdad del servidor (una
  petición extra solo si hay compañeros de sesión, nunca si el slot va solo). Al confirmar, el bucle
  vive en la propia pantalla: cada éxito se retira de la lista en el sitio (mismo patrón local que
  `reemplazarRegistro`, sin volver a pedir nada al servidor) y cada fallo queda con su motivo, con la
  confirmación reabierta solo sobre quien de verdad sigue pendiente. 8 tests nuevos.

26 tests nuevos en total (1548 en total, antes 1522). Verificación pre-push completa en verde:
`npm run typecheck`, `npm run lint`, `npm test` **1548/1548**, `npm run build`. Sin migración: no
toca ninguna tabla ni política, solo RPC ya existentes (`registrar_ausencia`, R-01).

**Sesión anterior (2026-09-09, rutina programada de programador, "P-23/P-24 urgentes: backlog del
auditor"):** revisado primero el
registro de hallazgos de `auditoriacontinua.md` (protocolo §0.3): sin pasada nueva del auditor desde
la de esta mañana (commit `c91f4c0`, mismo commit que vio la sesión anterior), confirmado con `git log`.
De sus tres `ABIERTO` de severidad alta, **#8** sigue esperando al dueño en la pregunta #16 de §6, sin
novedad; **#12** y **#13** siguen `ABIERTO` en la tabla del auditor pero ya resueltos de facto por
**P-20**/**P-21**, pendientes solo de que el auditor los confirme en su próxima pasada — no le
corresponde a esta sesión adelantárselo. Sin ningún hallazgo `ABIERTO` de severidad alta nuevo que
atender como P-XX urgente, se revisó §1: con **R-16** ya `COMPLETADA` (sesión anterior) no queda
ninguna T-XX/R-XX `PENDIENTE`/`EN CURSO` sin depender de una migración sin aplicar — el resto de
`BLOQUEADA` lo está solo por una fila de §3 a la espera del dueño (T-25, R-01, R-02, R-03, R-06, R-08,
R-12, R-14). Con la columna vertebral agotada, esta sesión aplicó §0.3 ("las P-XX se ejecutan cuando
la tarea en curso está terminada o bloqueada") y atendió backlog no urgente del propio auditor: dos
hallazgos `ABIERTO` de severidad media/baja con dirección de arreglo ya señalada por él mismo como
"evidente y de bajo riesgo".

- **P-23** (hallazgo #14 de auditoría, severidad media): `nucleo/registroServiceWorker.ts` recargaba
  la página en la PRIMERA instalación del Service Worker, sin ninguna actualización real de por
  medio — el manejador de `controllerchange` llamaba a `alRecargar()` en su primer disparo sin
  comprobar si ya había un `controller` antes, a diferencia del de `updatefound`, que sí distinguía
  ese caso. Cualquier persona abriendo la aplicación por primera vez en un dispositivo nuevo sufría
  una recarga completa mientras probablemente seguía escribiendo su email/contraseña en el login,
  perdiendo lo escrito sin ningún aviso. **Arreglado** con el mismo guard que ya usaba
  `updatefound`: se captura si YA había un `controller` antes de registrar, y solo se trata como
  actualización real (y se llama a `alRecargar`) si lo había, o si no es el primer disparo. El
  propio test que fijaba el defecto como comportamiento esperado
  (`registroServiceWorker.test.ts`, "controllerchange llama a alRecargar una sola vez aunque se
  dispare varias", sin `controller` previo) se sustituyó por dos casos que sí distinguen: sin
  `controller` previo nunca recarga; con `controller` previo recarga una sola vez aunque se dispare
  varias veces.
- **P-24** (hallazgo #16 de auditoría, severidad baja): el generador de CSV propio (`nucleo/csv.ts`,
  único punto de exportación del proyecto) no neutralizaba ninguna celda que empezara por `=`, `+`,
  `-` o `@` (inyección de fórmula CSV) — un campo importado sin validar más allá de "no vacío" (p.
  ej. un nombre de alumno) que empezara por uno de ellos se reexportaba literal en el histórico de
  asistencia (T-23) o el expediente RGPD (R-10), y se ejecutaría como fórmula viva si esa
  exportación se reabre en una hoja de cálculo con fórmulas activadas. **Arreglado** anteponiendo un
  apóstrofo a cualquier campo con uno de esos cuatro prefijos, dentro de `escaparCampo` — el único
  punto por el que pasan `filaCsv`/`documentoCsv`/`documentoCsvConMetadatos`, sin excepción — mismo
  mecanismo de defensa que documenta OWASP para esta clase de vulnerabilidad. 5 tests nuevos en
  `csv.test.ts`.

Verificación pre-push completa en verde: `npm run typecheck`, `npm run lint`, `npm test`
**1522/1522** (antes 1517, +5), `npm run build`. Ninguna de las dos toca el esquema:
`db/APLICADAS.md` sin cambio. Ambas registradas en §5 (`P-23`, `P-24`), pendientes de que el auditor
reevalúe y cierre los hallazgos #14 y #16 en su próxima pasada — el programador no edita
`auditoriacontinua.md`. Quedan en backlog, sin atender esta sesión: el hallazgo #15 (idempotencia de
la importación masiva de R-08, severidad media) y el #17 (dos defectos menores de `sw.js`, severidad
baja).

**Sesión anterior (2026-09-09, rutina programada de programador, "R-16 completada"):** revisado
primero el registro de hallazgos de `auditoriacontinua.md` (protocolo §0.3): sin pasada nueva del
auditor desde la de esta mañana (commit `c91f4c0`), confirmado con `git log` (ningún commit de
`auditoriacontinua.md` desde `698b150`, el commit de R-15). De sus tres `ABIERTO` de severidad alta,
**#8** sigue esperando al dueño en la pregunta #16 de §6, sin novedad; **#12** y **#13** siguen
`ABIERTO` en la tabla del auditor pero ya resueltos de facto por **P-20**/**P-21** de tres sesiones
atrás, pendientes solo de que el auditor los confirme en su próxima pasada — no le corresponde a esta
sesión adelantárselo. Sin ningún hallazgo `ABIERTO` de severidad alta nuevo que atender como P-XX
urgente, se revisó §1: con R-15 ya `COMPLETADA` (sesión anterior), la siguiente tarea `PENDIENTE` que
no depende de nada sin terminar era **R-16** ("Exportación completa del centro", oleada v3/F-07, spec
en `ROADMAP_PRODUCTO.md`, `Migración: No`, depende de T-11/T-12/T-13/T-15/T-23, las cinco
`COMPLETADA`).
...
| T-10 | Autorización: políticas RLS de los tres roles | COMPLETADA | 2026-09-14 | Migración `003_politicas_rls` (renumerada de `002`: P-01 se intercaló antes, ver §7) aplicada y **verificada en ejecución** contra `dev` (fila 5 de §3, `npm run probar-rls`: 51 comprobaciones, 3 omitidas —bucket de avatares, entonces vacío—, 0 fallidas). Código y tests completos; matriz en `DECISIONES_TECNICAS.md`. Estado corregido de `BLOQUEADA` a `COMPLETADA` en la sesión del 2026-08-31: el motivo del bloqueo llevaba ya resuelto desde la primera sesión de ese día, sin que ninguna sesión posterior lo hubiera anotado aquí (bookkeeping, ver `DECISIONES_TECNICAS.md`). **P-28 implementada 2026-09-14** (hallazgo #20 de auditoría): el barrido obligatorio de `student` de `db/pruebas_rls.sql` (requisito 5 de esta tarea, "debe fallar en todas las tablas y en el bucket") ganó su sección 7c, que hasta hoy faltaba — `student` nunca tenía un caso negativo ejecutable contra `storage.objects` |
...
| R-02 | Justificación de una ausencia | BLOQUEADA — pendiente aplicar migración `011` (fila 14 de §3) **y pendiente decisión del dueño** (pregunta #16 de §6, hallazgo #8 de auditoría, severidad alta) | 2026-09-05 | Oleada v1 / F-01 · Código y tests completos, contra dobles. Migración `011_justificacion_ausencia.sql` (renumerada por el PM el 2026-09-02: `007` lo ocupó ya T-20) escrita y empujada, todavía sin aplicar — **no aplicar hasta resolver la pregunta #16**: `motivo_justificacion` incluye valores de dato de salud (artículo 9 RGPD) sin autorización expresa del dueño. **P-30 (2026-09-21, hallazgo #22):** su botón "Guardar justificación" en «Registros» ya estaba montado pero rompía la llamada COMPARTIDA de `actualizar_asistencia` contra `dev` para T-21 también — ahora lanza `AccionNoDisponibleTodavia` de forma explícita en vez de fallar en cascada; sigue exactamente igual de `BLOQUEADA`, nada se ha desbloqueado |
| R-03 | Registro de salida y cómputo de horas reales | BLOQUEADA — pendiente aplicar migración `012` (fila 15 de §3) | 2026-09-04 | Oleada v1 / F-01 · Código y tests completos, contra dobles. Migración `012_registro_salida.sql` (renumerada por el PM el 2026-09-02: `008` lo ocupó ya T-21) escrita y empujada, todavía sin aplicar. **P-30 (2026-09-21, hallazgo #22):** mismo caso que R-02 — "Marcar/ajustar salida" ahora lanza `AccionNoDisponibleTodavia` explícito en vez de romper la llamada compartida; sigue exactamente igual de `BLOQUEADA` |
...
| P-23 | **Backlog del auditor (columna vertebral agotada, §0.3): `registroServiceWorker.ts` recargaba la página en la PRIMERA instalación del Service Worker, sin ninguna actualización real de por medio.** El manejador de `controllerchange` llamaba a `alRecargar()` en su primer disparo sin comprobar si ya existía un `controller` antes de registrar — a diferencia del de `updatefound`, que sí distinguía ese caso ("la primera instalación de todas... no es una actualización, es el arranque"). Cualquier persona abriendo la aplicación por primera vez en un dispositivo nuevo sufría, uno o dos segundos después de cargar, una recarga completa de la página mientras probablemente todavía estaba escribiendo su email/contraseña en el login — perdía lo escrito, sin ningún mensaje que lo explicara. El propio test que existía documentaba el defecto como si fuera el comportamiento esperado. **Implementado:** el mismo guard que ya tenía `updatefound` — se captura si YA había un `controller` antes de registrar (`huboControllerAlRegistrar`), y solo se trata como actualización real (llamando a `alRecargar`) si lo había, o si no es el primer disparo del evento | origen: hallazgo #14 de `auditoriacontinua.md` (severidad media, `ABIERTO` desde 2026-09-09, "aplicación instalable (R-09)") — el propio hallazgo señalaba esta misma dirección de arreglo como "evidente y de bajo riesgo" | **IMPLEMENTADA 2026-09-09** — el test que fijaba el defecto (`registroServiceWorker.test.ts`, "controllerchange llama a alRecargar una sola vez aunque se dispare varias") se sustituyó por dos casos que sí distinguen: sin `controller` previo nunca recarga (un único disparo, el que produce la toma de control inicial en un navegador real); con `controller` previo recarga una sola vez aunque se dispare varias veces. Verificación pre-push completa en verde (`npm run typecheck`, `npm run lint`, `npm test` 1522/1522, `npm run build`). Sin migración. Pendiente de que el auditor reevalúe y cierre el hallazgo #14 en su próxima pasada | — |
| P-24 | **Backlog del auditor (columna vertebral agotada, §0.3): el generador de CSV propio (`nucleo/csv.ts`) no neutralizaba ninguna celda que empezara por `=`, `+`, `-` o `@` (inyección de fórmula CSV).** Un campo importado sin validar más allá de "no vacío" (p. ej. un nombre de alumno mal escrito, a propósito o por error, en la importación masiva de R-08) que empezara por uno de esos cuatro caracteres se reexportaba literal en el histórico de asistencia (T-23) o el expediente RGPD (R-10), y se ejecutaría como fórmula viva si esa exportación se reabre en una hoja de cálculo con fórmulas activadas — la misma clase de riesgo por la que las hojas de cálculo avisan de macros en un fichero de origen desconocido. **Implementado:** `escaparCampo` antepone ahora un apóstrofo a cualquier campo con uno de esos cuatro prefijos, antes del escapado por comillas ya existente — es el único punto por el que pasan `filaCsv`/`documentoCsv`/`documentoCsvConMetadatos`, sin excepción, así que cubre los tres puntos de exportación del proyecto de una vez. Mismo mecanismo de defensa que documenta OWASP para esta clase de vulnerabilidad | origen: hallazgo #16 de `auditoriacontinua.md` (severidad baja, `ABIERTO` desde 2026-09-09, "importación masiva (R-08)") — el propio hallazgo señalaba este mismo arreglo ("anteponer un apóstrofo... el único punto de exportación a CSV del proyecto") como "estándar y barato" | **IMPLEMENTADA 2026-09-09** — 5 tests nuevos en `csv.test.ts` (prefijo `=`, `+`/`-`/`@`, un valor que solo contiene el prefijo en medio y no se toca, y la combinación con el escapado por comillas ya existente). Verificación pre-push completa en verde (`npm run typecheck`, `npm run lint`, `npm test` 1522/1522, `npm run build`). Sin migración: no toca ninguna tabla ni política. Pendiente de que el auditor reevalúe y cierre el hallazgo #16 en su próxima pasada | — |
| P-25 | **Backlog técnico (deuda de calidad, no urgente): la importación masiva de R-08 no es idempotente ante un reintento.** `importarAlumnosValidados` (`src/datos/importacionMasiva.ts:62-69`) genera un `id` nuevo con `crypto.randomUUID()` en cada invocación, y no existe ninguna restricción de unicidad del lado del servidor que detecte un lote repetido (la detección de duplicados de R-08 es solo de cliente, contra los alumnos ya cargados antes de confirmar). Un corte de red justo después de que el `INSERT` en lote llegue al servidor pero antes de que la respuesta llegue al navegador deja la pantalla en estado de error con el mismo lote listo para reenviar: un segundo toque en «Confirmar importación» duplicaría los alumnos ya creados. Mismo patrón exacto que el resto del proyecto evita en todas partes con una clave de idempotencia (`peticion_id`, T-18; mismo mecanismo en R-07). **Implementado:** el `id` de cada fila `'nueva'` ya no lo genera `importarAlumnosValidados` (`datos/`, se ejecuta en cada reintento) — lo genera y fija UNA vez `ui/pantallaImportacionMasiva.ts`, al analizar el fichero (`deps.generarId`, nuevo, inyectable), y lo guarda en el estado del bloque (`idsPorFila`, indexado por `numeroFila`) para reutilizarlo tal cual en cualquier reintento del mismo lote. Un reintento tras un corte de red reenvía ahora el MISMO `id` por fila: si el primer intento sí llegó a escribir, el segundo choca con la clave primaria de `alumno` (ya única, sin ninguna migración nueva) y PostgREST responde `409`/`Conflicto` en vez de crear una fila duplicada. `datos/importacionMasiva.ts` gana el tipo `FilaAlumnoParaConfirmar` (`{id, datos}`) y ya no genera ningún `id` internamente. 3 tests nuevos (2 en `datos/importacionMasiva.test.ts`, 1 en `ui/pantallaImportacionMasiva.test.ts` que fuerza un primer intento fallido y comprueba que el reintento envía el mismo `id`) | origen: hallazgo #15 de `auditoriacontinua.md` (severidad media, `ABIERTO` desde 2026-09-09, "importación masiva (R-08)") | **IMPLEMENTADA Y VERIFICADA 2026-09-10** — verificación pre-push completa en verde. Sin migración. Pendiente de que el auditor reevalúe y cierre el hallazgo #15 | — |
| P-26 | **Backlog técnico (deuda de calidad, no urgente): dos defectos menores de `sw.js`, ninguno con impacto práctico hoy.** (a) `RESPUESTA_SIN_RED_NI_CACHE` es una única instancia de `Response` a nivel de módulo — el cuerpo de una `Response` es de un solo uso, así que si el Service Worker necesita servir este `fallback` más de una vez en su propia vida (p. ej. tras un precache parcial en el `install`, que ya se tolera como "best-effort"), la segunda vez lanzaría al intentar leer un cuerpo ya consumido, sirviendo el error de red crudo del navegador en vez del mensaje en español pensado para ese caso; arreglo: una función que construya una `Response` nueva en cada uso, no una constante compartida. (b) La comprobación de "mismo origen" del manejador de `fetch` (`peticion.url.startsWith(self.location.origin)`) compara por prefijo de cadena en vez de comparar el origin real (`new URL(peticion.url).origin === self.location.origin`); sin impacto hoy porque la `Content-Security-Policy` de `_headers` ya acota `connect-src`/`img-src` a los orígenes esperados, pero es una comparación más frágil de lo necesario para la única guarda que decide qué intercepta este Service Worker. **Implementado: ambos arreglos exactos que el propio hallazgo señalaba**, `respuestaSinRedNiCache()` (función, ya no constante) y `new URL(peticion.url).origin === self.location.origin` (ya no `startsWith`); `URL` añadido a los globales de `sw.js` en `eslint.config.js`. Sin test propio (mismo motivo que el resto de `sw.js`, JavaScript plano sin `tsc`, ver R-09 en `DECISIONES_TECNICAS.md`) | origen: hallazgo #17 de `auditoriacontinua.md` (severidad baja, `ABIERTO` desde 2026-09-09, "aplicación instalable (R-09)") | **IMPLEMENTADA 2026-09-10** — verificación pre-push completa en verde. Sin migración. Pendiente de que el auditor reevalúe y cierre el hallazgo #17 | — |
| P-27 | **Urgente (§0.3): `datos/centrosEstudios.ts#contarAlumnosActivosDeCentro` (T-11, `COMPLETADA` desde 2026-08-28) leía `centro_referencia_id` de la tabla BASE `alumno`, columna que `003_politicas_rls.sql` nunca concede a `authenticated` en ninguna forma — mismo defecto exacto que **P-22**, esta vez en la función que avisa "cuántos alumnos quedarían apuntando a un centro inactivo" antes de desactivarlo (requisito 3 de T-11), invocada solo desde la pantalla de gestión de centros, exclusiva de `administrator`.** Contra Postgres real habría dado "permission denied for column centro_referencia_id" para cualquier rol, incluido `administrator`, desde que T-11 se completó — sin detectar porque la suite corre contra dobles de `fetch`, que no imponen `GRANT` de columna. **Implementado:** `contarAlumnosActivosDeCentro` consulta ahora `alumno_ficha` (mismo patrón que `idsAlumnosDeCentro` de P-22 y `resolverCentroReferenciaIdDeAlumno` de R-04), filtrando además por `activo = true` como ya hacía; test de `centrosEstudios.test.ts` actualizado para exigir `/rest/v1/alumno_ficha` en vez de `/rest/v1/alumno` | origen: hallazgo #18 de `auditoriacontinua.md` (severidad alta, `ABIERTO` desde 2026-09-10, "catálogo de centros (T-11) / superficie de columnas") — el propio hallazgo señalaba este mismo arreglo como "evidente y de bajo riesgo", mismo patrón que P-22 | **IMPLEMENTADA Y VERIFICADA 2026-09-10** — verificación pre-push completa en verde (`npm run typecheck`, `npm run lint`, `npm test` 1522/1522, `npm run build`). Sin migración: no toca ninguna tabla ni política, solo qué relación consulta el cliente. Pendiente de que el auditor reevalúe y cierre el hallazgo #18 en su próxima pasada | — |
| P-28 | **Backlog técnico (hueco de cobertura, no urgente): el barrido obligatorio de `student` de `db/pruebas_rls.sql` (sección 6, T-10 requisito 5: "debe fallar en todas las tablas y en el bucket") nunca ejercitaba `storage.objects`/bucket `avatares` — la única combinación rol×recurso de todo el proyecto donde el rol más restringido no tenía un caso negativo ejecutable contra el recurso más sensible (fotos de menores). La política SQL en sí ya estaba correcta (ninguna política de `storage.objects` menciona a `student`): faltaba la prueba en ejecución que lo demostrara y actuara de guarda de regresión.** **Implementado:** nueva sección **7c** en `db/pruebas_rls.sql` (después de la sección 7, porque reutiliza sus fixtures `alumno_prueba`/`alumno_inactivo` y las dos filas de `storage.objects` que crea — la sección 6 corre antes de que existan), mismo patrón exacto que P-06 ya usó para `anon`: `student` no lee el avatar del alumno activo, no lee el del alumno inactivo (dos casos separados, cada uno confirmando primero con el rol de conexión que la fila de fixture sigue existiendo, para que "0 filas" no sea un falso positivo por falta de dato), y no puede insertar en el bucket (tercer caso, por simetría con la escritura que la sección 7 ya prueba para `teacher`) | origen: hallazgo #20 de `auditoriacontinua.md` (severidad media, `ABIERTO` desde 2026-09-14, "Autorización (RLS) / calidad de la batería de pruebas — bucket de avatares"); mismo patrón de hallazgo que #2 (batería que no ejercita un caso obligatorio de su propia spec) | **IMPLEMENTADA 2026-09-14** — verificación pre-push completa en verde (`npm run typecheck`, `npm run lint`, `npm test`, `npm run build`). Sin migración: solo añade comprobaciones a un script de prueba, no toca ninguna tabla ni política. El script no se ejecuta contra ninguna base real desde este entorno (§0.1, mismo régimen que el resto de `pruebas_rls.sql`): pendiente de que el dueño lo ejecute con `npm run probar-rls` (no requiere ninguna migración nueva: corre contra `dev` tal cual está hoy) y de que el auditor reevalúe y cierre el hallazgo #20 en su próxima pasada | — |
| P-30 | **Urgente (§0.3): `src/datos/asistencia.ts#actualizarAsistencia` enviaba los 13 parámetros de la firma AMPLIADA de `011`/`012` (R-02/R-03) a la RPC `actualizar_asistencia`, pero la función real desplegada en `dev` sigue siendo la de 8 parámetros de `db/008_rpc_actualizar_asistencia.sql` — ninguna de las dos migraciones está aplicada (`011` bloqueada además por el hallazgo #8, RGPD artículo 9). PostgREST resuelve una RPC por coincidencia EXACTA de nombres de parámetro: enviar uno que la función no declara hace fallar la resolución COMPLETA de la llamada, así que **ninguna** llamada a `actualizar_asistencia` podía tener éxito contra `dev` — ni siquiera "editar nota/hora/slot/alumno" o "anular" (T-21, `COMPLETADA` y verificada en ejecución el 2026-09-02; y "Anular" de pasar lista, R-24, `COMPLETADA` 2026-09-18), que no necesitan ningún campo de R-02/R-03.** Implementado (opción (c) del propio hallazgo): `actualizarAsistencia` vuelve a construir SOLO los 8 parámetros de la firma real desplegada; pedir `justificar`/`marcarSalida`/`ocurridoEnSalida` (R-02/R-03, sus botones ya están montados en «Registros» y en pasar lista) lanza ahora `AccionNoDisponibleTodavia` (nueva, `src/datos/erroresDominio.ts` — guardián de CLIENTE, nunca traducido de una respuesta del servidor, fuera de la taxonomía cerrada de ocho de T-08) ANTES de tocar la red o el límite de tasa de cliente, con mensaje propio en `mensajesAbuso.ts`, en vez de un 404 opaco o —peor— un envío silencioso que el servidor ignoraría sin decirlo. R-02 y R-03 siguen exactamente igual de `BLOQUEADA` que antes (nada nuevo se ha desbloqueado ni se ha tocado ninguna migración): lo que se corrige es que T-21/R-24, ya `COMPLETADA`s, vuelvan a funcionar de verdad en `dev` sin esperar a la pregunta #16 de §6 | origen: hallazgo #22 de `auditoriacontinua.md` (severidad alta, `ABIERTO` desde 2026-09-19, "Funcionalidad / coherencia código-esquema aplicado — `actualizar_asistencia`"); opción (c) de las tres que el propio hallazgo proponía | **IMPLEMENTADA 2026-09-21** — verificación pre-push completa en verde (`npm run typecheck`, `npm run lint`, `npm test`: 1746/1746, `npm run build`). Sin migración: solo cliente, no toca ninguna tabla ni política. Detalle en `DECISIONES_TECNICAS.md` | — |
| P-29 | **Urgente (§0.3): `db/018_baja_profesor.sql` (R-22) añade la tabla `baja_profesor` — la decimotercera tabla de `public` — pero ninguno de los tres barridos genéricos "todas las tablas" de `db/pruebas_rls.sql` se actualizó para incluirla: ni la sección 6 (barrido obligatorio de `student`, línea 743), ni la sección 8 (`TRUNCATE` por `authenticated`, línea 1204 — la comprobación más grave del fichero, la que ya falló una vez de verdad con `perfil`), ni el barrido de `anon` (línea 1858). Mismo patrón exacto que el hallazgo #10 (`RESUELTO`, atendido por P-18).** El `GRANT`/RLS real de `018_baja_profesor.sql:182-184` ya es correcto (`revoke all` seguido solo de `grant select` a `authenticated`), así que no hay fuga activa hoy — falta la red de regresión que lo demuestre y que atraparía el descuido inverso en una migración futura. **Implementado:** añadida `'baja_profesor'` a los tres arrays (líneas 743, 1204 y 1858 de `db/pruebas_rls.sql`) | origen: hallazgo #21 de `auditoriacontinua.md` (severidad alta, `ABIERTO` desde 2026-09-17, "Autorización (RLS) / calidad de la batería de pruebas — privilegios de tabla"); mismo patrón exacto que #10 | **IMPLEMENTADA 2026-09-17** — verificación pre-push completa en verde (`npm run typecheck`, `npm run lint`, `npm test`, `npm run build`). Sin migración: solo añade comprobaciones a un script de prueba, no toca ninguna tabla ni política. El script no se ejecuta contra ninguna base real desde este entorno (§0.1): pendiente de que el dueño lo confirme con su próximo `npm run probar-rls` tras aplicar `018` y de que el auditor reevalúe y cierre el hallazgo #21 en su próxima pasada | — |
| P-31 | **Backlog del auditor (no urgente, severidad media, §0.3: "por defecto, las P-XX se ejecutan cuando la tarea en curso está terminada o bloqueada" — con la Oleada v12 cerrada y ninguna R-XX nueva abierta, sin cola normal propia): P-30 (hallazgo #22) ya cortaba la llamada de red de "Justificar"/"Marcar salida" mientras `011`/`012` sigan sin aplicar, pero ninguna pantalla consultaba esa señal ANTES de pintar el control — un profesor o `administrator` podía rellenar el formulario entero para recibir, solo al pulsar, el mensaje de `AccionNoDisponibleTodavia`.** El botón "Marcar salida" es especialmente visible: aparece en el flujo diario principal de cada profesor (pasar lista), no en una pantalla secundaria. **Implementado:** `datos/asistencia.ts` gana `justificarAusenciaDisponible()`/`marcarSalidaDisponible()` (mismo criterio ya usado por `accionPendienteDeMigracion`, con el tipo `entrada` estrechado a solo los campos que necesita). `pantallaRegistrosSlot.ts`/`pantallaPasarLista.ts` las reciben como dependencia OPCIONAL (`deps.justificarAusenciaDisponible?()`/`deps.marcarSalidaDisponible?()`, con `?? true` si se omiten) y ocultan el bloque completo ("Justificar" en Registros; "Marcar salida"/"Guardar salida" en ambas pantallas) mientras devuelvan `false` — en vez de importar las funciones directamente, para no dejar en rojo permanente los tests existentes del flujo completo (doble toque, reconciliación de conflicto, deshabilitado en curso), que siguen siendo válidos en cuanto la migración se aplique. Wireadas en `aplicacion.ts` a las funciones reales en los tres puntos de montaje (`registros` de `administrator`/`teacher`, `pasar-lista`) | origen: hallazgo #23 de `auditoriacontinua.md` (severidad media, `ABIERTO` desde 2026-09-22, "Funcionalidad / UX — controles muertos por migración bloqueada"); el propio hallazgo señalaba esta misma corrección mínima ("exponerla como una función que la pantalla también pueda consultar antes de pintar el bloque") | **IMPLEMENTADA Y VERIFICADA 2026-09-22** — 5 tests nuevos (2 en `datos/asistencia.test.ts` fijando que ambas señales son `false` hoy; 1 en `pantallaPasarLista.test.ts` y 2 en `pantallaRegistrosSlot.test.ts` comprobando que los controles no se ofrecen con `() => false`). Verificación pre-push completa en verde (`npm run typecheck`, `npm run lint`, `npm test`: 1816/1816, `npm run build`). Sin migración: solo cliente, no toca ninguna tabla ni política. Detalle del patrón de inyección elegido en `DECISIONES_TECNICAS.md`. Pendiente de que el auditor reevalúe y cierre el hallazgo #23 en su próxima pasada | — |
| P-32 | **Backlog del auditor (no urgente, severidad baja, gobernanza documental, §0.3): la matriz rol × tabla × operación de `DECISIONES_TECNICAS.md` no tenía ninguna fila para `pausa_alumno` (R-21, `017_pausa_alumno.sql`), aunque sí la tenía para `baja_profesor` (R-22), la tarea inmediatamente siguiente — mismo patrón exacto que los hallazgos #4/#5/#6/#9/#11, todos ya `RESUELTO`.** Las políticas RLS reales de `pausa_alumno` ya eran correctas (sin ninguna política para `student`, cubiertas por los barridos de `student`/`TRUNCATE` de `db/pruebas_rls.sql`): solo faltaba la fila de referencia. **Implementado:** añadida la fila de `pausa_alumno` a la matriz, mismo formato que las demás | origen: hallazgo #24 de `auditoriacontinua.md` (severidad baja, `ABIERTO` desde 2026-09-22, "Gobernanza documental") | **IMPLEMENTADA 2026-09-22** — cambio de documentación pura, sin código ni migración. Verificación pre-push completa en verde (compartida con P-31, mismo commit). Pendiente de que el auditor reevalúe y cierre el hallazgo #24 en su próxima pasada | — |
...
| 18 | R-28 (Oleada v13) muestra un indicador de "ausencias repetidas" en la card de pasar lista y en Mi horario del profesor cuando un alumno acumula `UMBRAL_AVISO_AUSENCIAS_REPETIDAS` (valor por defecto: 3) ausencias sin justificar en los últimos `VENTANA_AVISO_AUSENCIAS_REPETIDAS_DIAS` (valor por defecto: 30) días. Son dos constantes de dominio nuevas, no una migración: cambiarlas no exige tocar el esquema. ¿Confirma el dueño estos dos valores, o prefiere un umbral/ventana distintos (p. ej. contar por número de sesiones en vez de por días naturales, para academias con horarios muy espaciados)? Mientras no haya respuesta, se usan los valores conservadores de la spec y esto no bloquea nada. | R-28 | |
